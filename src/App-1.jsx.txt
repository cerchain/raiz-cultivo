...
export default function App() {
  ...
  const cargarTodo = async () => {
    try {
      ...
      const salaId = salaR.data?.id;
      if(!salaId) return;

      const [spR, sR, gR, lR] = await Promise.all([
        supabase.from("sectores_padre").select("*").eq("sala_id", salaId).order("orden"),
        supabase.from("sectores").select("*").eq("sala_id", salaId),
        supabase.from("geneticas").select("*").eq("sala_id", salaId).order("nombre"),
        supabase.from("lotes").select("*").eq("sala_id", salaId).order("fecha_inicio", { ascending:false })
      ]);
        ...
        {sectoresPadre.find(sp=>sp.id===tab) && (()=>{
          ...
          return (
            <div className="fade">
              ...
              {subs.length===0 && (
                <div style={{textAlign:"center",padding:"40px 20px",color:"#3a5a2a",fontSize:"14px"}}>Sin subsectores configurados aún</div>
              )}
                    ...
                    {lotesSub.length>0 && (
                      <div style={{display:"flex",flexDirection:"column",gap:"6px",marginTop:"10px"}}>
                        {lotesSub.map(l=>{
                          ...
                          return (
                            <div key={l.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#111e11",borderRadius:"8px",padding:"8px 12px",border:`1px solid ${g?.color||"#2a4a2a"}44`}}>
                              <span style={{fontSize:"13px",color:g?.color||"#9aaa9a"}}>{g?.nombre||"Genética desconocida"}</span>
