...
export default function Invitacion({ usuarioId, sesionActiva, onCerrarSesion, onIrALogin }) {
  ...
  const handleAceptar = async (e) => {
    ...
    setEnviando(true);
    try {
      // El endpoint crea la cuenta YA CONFIRMADA (sin depender del mail) y la
      // vincula al usuario. Reemplaza al viejo signUp(), que con la confirmación
      // de email activada dejaba la cuenta sin confirmar y sin vincular.
      const resp = await fetch("/api/aceptar-invitacion", {
        method: "POST",
      ...
      if (!resp.ok) {
        setError(out.error || "No se pudo aceptar la invitación. Probá de nuevo.");
        setEnviando(false);
        return;
      }
      // Cuenta lista y confirmada: iniciar sesión directo, sin mail de por medio.
      const { error: loginError } = await supabase.auth.signInWithPassword({
        email: invitacion.email,
        password
      });
      if (loginError) {
        setError("Cuenta creada. Iniciá sesión con tu email y esta contraseña.");
        setEnviando(false);
        setTimeout(() => onIrALogin && onIrALogin(), 1500);
        return;
      }
  ...
  if (sesionActiva && invitacion) return shell(
    <>
      <div style={{ fontSize: "36px", marginBottom: "10px" }}>👤</div>
      <div style={{ color: "#dff0cf", fontSize: "13px", lineHeight: "1.6", marginBottom: "8px" }}>
        Este dispositivo tiene una sesión abierta como <b>{sesionActiva.nombre}</b>.
      </div>
      <div style={{ color: "#8ab88a", fontSize: "12px", lineHeight: "1.6", marginBottom: "18px" }}>
        Esta invitación es para <b>{invitacion.email}</b>. Para aceptarla hay que cerrar la sesión de {sesionActiva.nombre} primero.
      </div>
      <button disabled={cerrandoSesion} onClick={async ()=>{ setCerrandoSesion(true); await onCerrarSesion(); }} style={{ width: "100%", padding: "12px", borderRadius: "10px", fontSize: "13px", fontWeight: "600", background: "#3a6a10", color: "#dff0cf", border: "1px solid #5a9a20" }}>
        {cerrandoSesion ? "Cerrando sesión..." : "Cerrar esa sesión y continuar"}
      </button>
    </>
  );
