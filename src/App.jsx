                          ...
                          return (
                            <div key={l.id} style={{background:"#111e11",borderRadius:"8px",padding:"10px 12px",border:`1px solid ${g?.color||"#2a4a2a"}44`}}>
                              ...
                              {sp.nombre==="Floración" && sub.fecha_inicio_floracion && (()=>{
                                ...
                                if(!g2?.semanas_floracion) return null;
                                const fechaCorte = new Date(sub.fecha_inicio_floracion);
                                fechaCorte.setDate(fechaCorte.getDate() + g2.semanas_floracion*7);
                                return <div style={{fontSize:"11px",color:"#c8a020",marginBottom:"6px"}}>🗓 Estimado listo para corte: {fechaCorte.toISOString().slice(0,10)} ({g2.semanas_floracion} sem.)</div>;
                              })()}
          ...
          <div className="fade">
            ...
            {(()=>{
              ...
              const gruposCosecha = Object.entries(porGenetica).map(([gid,items])=>{
                ...
                return { key:gid, g, nombre:g?.nombre||"Sin genética", items:ordenados, totalG };
                      ...
                      return (
                        <div key={grupo.key} style={{border:`1px solid ${color}44`,borderRadius:"14px",overflow:"hidden"}}>
                          <div onClick={()=>setCosechaGrupoAbierto(prev=>({...prev,[grupo.key]:!prev[grupo.key]}))} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:abierto?`${color}14`:"transparent"}}>
                            <span style={{fontSize:"14px",fontWeight:"600",color}}>{grupo.nombre} <span style={{fontSize:"11px",color:"#7a9a6a",fontWeight:"400"}}>· {grupo.items.length} cosecha{grupo.items.length!==1?"s":""} · {grupo.totalG.toFixed(0)}g</span></span>
