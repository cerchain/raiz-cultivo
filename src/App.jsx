    ...
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
      ...
      <div style={{padding:"20px 20px 110px"}}>
        ...
        {tab==="_reportes" && !esEjecutor && (()=>{
          // Promedio por genética, sobre stats_lotes (RPC). Regla: una etapa cuenta
          // para el promedio SOLO si el lote ya la CUMPLIÓ (la dejó atrás en el
          // pipeline Enraizado→Vegetativo→Floración→Secado). La etapa en curso y las
          // futuras no entran — así el promedio refleja duraciones reales de ciclos
          // terminados, no ciclos a mitad de camino.
          const ordenEtapa = { "Enraizado":0, "Vegetativo":1, "Floración":2, "Secado":3 };
          const promedioPorGenetica = () => {
            ...
            lotes.forEach(l=>{
              ...
              // Orden de la etapa EN CURSO. Solo cuentan las etapas con orden < curOrden.
              let curOrden;
              if(l.estado==="cosechado"){
                // La cosecha cierra floración. Secado se considera cumplido solo si hay días reales.
                curOrden = (dias["Secado"]>0) ? 4 : 3;
              } else {
                const sAct = sectores.find(x=>x.id===l.sector_actual_id);
                let spAct = sAct ? sectoresPadre.find(x=>x.id===sAct.sector_padre_id)?.nombre : null;
                // En carpa de floración sin 12/12 activo la planta sigue vegetando: aún no entró a floración.
                if(spAct==="Floración"){
                  const con1212 = l.fecha_inicio_floracion || sAct?.fecha_inicio_floracion;
                  if(!con1212) spAct = "Vegetativo";
                }
                curOrden = (spAct!=null && ordenEtapa[spAct]!=null) ? ordenEtapa[spAct] : -1;
              }
              etapasNom.forEach(et=>{
                if(ordenEtapa[et] >= curOrden) return;  // etapa en curso o futura: no cumplida
                if(!(dias[et]>0)) return;               // etapa que este lote no transitó
