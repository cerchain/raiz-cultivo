...
function RaizApp() {
  ...
  const carpaCerradaAlIngreso = (sub) => {
    ...
    return !!sub.fecha_inicio_floracion;
  };
  ...
  const destinosPermitidos = (subOrigenId) => {
    ...
    return sectores.filter(s=>{
      ...
      if(carpaCerradaAlIngreso(s)) return false; // carpa con 12/12 ya activado
      return true;
    });
  };
  ...
  const confirmarMovimiento = async () => {
    ...
    if(carpaCerradaAlIngreso(subDest)){ setStatus(`${subDest.nombre} ya tiene el 12/12 activado — no puede recibir plantas nuevas hasta terminar el ciclo`); setTimeout(()=>setStatus(""),7000); return; }
  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"var(--c074)",minHeight:"100vh",color:"var(--c019)"}}>
      ...
      {movingLote && (()=>{
        ...
        const subsDestinoBruto = modoMover ? destinosPermitidos(movingLote.sector_actual_id) : modoRetroceso ? (spVegetativo ? subsectoresDe(spVegetativo.id) : []) : (spDestinoNatural ? subsectoresDe(spDestinoNatural.id) : []);
        const subsDestino = subsDestinoBruto.filter(s=>!carpaCerradaAlIngreso(s));
        const carpasCerradas = subsDestinoBruto.filter(s=>carpaCerradaAlIngreso(s));
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"var(--c073)",border:"1px solid var(--c011)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              ...
              {carpasCerradas.length>0 && (
                <div style={{fontSize:"11px",color:"var(--c088)",marginBottom:"8px",lineHeight:"1.5"}}>
                  🔒 No figura{carpasCerradas.length>1?"n":""} {carpasCerradas.map(s=>s.nombre).join(", ")}: {carpasCerradas.length>1?"esas carpas ya tienen":"esa carpa ya tiene"} el 12/12 activado y no {carpasCerradas.length>1?"aceptan":"acepta"} plantas nuevas hasta terminar el ciclo.
                </div>
              )}
              {subsDestino.length===0 && (
                <div style={{fontSize:"12px",color:"var(--c022)",background:"var(--c094)",borderRadius:"8px",padding:"10px 12px",marginBottom:"12px"}}>
                  No hay ningún subsector disponible para recibir estas plantas ahora mismo.
                </div>
              )}
