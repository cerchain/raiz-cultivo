...
function RaizApp() {
  ...
  const confirmarEliminarEntidad = async () => {
    ...
    try {
      await supabase.from("entidades").delete().eq("id", eliminandoEntidad.entidad.id);
    ...
    try {
      ...
      if(insS.data){
        ...
        if(creandoSalaEntidad.mgNombre && creandoSalaEntidad.mgEmail){
          ...
          if(insU.data) await supabase.from("salas").update({master_grower_id: insU.data.id}).eq("id", insS.data.id);
        }
      }
  ...
  if(esAdminEntidad && adminEntidadModo==="entidad"){
    ...
    const salaDelMG = entidadSalas.find(s=>s.id===usuario.sala_mg_id);
    return (
        ...
        <div style={{background:"#10142a",borderBottom:"2px solid #2a2a5a",padding:"18px 20px"}}>
          ...
          {usuario.es_master_grower && (
            <button onClick={entrarModoMG} style={{marginTop:"12px",width:"100%",padding:"11px",borderRadius:"10px",fontSize:"13px",fontWeight:"600",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>
              🌱 Entrar como Master Grower — {salaDelMG?.nombre||"mi sala"}
            </button>
          )}
        </div>
            ...
            {entidadSalas.map(sala=>{
              ...
              return (
                <div key={sala.id} style={{background:"#12162a",border:"1px solid #2a2a5a",borderRadius:"14px",padding:"16px"}}>
                  ...
                  <button onClick={()=>setCreandoEjecutorEntidad({nombre:"",email:"",salaId:sala.id})} style={{marginTop:"10px",padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#102a18",color:"#7ec850",border:"1px solid #205a2a"}}>+ Nuevo Ejecutor para esta sala</button>
                </div>
              );
            })}
          </div>
