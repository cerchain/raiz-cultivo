              ...
              {tareasDelDia.length===0 ? (
                ...
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                  {tareasDelDia.map(t=>{
                    ...
                    const esMia = t.asignado_a===usuario.id;
                    return (
                      <div key={t.id} style={{background:"#0e1a24",border:`1px solid ${TAREA_ESTADO_COLOR[estadoMostrado]}55`,borderRadius:"12px",padding:"14px",borderLeft:`4px solid ${TAREA_PRIORIDAD_COLOR[t.prioridad]||"#5a7a9a"}`}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"6px"}}>
                          <div style={{fontSize:"14px",fontWeight:"600",color:"#dfe8f0"}}>{t.titulo}</div>
                          <span style={{fontSize:"10px",padding:"3px 8px",borderRadius:"8px",background:`${TAREA_ESTADO_COLOR[estadoMostrado]}33`,color:TAREA_ESTADO_COLOR[estadoMostrado],whiteSpace:"nowrap"}}>{TAREA_ESTADO_LABEL[estadoMostrado]}</span>
                        </div>
                        <div style={{fontSize:"11px",color:"#6a8aa0",marginBottom:"6px"}}>{tipoObj?`${tipoObj.icono} ${tipoObj.nombre}`:""}{sp?` · ${sp.nombre}${sub?` › ${sub.nombre}`:""}`:""}</div>
                        <div style={{fontSize:"11px",color:"#5a7a90",marginBottom:"8px"}}>{t.asignado_a?`→ ${usuariosMap[t.asignado_a]||"—"}`:"sin asignar"}</div>
                        <div style={{display:"flex",gap:"6px"}}>
                          {esMia && t.estado==="pendiente" && (
                            <button onClick={()=>updateEstadoTarea(t,"en_progreso")} style={{flex:1,padding:"6px",borderRadius:"6px",fontSize:"10px",background:"#2a2a10",color:"#c8a020",border:"1px solid #4a4a20"}}>▶ Iniciar</button>
                          )}
                          {esMia && (t.estado==="pendiente"||t.estado==="en_progreso") && (
                            <button onClick={()=>updateEstadoTarea(t,"completada")} style={{flex:1,padding:"6px",borderRadius:"6px",fontSize:"10px",background:"#0e2a1a",color:"#3a8a5a",border:"1px solid #1a5a3a"}}>✓ Completar</button>
                          )}
                          {!esEjecutor && (
                            <button onClick={()=>setEditingTarea({...t, lote_id:t.lote_id||"", sector_id:t.sector_id||"", asignado_a:t.asignado_a||"", descripcion:t.descripcion||"", tipo_id:t.tipo_id||""})} style={{padding:"6px 10px",borderRadius:"6px",fontSize:"10px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>✏️</button>
                          )}
                        </div>
                      </div>
                    );
        ...
        {tab==="_misala" && !esEjecutor && (
          <div className="fade">
            ...
            <div style={card({borderColor:"#0e6a5a",background:"#0a1e1a"})}>
              <span style={lbl("#16a085")}>TIPOS DE TAREA</span>
              {editingTipoTarea ? (
                <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                  <div style={{display:"flex",gap:"8px"}}>
