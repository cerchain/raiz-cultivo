...
const FASE_ICONOS = ["🌱","🪴","🌸","🟫","🟫","🟫"];
const colorDeFase = (orden) => FASE_COLORES[(orden-1+FASE_COLORES.length) % FASE_COLORES.length];
...
class ErrorBoundary extends Component {
  ...
  render(){
    if(this.state.error){
      return (
        <div style={{background:"#0e160e",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:"16px",padding:"30px",fontFamily:"'IBM Plex Mono',monospace",color:"#dff0cf",textAlign:"center"}}>
          <div style={{fontSize:"40px"}}>🌱💥</div>
          <div style={{fontSize:"16px",color:"#e0b850"}}>Algo se rompió en pantalla</div>
          <div style={{fontSize:"12px",color:"#7a9a7a",maxWidth:"320px"}}>Tus datos en Supabase no se tocaron — esto es solo un error de la interfaz. Recargá la página.</div>
          <button onClick={()=>window.location.reload()} style={{padding:"12px 24px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"#3a6a10",color:"#dff0cf",border:"1px solid #5a9a20"}}>↺ Recargar</button>
        </div>
      );
    }
...
function RaizApp() {
    ...
    (async()=>{
      try {
        ...
        if(authUser){
          ...
          if(r.data && r.data.activo!==false) setUsuario(r.data);
  ...
  const agruparTareasPorSector = (lista) => {
    ...
    if(grupos["_sin"]) ordenados.push({key:"_sin", label:"Sin sector asignado", color:"#5a7a7a", tareas:grupos["_sin"]});
    return ordenados;
  };
  ...
  const TareaCard = ({t}) => {
    ...
    return (
      <div style={{background:"#0e1a18",border:`1px solid ${TAREA_ESTADO_COLOR[estadoMostrado]}55`,borderRadius:"14px",padding:"16px",borderLeft:`4px solid ${TAREA_PRIORIDAD_COLOR[t.prioridad]||"#5a7a9a"}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}>
          <div>
            <div style={{fontSize:"15px",fontWeight:"600",color:"#dff0e8"}}>{t.titulo}</div>
            <div style={{fontSize:"12px",color:"#6a9a8a",marginTop:"2px"}}>{tipoObj ? `${tipoObj.icono} ${tipoObj.nombre}` : (TAREA_TIPO_LABEL[t.tipo] || t.tipo || "Sin tipo")}</div>
          </div>
        ...
        <div style={{display:"flex",gap:"6px",marginBottom:"10px"}}>
          ...
          {!esEjecutor && (
            <button onClick={()=>setEditingTarea({...t, lote_id:t.lote_id||"", sector_id:t.sector_id||"", asignado_a:t.asignado_a||"", descripcion:t.descripcion||"", tipo_id:t.tipo_id||""})} style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>✏️</button>
          )}
          {!esEjecutor && (
  ...
  if(esAdminEntidad && adminEntidadModo==="entidad"){
    if(!entidadLoaded) return <div style={{background:"#0a0e1a",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Mono',monospace",color:"#5a6a9a",fontSize:"14px"}}>Cargando panel de entidad...</div>;
    ...
    return (
      <div style={{background:"#0a0e1a",minHeight:"100vh",fontFamily:"'IBM Plex Mono',monospace",color:"#dfe0f0"}}>
        <div style={{background:"#10142a",borderBottom:"2px solid #2a2a5a",padding:"18px 20px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"21px",color:"#7a8af0"}}>🏢 {entidadNombre}</div>
              <div style={{fontSize:"11px",color:"#5a5a9a",letterSpacing:"2px",marginTop:"4px"}}>{usuario.nombre} · ADMINISTRADOR DE ENTIDAD</div>
            </div>
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"6px"}}>
              {status && <div style={{fontSize:"11px",color:status==="guardado"?"#7ec850":"#e74c3c",background:"#1a1a3a",padding:"4px 10px",borderRadius:"10px"}}>{status}</div>}
              <button onClick={handleLogout} style={{fontSize:"11px",color:"#7a7aba",background:"transparent",border:"1px solid #2a2a5a",borderRadius:"14px",padding:"4px 12px"}}>salir</button>
            </div>
          </div>
          {usuario.es_master_grower && misSalasComoMG.length>0 && (
            <button onClick={()=>entrarModoMG()} style={{marginTop:"12px",width:"100%",padding:"11px",borderRadius:"10px",fontSize:"13px",fontWeight:"600",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>
        ...
        {eligiendoSalaMG && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"#12162a",border:"1px solid #2a5a2a",borderRadius:"16px",padding:"20px",width:"100%",maxWidth:"360px"}}>
              <div style={{fontSize:"13px",color:"#7ec850",marginBottom:"14px"}}>¿A qué sala querés entrar?</div>
              <div style={{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"12px"}}>
                {eligiendoSalaMG.map(s=>(
                  <button key={s.id} onClick={()=>entrarModoMG(s.id)} style={{padding:"12px",borderRadius:"10px",fontSize:"13px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a",textAlign:"left"}}>{s.nombre}</button>
                ))}
              </div>
              <button onClick={()=>setEligiendoSalaMG(null)} style={{width:"100%",padding:"10px",borderRadius:"10px",fontSize:"12px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
            </div>
          </div>
        )}

        <div style={{padding:"18px"}}>
          {totalAtrasadas>0 && <div style={{background:"#2a1010",border:"1px solid #5a2020",borderRadius:"10px",padding:"10px 14px",marginBottom:"16px",fontSize:"12px",color:"#e07070"}}>⚠ {totalAtrasadas} tarea{totalAtrasadas!==1?"s":""} atrasada{totalAtrasadas!==1?"s":""} en toda la entidad</div>}

          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"10px",marginBottom:"20px"}}>
            <div style={{background:"#12162a",borderRadius:"12px",padding:"14px 10px",textAlign:"center",border:"1px solid #2a2a5a"}}>
              <div style={{fontSize:"22px",fontWeight:"700",color:"#7a8af0"}}>{entidadSalas.length}</div>
              <div style={{fontSize:"9px",color:"#5a6a9a",marginTop:"4px"}}>SALAS</div>
            </div>
            <div style={{background:"#12162a",borderRadius:"12px",padding:"14px 10px",textAlign:"center",border:"1px solid #2a2a5a"}}>
              <div style={{fontSize:"22px",fontWeight:"700",color:"#7ec850"}}>{totalPlantas}</div>
              <div style={{fontSize:"9px",color:"#5a6a9a",marginTop:"4px"}}>PLANTAS ACTIVAS</div>
            </div>
            <div style={{background:"#12162a",borderRadius:"12px",padding:"14px 10px",textAlign:"center",border:"1px solid #2a2a5a"}}>
              <div style={{fontSize:"22px",fontWeight:"700",color:"#c8a020"}}>{totalGramos.toFixed(0)}g</div>
              <div style={{fontSize:"9px",color:"#5a6a9a",marginTop:"4px"}}>PRODUCCIÓN</div>
            </div>
          </div>

          <div style={{marginBottom:"24px",border:"1px solid #2a2a5a",borderRadius:"14px",background:"#0e1226",overflow:"hidden"}}>
            <div onClick={()=>setSalasEntidadAbierto(!salasEntidadAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:"#10142a"}}>
              <span style={{fontSize:"11px",color:"#7a8af0",letterSpacing:"2px",fontWeight:"600"}}>🚪 SALAS · {entidadSalas.length}</span>
              <span style={{fontSize:"13px",color:"#7a8af0",transform:salasEntidadAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
            </div>
            {salasEntidadAbierto && (<div style={{padding:"14px"}}>

          {creandoSalaEntidad ? (
            <div style={{background:"#12162a",border:"1px dashed #3a3a7a",borderRadius:"12px",padding:"14px",marginBottom:"14px"}}>
              <div style={{fontSize:"10px",color:"#7a8af0",letterSpacing:"1px",marginBottom:"8px"}}>NUEVA SALA</div>
              <input value={creandoSalaEntidad.nombre||""} onChange={e=>setCreandoSalaEntidad({...creandoSalaEntidad,nombre:e.target.value})} placeholder="Nombre de la sala" style={{width:"100%",background:"#0a0e1a",border:"1px solid #2a2a5a",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"8px"}}/>
              <div style={{fontSize:"9px",color:"#5a6a9a",marginBottom:"6px"}}>MASTER GROWER (opcional, podés asignarlo después)</div>
              <input value={creandoSalaEntidad.mgNombre||""} onChange={e=>setCreandoSalaEntidad({...creandoSalaEntidad,mgNombre:e.target.value})} placeholder="Nombre del MG" style={{width:"100%",background:"#0a0e1a",border:"1px solid #2a2a5a",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"8px"}}/>
              <input value={creandoSalaEntidad.mgEmail||""} onChange={e=>setCreandoSalaEntidad({...creandoSalaEntidad,mgEmail:e.target.value})} placeholder="Email del MG" style={{width:"100%",background:"#0a0e1a",border:"1px solid #2a2a5a",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"10px"}}/>
              <div style={{display:"flex",gap:"8px"}}>
                <button disabled={guardandoSalaEntidad || !creandoSalaEntidad.nombre} onClick={crearSalaEntidad} style={{flex:1,padding:"9px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"#4a4ad0",color:"#fff",border:"none"}}>{guardandoSalaEntidad?"Guardando...":"✓ Crear sala"}</button>
                <button onClick={()=>setCreandoSalaEntidad(null)} style={{padding:"9px 14px",borderRadius:"8px",fontSize:"12px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
              </div>
            </div>
          ) : (
            <button onClick={()=>setCreandoSalaEntidad({nombre:"",mgNombre:"",mgEmail:""})} style={{width:"100%",padding:"12px",borderRadius:"10px",fontSize:"12px",fontWeight:"600",background:"#12162a",color:"#7a8af0",border:"2px dashed #3a3a7a",marginBottom:"14px"}}>+ Nueva Sala</button>
          )}
          <div style={{display:"flex",flexDirection:"column",gap:"10px",marginBottom:"24px"}}>
            {entidadSalas.map(sala=>{
              ...
              const mgColor = mg ? "#7a9af0" : "#c0392b";
              return (
                <div key={sala.id} style={{background:"#12162a",border:"1px solid #2a2a5a",borderRadius:"14px",padding:"16px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                    <div>
                      <div style={{fontSize:"15px",fontWeight:"600",color:"#e8e8f8"}}>{sala.nombre}</div>
                      <div style={{fontSize:"12px",color:mgColor,marginTop:"3px"}}>{mgLabel}</div>
                    </div>
                    <button onClick={()=>setEliminandoSalaEntidad({sala, confirmText:""})} style={{padding:"4px 9px",borderRadius:"6px",fontSize:"10px",background:"#2a1010",color:"#aa5050",border:"1px solid #4a2020",whiteSpace:"nowrap"}}>🗑 Eliminar sala</button>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"6px"}}>
                    <div style={{background:"#0a0e1e",borderRadius:"8px",padding:"8px 4px",textAlign:"center"}}>
                      <div style={{fontSize:"16px",fontWeight:"700",color:"#7ec850"}}>{d.plantasActivas}</div>
                      <div style={{fontSize:"8px",color:"#4a5a7a",marginTop:"2px"}}>PLANTAS</div>
                    </div>
                    <div style={{background:"#0a0e1e",borderRadius:"8px",padding:"8px 4px",textAlign:"center"}}>
                      <div style={{fontSize:"16px",fontWeight:"700",color:d.tareasAtrasadas>0?"#c0392b":"#c8a020"}}>{d.tareasPendientes}</div>
                      <div style={{fontSize:"8px",color:"#4a5a7a",marginTop:"2px"}}>TAREAS</div>
                    </div>
                    <div style={{background:"#0a0e1e",borderRadius:"8px",padding:"8px 4px",textAlign:"center"}}>
                      <div style={{fontSize:"16px",fontWeight:"700",color:"#c8a020"}}>{d.cosechasTotal}</div>
                      <div style={{fontSize:"8px",color:"#4a5a7a",marginTop:"2px"}}>COSECHAS</div>
                    </div>
                    <div style={{background:"#0a0e1e",borderRadius:"8px",padding:"8px 4px",textAlign:"center"}}>
                      <div style={{fontSize:"16px",fontWeight:"700",color:"#7ec850"}}>{d.gramosTotal.toFixed(0)}g</div>
                      <div style={{fontSize:"8px",color:"#4a5a7a",marginTop:"2px"}}>GRAMOS</div>
                    </div>
                  </div>
                  {d.tareasAtrasadas>0 && <div style={{marginTop:"8px",fontSize:"10px",color:"#c0392b"}}>⚠ {d.tareasAtrasadas} tarea{d.tareasAtrasadas!==1?"s":""} atrasada{d.tareasAtrasadas!==1?"s":""}</div>}
                  <div style={{display:"flex",gap:"8px",flexWrap:"wrap",marginTop:"10px"}}>
                    <button onClick={()=>setCreandoEjecutorEntidad({nombre:"",email:"",salaId:sala.id})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#102a18",color:"#7ec850",border:"1px solid #205a2a"}}>+ Nuevo Ejecutor para esta sala</button>
                    {!sala.master_grower_id ? (
                      <>
                        <button onClick={()=>setAgregandoMGSala({salaId:sala.id, salaNombre:sala.nombre, nombre:"", email:""})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#1a1a2e",color:"#7a8af0",border:"1px solid #3a3a7a"}}>🌱 Agregar Master Grower</button>
                        <button disabled={asignandoComoMG} onClick={()=>asignarmeComoMG(sala)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#12162a",color:"#5a6a9a",border:"1px solid #2a2a5a"}}>{asignandoComoMG?"Asignando...":"Asignarme a mí"}</button>
                      </>
                    ) : (mg && mg.activo===false) ? (
                      <button onClick={()=>reactivarODesactivarUsuarioEntidad(mg)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#102a18",color:"#7ec850",border:"1px solid #205a2a"}}>↺ Reactivar MG ({mg.nombre})</button>
                    ) : (
                      <button disabled style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#0e1020",color:"#4a5372",border:"1px solid #23264a",cursor:"default"}}>✓ MG asignado{mg?": "+mg.nombre:""}</button>
                    )}
                  </div>
                </div>
              );
          ...
          {linkInvitacion && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"#12162a",border:"1px solid #2a5a2a",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontSize:"13px",color:"#7ec850",marginBottom:"10px"}}>✓ Cuenta creada para {linkInvitacion.nombre}</div>
              <div style={{fontSize:"11px",color:"#8ab88a",marginBottom:"12px",lineHeight:"1.6"}}>Todavía no puede loguearse. Mandale este link por WhatsApp o el medio que uses — al abrirlo va a poder poner su propia contraseña.</div>
              <div style={{background:"#0a0e1a",border:"1px solid #2a4a2a",borderRadius:"8px",padding:"10px 12px",fontSize:"11px",color:"#dfe0f0",wordBreak:"break-all",marginBottom:"14px"}}>{linkInvitacion.url}</div>
              <div style={{display:"flex",gap:"8px"}}>
                <button onClick={()=>{navigator.clipboard.writeText(linkInvitacion.url); setStatus("link copiado"); setTimeout(()=>setStatus(""),1800);}} style={{flex:1,padding:"11px",borderRadius:"9px",fontSize:"12px",fontWeight:"600",background:"#3a6a10",color:"#dff0cf",border:"1px solid #5a9a20"}}>📋 Copiar link</button>
                <button onClick={()=>setLinkInvitacion(null)} style={{padding:"11px 16px",borderRadius:"9px",fontSize:"12px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cerrar</button>
              </div>
            </div>
          </div>
        )}
        ...
        {eliminandoSalaEntidad && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"#1e0e0e",border:"1px solid #6a2020",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
                <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"18px",color:"#e07070",marginBottom:"10px"}}>⚠ Eliminar "{eliminandoSalaEntidad.sala.nombre}"</div>
                {(() => {
                  ...
                  return tieneDatos ? (
                    <div style={{background:"#2a1608",border:"1px solid #5a4020",borderRadius:"10px",padding:"10px 12px",fontSize:"11px",color:"#d0a860",marginBottom:"12px"}}>
                    ...
                    <div style={{fontSize:"12px",color:"#c0a0a0",marginBottom:"14px",lineHeight:"1.6"}}>Esta sala está vacía. Se borra definitivamente.</div>
                  );
                })()}
                <div style={{fontSize:"10px",color:"#c89090",letterSpacing:"1px",marginBottom:"6px"}}>ESCRIBÍ "{eliminandoSalaEntidad.sala.nombre}" PARA CONFIRMAR</div>
                <input value={eliminandoSalaEntidad.confirmText} onChange={e=>setEliminandoSalaEntidad({...eliminandoSalaEntidad,confirmText:e.target.value})} style={{width:"100%",background:"#0a0e1a",border:"1px solid #4a2020",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"14px"}}/>
                <div style={{display:"flex",gap:"10px"}}>
                  <button disabled={borrandoSalaEntidad || eliminandoSalaEntidad.confirmText!==eliminandoSalaEntidad.sala.nombre} onClick={confirmarEliminarSalaEntidad} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"#7a2020",color:"#fff",border:"1px solid #aa3030"}}>{borrandoSalaEntidad?"Eliminando...":"🗑 Eliminar definitivamente"}</button>
                  <button onClick={()=>setEliminandoSalaEntidad(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {creandoEjecutorEntidad && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"#12162a",border:"1px solid #3a3a7a",borderRadius:"16px",padding:"20px",width:"100%",maxWidth:"360px"}}>
                <div style={{fontSize:"11px",color:"#7a8af0",letterSpacing:"1px",marginBottom:"12px"}}>NUEVO EJECUTOR</div>
                <input value={creandoEjecutorEntidad.nombre} onChange={e=>setCreandoEjecutorEntidad({...creandoEjecutorEntidad,nombre:e.target.value})} placeholder="Nombre" style={{width:"100%",background:"#0a0e1a",border:"1px solid #2a2a5a",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"8px"}}/>
                <input value={creandoEjecutorEntidad.email} onChange={e=>setCreandoEjecutorEntidad({...creandoEjecutorEntidad,email:e.target.value})} placeholder="Email" style={{width:"100%",background:"#0a0e1a",border:"1px solid #2a2a5a",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"14px"}}/>
                <div style={{display:"flex",gap:"8px"}}>
                  <button disabled={guardandoEjecutorEntidad || !creandoEjecutorEntidad.nombre || !creandoEjecutorEntidad.email} onClick={crearEjecutorEntidad} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"#4a4ad0",color:"#fff",border:"none"}}>{guardandoEjecutorEntidad?"Guardando...":"✓ Crear"}</button>
                  <button onClick={()=>setCreandoEjecutorEntidad(null)} style={{padding:"10px 14px",borderRadius:"8px",fontSize:"12px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}

          <div style={{fontSize:"10px",color:"#5a6a9a",letterSpacing:"2px",marginBottom:"12px"}}>USUARIOS DE LA ENTIDAD ({entidadUsuarios.length})</div>
          <div style={{display:"flex",flexDirection:"column",gap:"8px",paddingBottom:"30px"}}>
            {entidadUsuarios.map(u=>{
              ...
              const btnBg = u.activo===false?"#102a18":"#2a1010";
              const btnColor = u.activo===false?"#7ec850":"#aa5050";
              const btnBorder = u.activo===false?"#205a2a":"#4a2020";
              ...
              return (
                <div key={u.id} style={{background:"#12162a",borderRadius:"10px",padding:"10px 14px",opacity:u.activo===false?0.5:1,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:"13px",color:"#e0e0f0"}}>{u.nombre}</div>
                    <div style={{fontSize:"10px",color:"#5a6a9a",marginTop:"2px"}}>{rolLabel}{u.es_master_grower?" + MG":""}{u.activo===false?" · INACTIVO":""}{!u.auth_id?" · SIN CUENTA CREADA":""}</div>
                    <div style={{fontSize:"10px",color:"#6a7aaa",marginTop:"3px"}}>📍 {ubicacion}</div>
                    {u.email && <div style={{fontSize:"10px",color:"#4a5a7a",marginTop:"2px"}}>{u.email}</div>}
                  </div>
                  {u.id!==usuario.id && (
                    <div style={{display:"flex",gap:"6px",flexWrap:"wrap",justifyContent:"flex-end"}}>
                      <button onClick={()=>setEditandoUsuarioEntidad({id:u.id, nombre:u.nombre||"", email:u.email||""})} style={{padding:"5px 10px",borderRadius:"6px",fontSize:"10px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>✏️</button>
                      {!u.auth_id && (
                        <button onClick={()=>{navigator.clipboard.writeText(window.location.origin+"/?invite="+u.id); setStatus("link copiado"); setTimeout(()=>setStatus(""),1800);}} style={{padding:"5px 10px",borderRadius:"6px",fontSize:"10px",background:"#1a1a2e",color:"#7a8af0",border:"1px solid #3a3a7a"}}>📋 Invitación</button>
                      )}
                      ...
                      <button onClick={()=>setEliminandoUsuarioEntidad({usuario:u, confirmText:""})} style={{padding:"5px 10px",borderRadius:"6px",fontSize:"10px",background:"#2a1010",color:"#e05050",border:"1px solid #5a2020"}}>🗑</button>
                    </div>
                  )}
                </div>
              );
          ...
          {eliminandoUsuarioEntidad && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"#1e0e0e",border:"1px solid #6a2020",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
                <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"18px",color:"#e07070",marginBottom:"10px"}}>⚠ Eliminar a "{eliminandoUsuarioEntidad.usuario.nombre}"</div>
                <div style={{fontSize:"12px",color:"#c0a0a0",marginBottom:"14px",lineHeight:"1.6"}}>Esto borra al usuario definitivamente (no solo lo desactiva). Su historial de tareas y movimientos queda, pero desvinculado. La cuenta de acceso (Auth) sigue existiendo del lado de Supabase — esto no la borra.</div>
                <div style={{fontSize:"10px",color:"#c89090",letterSpacing:"1px",marginBottom:"6px"}}>ESCRIBÍ "{eliminandoUsuarioEntidad.usuario.nombre}" PARA CONFIRMAR</div>
                <input value={eliminandoUsuarioEntidad.confirmText} onChange={e=>setEliminandoUsuarioEntidad({...eliminandoUsuarioEntidad,confirmText:e.target.value})} style={{width:"100%",background:"#0a0e1a",border:"1px solid #4a2020",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"14px"}}/>
                {status && status.startsWith("error") && <div style={{fontSize:"11px",color:"#e07070",marginBottom:"12px"}}>{status}</div>}
                <div style={{display:"flex",gap:"10px"}}>
                  <button disabled={borrandoUsuarioEntidad || eliminandoUsuarioEntidad.confirmText!==eliminandoUsuarioEntidad.usuario.nombre} onClick={confirmarEliminarUsuarioEntidad} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"#7a2020",color:"#fff",border:"1px solid #aa3030"}}>{borrandoUsuarioEntidad?"Eliminando...":"🗑 Eliminar definitivamente"}</button>
                  <button onClick={()=>setEliminandoUsuarioEntidad(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {agregandoMGSala && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"#12162a",border:"1px solid #3a3a7a",borderRadius:"16px",padding:"20px",width:"100%",maxWidth:"360px"}}>
                <div style={{fontSize:"11px",color:"#7a8af0",letterSpacing:"1px",marginBottom:"4px"}}>NUEVO MASTER GROWER</div>
                <div style={{fontSize:"11px",color:"#5a6a9a",marginBottom:"12px"}}>para la sala {agregandoMGSala.salaNombre}</div>
                <input value={agregandoMGSala.nombre} onChange={e=>setAgregandoMGSala({...agregandoMGSala,nombre:e.target.value})} placeholder="Nombre del MG" style={{width:"100%",background:"#0a0e1a",border:"1px solid #2a2a5a",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"8px"}}/>
                <input value={agregandoMGSala.email} onChange={e=>setAgregandoMGSala({...agregandoMGSala,email:e.target.value})} placeholder="Email del MG" style={{width:"100%",background:"#0a0e1a",border:"1px solid #2a2a5a",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"14px"}}/>
                <div style={{display:"flex",gap:"8px"}}>
                  <button disabled={guardandoMGSala || !agregandoMGSala.nombre || !agregandoMGSala.email} onClick={crearMGparaSala} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"#4a4ad0",color:"#fff",border:"none"}}>{guardandoMGSala?"Guardando...":"✓ Crear MG"}</button>
                  <button onClick={()=>setAgregandoMGSala(null)} style={{padding:"10px 14px",borderRadius:"8px",fontSize:"12px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {editandoUsuarioEntidad && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"#12162a",border:"1px solid #3a3a7a",borderRadius:"16px",padding:"20px",width:"100%",maxWidth:"360px"}}>
                <div style={{fontSize:"11px",color:"#7a8af0",letterSpacing:"1px",marginBottom:"12px"}}>EDITAR USUARIO</div>
                <div style={{fontSize:"9px",color:"#5a6a9a",marginBottom:"4px"}}>NOMBRE</div>
                <input value={editandoUsuarioEntidad.nombre} onChange={e=>setEditandoUsuarioEntidad({...editandoUsuarioEntidad,nombre:e.target.value})} placeholder="Nombre" style={{width:"100%",background:"#0a0e1a",border:"1px solid #2a2a5a",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"10px"}}/>
                <div style={{fontSize:"9px",color:"#5a6a9a",marginBottom:"4px"}}>EMAIL</div>
                <input value={editandoUsuarioEntidad.email} onChange={e=>setEditandoUsuarioEntidad({...editandoUsuarioEntidad,email:e.target.value})} placeholder="Email" style={{width:"100%",background:"#0a0e1a",border:"1px solid #2a2a5a",borderRadius:"8px",color:"#dfe0f0",fontSize:"13px",padding:"9px 12px",marginBottom:"6px"}}/>
                <div style={{fontSize:"10px",color:"#6a8a6a",background:"#0e1a10",borderRadius:"6px",padding:"7px 10px",marginBottom:"14px",lineHeight:"1.4"}}>Si el usuario ya tiene cuenta, cambiar el email acá <b>también cambia su correo de acceso</b> (login). Queda confirmado al instante, sin mail de verificación. Los que todavía no aceptaron la invitación no tienen cuenta, así que solo se actualiza su ficha.</div>
                {status && status.startsWith("error") && <div style={{fontSize:"11px",color:"#e07070",background:"#2a1010",border:"1px solid #5a2020",borderRadius:"6px",padding:"8px 10px",marginBottom:"12px",lineHeight:"1.4",wordBreak:"break-word"}}>{status}</div>}
                <div style={{display:"flex",gap:"8px"}}>
                  <button disabled={guardandoUsuarioEntidad || !editandoUsuarioEntidad.nombre} onClick={guardarUsuarioEntidad} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"#0e8a6a",color:"#fff",border:"none"}}>{guardandoUsuarioEntidad?"Guardando...":"✓ Guardar"}</button>
                  <button onClick={()=>setEditandoUsuarioEntidad(null)} style={{padding:"10px 14px",borderRadius:"8px",fontSize:"12px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
  ...
  if(usuario.rol==="administrador" && !usuario.entidad_id){
    if(!adminLoaded) return <div style={{background:"#0e0e16",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Mono',monospace",color:"#4a4a7a",fontSize:"14px"}}>Cargando panel...</div>;
    return (
      <div style={{background:"#0e0e16",minHeight:"100vh",fontFamily:"'IBM Plex Mono',monospace",color:"#dfe0f0"}}>
        <div style={{background:"#14142a",borderBottom:"2px solid #2a2a5a",padding:"18px 20px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"#7a8af0"}}>🛡 Panel Administrador</div>
            <div style={{fontSize:"11px",color:"#5a5a9a",letterSpacing:"2px",marginTop:"4px"}}>{usuario.nombre}</div>
          </div>
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"6px"}}>
            {status && <div style={{fontSize:"11px",color:status==="guardado"||status==="sala eliminada"?"#7ec850":"#e74c3c",background:"#1a1a3a",padding:"5px 10px",borderRadius:"10px"}}>{status}</div>}
            <button onClick={handleLogout} style={{fontSize:"11px",color:"#7a7aba",background:"transparent",border:"1px solid #2a2a5a",borderRadius:"14px",padding:"4px 12px"}}>salir</button>
          </div>
        </div>
        ...
        <div style={{padding:"20px"}}>
          {editingMGAdmin ? (
            <div style={card({background:"#161630",borderColor:"#3a3a7a"})}>
              <span style={lbl("#7a8af0")}>{editingMGAdmin.salaIdExistente?"ASIGNAR MASTER GROWER A SALA SIN DUEÑO":"NUEVO MASTER GROWER + SALA NUEVA"}</span>
              <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                <div>
                  <div style={{...lbl("#8a8ad0"),marginBottom:"6px"}}>NOMBRE</div>
                  <input value={editingMGAdmin.nombre} onChange={e=>setEditingMGAdmin({...editingMGAdmin,nombre:e.target.value})} style={inp()}/>
                </div>
                <div>
                  <div style={{...lbl("#8a8ad0"),marginBottom:"6px"}}>EMAIL</div>
                ...
                {!editingMGAdmin.salaIdExistente && (
                  <div>
                    <div style={{...lbl("#8a8ad0"),marginBottom:"6px"}}>NOMBRE DE LA SALA NUEVA</div>
                ...
                <div style={{display:"flex",gap:"10px"}}>
                  <button disabled={savingMGAdmin || !editingMGAdmin.nombre || !editingMGAdmin.email} onClick={()=>crearMasterGrowerAdmin(editingMGAdmin.salaIdExistente)} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"13px",fontWeight:"600",background:"#4a4ad0",color:"#fff",border:"none"}}>{savingMGAdmin?"Guardando...":"✓ Guardar"}</button>
                  <button onClick={()=>setEditingMGAdmin(null)} style={{padding:"10px 16px",borderRadius:"8px",fontSize:"13px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
                </div>
              </div>
            </div>
          ) : (
            <button onClick={()=>setEditingMGAdmin({nombre:"",email:"",salaNombre:"",salaIdExistente:null})} style={{width:"100%",padding:"14px",borderRadius:"10px",fontSize:"13px",fontWeight:"600",background:"#14142a",color:"#7a8af0",border:"2px dashed #3a3a7a",marginBottom:"20px"}}>+ Nuevo Master Grower (Sala nueva)</button>
          )}

          <div style={{fontSize:"13px",color:"#7a7aba",marginBottom:"14px"}}>{salasTodas.length} sala{salasTodas.length!==1?"s":""}</div>

          <div style={card({background:"#0e1420",borderColor:"#2a3a5a"})}>
            <span style={lbl("#7a9af0")}>🏢 ENTIDADES ({todasEntidades.length})</span>
            <div style={{display:"flex",flexDirection:"column",gap:"8px",marginTop:"6px"}}>
              {todasEntidades.map(ent=>{
                ...
                return (
                  <div key={ent.id} style={{background:"#0e0e22",borderRadius:"10px",padding:"10px 12px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px"}}>
                      <span style={{fontSize:"13px",color:"#dfe0f8",fontWeight:"600"}}>{ent.nombre}</span>
                      {puedeBorrar ? (
                        <button onClick={()=>setEliminandoEntidad({entidad:ent, confirmText:""})} style={{padding:"5px 10px",borderRadius:"7px",fontSize:"10px",background:"#2a1010",color:"#aa5050",border:"1px solid #4a2020"}}>🗑 Eliminar</button>
                      ) : (
                        <span style={{fontSize:"9px",color:"#5a6a9a"}}>no se puede borrar con salas activas</span>
                      )}
                    </div>
                    <div style={{fontSize:"10px",color:"#7a7aba",marginBottom:"8px"}}>{salasDeEsta} sala{salasDeEsta!==1?"s":""} · {usuariosDeEsta} usuario{usuariosDeEsta!==1?"s":""}</div>
                    <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                      <span style={{fontSize:"9px",color:"#7a7aba"}}>EXPIRA:</span>
                      <input type="date" value={ent.fecha_expiracion? ent.fecha_expiracion.slice(0,10):""} onChange={e=>actualizarExpiracionEntidad(ent.id, e.target.value)} style={{background:"#12162a",border:"1px solid #2a2a5a",borderRadius:"6px",color:"#dfe0f0",fontSize:"10px",padding:"4px 8px",colorScheme:"dark"}}/>
                      {ent.fecha_expiracion && <button onClick={()=>actualizarExpiracionEntidad(ent.id, null)} style={{fontSize:"9px",color:"#7a7aba",background:"transparent",border:"none"}}>quitar</button>}
                    </div>
                  </div>
                );
              })}
          ...
          {salasTodas.map(sala=>{
            ...
            return (
              <div key={sala.id} style={card({background:"#12122a",borderColor:"#2a2a5a"})}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"10px"}}>
                  <div>
                    <div style={{fontSize:"16px",fontWeight:"600",color:"#e8e8f8"}}>{sala.nombre}</div>
                    <div style={{fontSize:"12px",color:mg?"#7a9af0":"#c0392b",marginTop:"3px"}}>{mg?"Master Grower: "+mg.nombre+(mg.activo===false?" (desactivado)":""):"⚠ Sin Master Grower asignado"}</div>
                  </div>
                  <button onClick={()=>setEliminandoSala({sala, confirmText:""})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#2a1010",color:"#aa5050",border:"1px solid #4a2020"}}>🗑 Eliminar sala</button>
                </div>

                {mg && (
                  <button onClick={()=>eliminarMasterGrowerAdmin(mg)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#2a2010",color:"#c8a020",border:"1px solid #4a3a20",marginBottom:"10px"}}>⏸ Quitar como Master Grower</button>
                )}
                {!mg && (
                  <button onClick={()=>setEditingMGAdmin({nombre:"",email:"",salaNombre:"",salaIdExistente:sala.id})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#102a18",color:"#7ec850",border:"1px solid #205a2a",marginBottom:"10px"}}>+ Asignar Master Grower</button>
                )}

                <div style={{fontSize:"11px",color:"#5a5a9a",marginBottom:"8px"}}>{miembros.length} usuario{miembros.length!==1?"s":""} en esta sala</div>
                <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
                  {miembros.map(u=>(
                    <div key={u.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#0e0e22",borderRadius:"8px",padding:"8px 12px",opacity:u.activo===false?0.5:1}}>
                      <span style={{fontSize:"12px",color:"#c8c8e8"}}>{u.nombre} <span style={{color:"#5a5a9a"}}>· {u.rol==="ejecutor"?"Ejecutor":"Master Grower"}{u.activo===false?" · desactivado":""}</span></span>
                      {u.activo===false ? (
                        <button onClick={()=>reactivarUsuarioAdmin(u)} style={{padding:"4px 9px",borderRadius:"6px",fontSize:"10px",background:"#102a18",color:"#7ec850",border:"1px solid #205a2a"}}>↺ Reactivar</button>
                      ) : (
                        <button onClick={()=>u.rol==="master_grower"?eliminarMasterGrowerAdmin(u):eliminarEjecutorAdmin(u)} style={{padding:"4px 9px",borderRadius:"6px",fontSize:"10px",background:"#2a1010",color:"#aa5050",border:"1px solid #4a2020"}}>Desactivar</button>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {eliminandoEntidad && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"#1e0e0e",border:"1px solid #6a2020",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"18px",color:"#e07070",marginBottom:"10px"}}>⚠ Eliminar "{eliminandoEntidad.entidad.nombre}"</div>
              <div style={{fontSize:"12px",color:"#c0a0a0",marginBottom:"14px",lineHeight:"1.6"}}>Esto borra DEFINITIVAMENTE la entidad y su usuario administrador. No se puede deshacer.</div>
              <div style={{...lbl("#c89090"),marginBottom:"6px"}}>ESCRIBÍ "{eliminandoEntidad.entidad.nombre}" PARA CONFIRMAR</div>
              ...
              <div style={{display:"flex",gap:"10px"}}>
                <button disabled={borrandoEntidad || eliminandoEntidad.confirmText!==eliminandoEntidad.entidad.nombre} onClick={confirmarEliminarEntidad} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"#7a2020",color:"#fff",border:"1px solid #aa3030"}}>{borrandoEntidad?"Eliminando...":"🗑 Eliminar definitivamente"}</button>
                <button onClick={()=>setEliminandoEntidad(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
              </div>
            </div>
          </div>
        )}
        ...
        {eliminandoSala && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"#1e0e0e",border:"1px solid #6a2020",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"18px",color:"#e07070",marginBottom:"10px"}}>⚠ Eliminar "{eliminandoSala.sala.nombre}"</div>
              <div style={{fontSize:"12px",color:"#c0a0a0",marginBottom:"14px",lineHeight:"1.6"}}>Esto borra DEFINITIVAMENTE la sala y todo su contenido: sectores, lotes, genéticas, tareas, comentarios, relevamientos y los usuarios que pertenecen a ella. No se puede deshacer.</div>
              <div style={{...lbl("#c89090"),marginBottom:"6px"}}>ESCRIBÍ "{eliminandoSala.sala.nombre}" PARA CONFIRMAR</div>
              ...
              <div style={{display:"flex",gap:"10px"}}>
                <button disabled={borrandoSala || eliminandoSala.confirmText!==eliminandoSala.sala.nombre} onClick={confirmarEliminarSala} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"#7a2020",color:"#fff",border:"1px solid #aa3030"}}>{borrandoSala?"Eliminando...":"🗑 Eliminar definitivamente"}</button>
                <button onClick={()=>setEliminandoSala(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
              </div>
            </div>
          </div>
        )}
  ...
  const tabsExtra = [
    ...
    {key:"_calendario",    label:"📅 Calendario",    col:"#3a8ad0"},
    {key:"_asistente",     label:"🤖 Asistente",     col:"#9b59b6"},
  ...
  if(esEjecutor) tabsExtra.splice(3, 0, {key:"_geneticas", label:"🧬 Genéticas", col:"#2980b9"});
  if(!esEjecutor) tabsExtra.push({key:"_cosechas", label:"📦 Cosechas", col:"#c8a020"});
  if(!esEjecutor) tabsExtra.push({key:"_reportes", label:"📊 Reportes", col:"#5a9ad0"});
  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
      ...
      <style>{"*{box-sizing:border-box;margin:0;padding:0;}button{cursor:pointer;transition:all .15s ease;font-family:'IBM Plex Mono',monospace;}textarea,input,select{outline:none;font-family:'IBM Plex Mono',monospace;}textarea:focus,input:focus,select:focus{border-color:#4aaa4a!important;}.fade{animation:fi .22s ease;}@keyframes fi{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:none}}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#2a4a2a;border-radius:2px}"}</style>

      <div style={{background:"#111e11",borderBottom:"2px solid #2a4a2a",padding:"18px 20px 16px",position:"sticky",top:0,zIndex:100}}>
        {usuario.rol==="administrador" && adminEntidadModo==="mg" && (
          <button onClick={volverModoEntidad} style={{width:"100%",marginBottom:"12px",padding:"8px",borderRadius:"9px",fontSize:"12px",background:"#0a0e1a",color:"#7a8af0",border:"1px solid #2a2a5a",textAlign:"left"}}>← Volver al panel de {entidadNombre}</button>
        )}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"#7ec850"}}>🌱 RAÍZ — Cuaderno de Cultivo</div>
            <div style={{fontSize:"11px",color:"#4a8a4a",letterSpacing:"2px",marginTop:"4px"}}>
      ...
      <div style={{padding:"20px 20px 110px"}}>
          ...
          return (
            <div className="fade">
              ...
              {subs.map(sub=>{
                ...
                return (
                  <div key={sub.id} style={card(en1212 ? {borderColor:"#e8c43a99", background:"#241f10"} : {borderColor:colorDeFase(sp.orden)+"55"})}>
                    <div onClick={()=>toggleSub(sub.id)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",alignItems:"center",gap:"8px",flexWrap:"wrap"}}>
                          ...
                          {sub.capacidad_unidades && <span style={{fontSize:"11px",color:"#5a8a5a"}}>cap. {sub.capacidad_unidades}</span>}
                          {en1212 && <span style={{fontSize:"10px",color:"#e8c43a",background:"#3a3010",padding:"2px 8px",borderRadius:"9px",fontWeight:"600"}}>☀ 12/12</span>}
                          {diasParaCorte!==null && (
                            <span style={{fontSize:"10px",color:diasParaCorte<0?"#e05050":"#c8a020",background:diasParaCorte<0?"#3a1010":"#3a2f10",padding:"2px 8px",borderRadius:"9px",fontWeight:"600"}}>
                        ...
                        {lotesMaduracion.length>0 && <div style={{fontSize:"11px",color:"#d0a840",marginTop:"6px"}}>🫙 {lotesMaduracion.length} en maduración</div>}
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:"10px",flexShrink:0,marginLeft:"10px"}}>
                        ...
                        <span style={{fontSize:"13px",color:"#5a8a5a",display:"inline-block",transform:abierta?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                      </div>
                    </div>
                    ...
                    {sp.nombre==="Floración" && !esEjecutor && (
                      <div style={{background:"#1a1024",borderRadius:"9px",padding:"10px 12px",marginBottom:"14px",border:"1px solid #3a2a4a"}}>
                        <div style={{fontSize:"10px",color:"#a77dc2",letterSpacing:"0.5px",marginBottom:"6px"}}>🌸 12/12 ACTIVADO EN ESTA CARPA DESDE</div>
                        <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                          ...
                          <button onClick={()=>guardarFechaFloracion(sub)} disabled={savingFloracion} style={{padding:"6px 12px",borderRadius:"6px",fontSize:"11px",background:"#3a2a4a",color:"#c8a0e0",border:"1px solid #5a3a7a"}}>✓</button>
                        </div>
                        {!sub.fecha_inicio_floracion && <div style={{fontSize:"10px",color:"#7a5a9a",marginTop:"6px"}}>Sin activar — cargá el día que esta carpa pasa a 12/12. Aplica a todos los lotes que tenga adentro.</div>}
                        {sub.fecha_inicio_floracion && <div style={{fontSize:"10px",color:"#7a5a9a",marginTop:"6px"}}>Se congela en cada lote al cortar, y esta fecha se resetea sola cuando la carpa quede vacía para el próximo ciclo.</div>}
                      </div>
                    )}
                    {lotesSub.length>0 && (
                        ...
                        {lotesSub.map(l=>{
                          ...
                          return (
                            <div key={l.id} style={{background:"#111e11",borderRadius:"8px",padding:"10px 12px",border:`1px solid ${g?.color||"#2a4a2a"}44`}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px"}}>
                                <span style={{fontSize:"13px",color:g?.color||"#9aaa9a"}}>{g?.nombre||"Sin genética"}{l.numero_lote?` · ${loteLabel(l.numero_lote, sufijoDeLote(l))}`:""}</span>
                              ...
                              {sp.nombre!=="Floración" && (
                                <div style={{display:"flex",alignItems:"baseline",gap:"7px",marginBottom:"6px",flexWrap:"wrap"}}>
                                  <span style={{fontSize:"12px",color:"#7ab070",fontWeight:"600"}}>{Math.max(0,Math.round((new Date()-new Date(l.fecha_ultima_movida||l.fecha_inicio))/86400000))}d en {sp.nombre.toLowerCase()}</span>
                                  <span style={{fontSize:"9px",color:"#3d5a3d"}}>desde {l.fecha_ultima_movida||l.fecha_inicio}</span>
                                </div>
                              )}
                              {sp.nombre==="Floración" && (()=>{
                                ...
                                return (
                                  <div style={{display:"flex",alignItems:"baseline",gap:"9px",marginBottom:"6px",flexWrap:"wrap"}}>
                                    {corteTxt && <span style={{fontSize:"12px",color:corteAtrasado?"#e05050":"#c8a020",fontWeight:"600"}}>{corteTxt}</span>}
                                    <span style={{fontSize:"11px",color:"#7ab070"}}>🌱 {st?st.dias_vegetativo:"—"}d veg</span>
                                    {sub.fecha_inicio_floracion && <span style={{fontSize:"11px",color:"#a77dc2"}}>🌸 {st?st.dias_floracion:"—"}d flo</span>}
                                  </div>
                                );
                              })()}
                              {sp.nombre==="Floración" && sub.fecha_inicio_floracion && (
                                <div style={{fontSize:"11px",color:"#a77dc2",marginBottom:"6px"}}>🌸 En 12/12 desde {sub.fecha_inicio_floracion}</div>
                              )}
                              {sp.nombre==="Floración" && !sub.fecha_inicio_floracion && !esEjecutor && (
                                <div style={{fontSize:"11px",color:"#7a5a9a",marginBottom:"6px"}}>🌸 Carpa todavía sin 12/12 activado — cargalo arriba, en el encabezado de {sub.nombre}.</div>
                              )}
                              <div style={{display:"flex",gap:"6px"}}>
                                ...
                                {!esEjecutor && sigPadre && (
                                  (sp.nombre==="Floración" && !sub.fecha_inicio_floracion) ? (
                                    <button disabled title="Activá el 12/12 de esta carpa antes de poder cortar" style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"#1a1a1a",color:"#5a5a5a",border:"1px solid #2a2a2a",cursor:"not-allowed"}}>→ Avanzar a {sigPadre.nombre} (activá 12/12 primero)</button>
                                  ) : (sp.nombre==="Vegetativo" && sub.puede_avanzar_etapa===false) ? (
                                    <button disabled title="Este tamaño de maceta no está habilitado para pasar a Floración (lo configurás en Mi Sala)" style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"#1a1a1a",color:"#5a5a5a",border:"1px solid #2a2a2a",cursor:"not-allowed"}}>→ Avanzar a {sigPadre.nombre} (maceta no habilitada)</button>
                                  ) : (
                                ...
                                {!esEjecutor && !(sp.nombre==="Floración" && sub.fecha_inicio_floracion) && (
                                  <button onClick={()=>{setMovingLote(l); setDestinoSub(""); setMovingMovida(String(l.cantidad_plantas)); setModoMover(true); setModoRetroceso(false);}} title="Mover a otro subsector (corrección)" style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"#1a2030",color:"#7a9ad0",border:"1px solid #2a3a5a"}}>↔</button>
                                )}
                                {!esEjecutor && sp.nombre==="Floración" && !sub.fecha_inicio_floracion && (
                                  <button onClick={()=>iniciarRetroceso(l)} title="Todavía no entró en 12/12 — volver a Vegetativo (5L)" style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"#1a2418",color:"#7aa080",border:"1px solid #2a4a30"}}>↩ 5L</button>
                                )}
                              </div>
                              {sp.nombre==="Floración" && sub.fecha_inicio_floracion && (
                                <div style={{fontSize:"10px",color:"#7a5a9a",marginTop:"6px"}}>🔒 En 12/12 — no se puede mover hasta el corte (avanzar a Secado).</div>
                              )}
                            </div>
                          );
                        })}
                    ...
                    {!esEjecutor && (
                      <div style={{display:"flex",gap:"8px"}}>
                        {(sp.nombre==="Enraizado" || sp.nombre==="Vegetativo") && (
                          <button onClick={()=>setEditingLote(emptyLote(sub.id))} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"#162816",color:"#7ec850",border:"1px dashed #2a5a2a"}}>+ Planta/lote</button>
                        )}
                        <button onClick={()=>setEditingTarea(emptyTarea(sub.id))} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"#0a2420",color:"#16a085",border:"1px dashed #0e6a5a"}}>+ Tarea</button>
                      </div>
                    )}

                    {tareasDelSub.length>0 && (
                      <div style={{marginTop:"14px"}}>
                        <div onClick={()=>setTareasEnSubAbierto(prev=>({...prev,[sub.id]:!prev[sub.id]}))} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 12px",background:tareasEnSubAbierto[sub.id]?"#0e241e":"#0a1814",borderRadius:"9px",border:"1px solid #16a08533"}}>
                          <span style={{fontSize:"12px",color:"#16a085"}}>📋 Tareas planificadas aquí <span style={{color:"#5a9a8a"}}>· {tareasDelSub.length}</span></span>
                          <span style={{fontSize:"11px",color:"#16a085",transform:tareasEnSubAbierto[sub.id]?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                        </div>
                        {tareasEnSubAbierto[sub.id] && (
                          <div style={{display:"flex",flexDirection:"column",gap:"6px",marginTop:"8px"}}>
                            {tareasDelSub.map(t=>{
                              ...
                              return (
                                <div key={t.id} style={{background:"#0e1a18",borderRadius:"8px",padding:"9px 11px",borderLeft:`3px solid ${TAREA_PRIORIDAD_COLOR[t.prioridad]||"#5a7a9a"}`}}>
                                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                                    <span style={{fontSize:"12px",color:"#dfe8e0"}}>{t.titulo}</span>
                                  ...
                                  <div style={{fontSize:"10px",color:"#5a8a7a",marginTop:"3px"}}>📅 {t.fecha_programada}{t.asignado_a?` · → ${usuariosMap[t.asignado_a]||"—"}`:""}</div>
                                </div>
                              );
                            })}
                          </div>
        ...
        {tab==="_tareas" && (()=>{
          ...
          return (
            <div className="fade">
              ...
              <div style={{background:"#0a1a14",borderRadius:"12px",padding:"12px 14px",marginBottom:"18px",border:"1px solid #1a3a2a"}}>
                <div style={{fontSize:"11px",color:"#5a9a8a",letterSpacing:"1px",marginBottom:"10px"}}>FILTRAR POR FECHA</div>
                <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
                  {[["ninguno","Sin filtro"],["exacta","Fecha exacta"],["rango","Rango"]].map(([m,l])=>(
                    <button key={m} onClick={()=>setTModoFecha(m)} style={{flex:1,padding:"7px 6px",borderRadius:"9px",fontSize:"11px",background:tModoFecha===m?"#0e5a3a":"#0a1a14",color:tModoFecha===m?"#fff":"#5a8a7a",border:`1px solid ${tModoFecha===m?"#16a085":"#1a3a2a"}`}}>{l}</button>
                  ))}
                </div>
                {tModoFecha==="exacta" && (
                  <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                    <input type="date" value={tFechaExacta} onChange={e=>setTFechaExacta(e.target.value)} style={{...inp(),flex:1}}/>
                    {tFechaExacta && <button onClick={()=>setTFechaExacta("")} style={{padding:"7px 10px",borderRadius:"8px",fontSize:"11px",background:"#2a1010",color:"#aa5050",border:"1px solid #4a2020"}}>✕</button>}
                  </div>
                )}
                {tModoFecha==="rango" && (
                  <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                      <span style={{fontSize:"11px",color:"#5a9a8a",width:"36px"}}>Desde</span>
                      <input type="date" value={tFechaDesde} onChange={e=>setTFechaDesde(e.target.value)} style={{...inp(),flex:1}}/>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                      <span style={{fontSize:"11px",color:"#5a9a8a",width:"36px"}}>Hasta</span>
                      <input type="date" value={tFechaHasta} onChange={e=>setTFechaHasta(e.target.value)} style={{...inp(),flex:1}}/>
                    </div>
                    {(tFechaDesde||tFechaHasta) && <button onClick={()=>{setTFechaDesde("");setTFechaHasta("");}} style={{padding:"6px",borderRadius:"8px",fontSize:"11px",background:"#2a1010",color:"#aa5050",border:"1px solid #4a2020"}}>✕ Limpiar fechas</button>}
                  </div>
                )}
              </div>
                    ...
                    return (
                      <div key={grupo.key} style={{border:`1px solid ${grupo.color}44`,borderRadius:"14px",overflow:"hidden"}}>
                        <div onClick={()=>setGrupoTareasAbiertos(prev=>({...prev,[grupo.key]:!prev[grupo.key]}))} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:abierto?`${grupo.color}14`:"transparent"}}>
                          <span style={{fontSize:"13px",fontWeight:"600",color:grupo.color}}>{grupo.label} <span style={{fontSize:"11px",color:"#6a9a8a",fontWeight:"400"}}>· {grupo.tareas.length}</span></span>
        ...
        {tab==="_asistente" && (()=>{
          ...
          return (
            <div className="fade" style={{display:"flex",flexDirection:"column",height:"calc(100vh - 180px)"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"#9b59b6",marginBottom:"4px"}}>🤖 Asistente IA</div>
              <div style={{fontSize:"12px",color:"#7a6a9a",marginBottom:"14px"}}>Hacé preguntas o subí una foto para diagnóstico</div>

              <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:"10px",paddingBottom:"8px"}}>
                {chatMensajes.length===0 && (
                  <div style={{textAlign:"center",padding:"40px 20px",color:"#5a4a7a",fontSize:"13px",lineHeight:"1.7"}}>
                    <div style={{fontSize:"40px",marginBottom:"12px"}}>🌿</div>
                    <div>Podés preguntarme sobre deficiencias, plagas, nutrición, tiempos de floración...</div>
                    <div style={{marginTop:"10px",color:"#4a3a6a"}}>También podés adjuntar una foto para que la analice.</div>
                  </div>
                )}
                {chatMensajes.map((m,i)=>(
                  <div key={i} style={{display:"flex",flexDirection:"column",alignItems:m.rol==="usuario"?"flex-end":"flex-start"}}>
                    {m.imagen && (
                      <img src={`data:image/jpeg;base64,${m.imagen}`} alt="foto" style={{maxWidth:"70%",borderRadius:"12px",marginBottom:"4px",border:"1px solid #3a2a4a"}}/>
                    )}
                    {m.texto && (
                      <div style={{
                        maxWidth:"85%",padding:"12px 14px",borderRadius:"14px",fontSize:"13px",lineHeight:"1.6",whiteSpace:"pre-wrap",
                        background:m.rol==="usuario"?"#2a1a4a":m.rol==="error"?"#3a1010":"#0e1a24",
                        color:m.rol==="usuario"?"#d8c8f0":m.rol==="error"?"#e08080":"#cce0d0",
                        borderBottomRightRadius:m.rol==="usuario"?"4px":"14px",
                        borderBottomLeftRadius:m.rol==="usuario"?"14px":"4px",
                        border:m.rol==="asistente"?"1px solid #1a3a2a":"none"
                      }}>{m.texto}</div>
                    )}
                    <div style={{fontSize:"10px",color:"#5a4a7a",marginTop:"3px"}}>{m.ts}</div>
                  </div>
                ))}
                {chatCargando && (
                  <div style={{display:"flex",alignItems:"flex-start"}}>
                    <div style={{background:"#0e1a24",border:"1px solid #1a3a2a",borderRadius:"14px",borderBottomLeftRadius:"4px",padding:"12px 16px",color:"#5a9a7a",fontSize:"13px"}}>Analizando...</div>
                  </div>
                )}
                <div ref={chatBottomRef}/>
              </div>

              <div style={{borderTop:"1px solid #2a1a4a",paddingTop:"12px",marginTop:"8px"}}>
                {chatImagen && (
                  <div style={{position:"relative",display:"inline-block",marginBottom:"8px"}}>
                    <img src={`data:image/jpeg;base64,${chatImagen}`} alt="preview" style={{height:"70px",borderRadius:"8px",border:"1px solid #4a3a6a"}}/>
                    <button onClick={()=>setChatImagen(null)} style={{position:"absolute",top:"-6px",right:"-6px",width:"20px",height:"20px",borderRadius:"50%",background:"#c0392b",color:"#fff",border:"none",fontSize:"11px",lineHeight:"20px",textAlign:"center",padding:0}}>✕</button>
                  </div>
                )}
                <div style={{display:"flex",gap:"8px",alignItems:"flex-end"}}>
                  <input type="file" accept="image/*" ref={fileInputRef} onChange={seleccionarImagen} style={{display:"none"}}/>
                  <button onClick={()=>fileInputRef.current?.click()} style={{padding:"10px",borderRadius:"10px",background:"#1a0a2a",color:"#9b59b6",border:"1px solid #3a2a5a",fontSize:"18px",flexShrink:0}}>📷</button>
                  ...
                  <button disabled={chatCargando||(!chatInput.trim()&&!chatImagen)} onClick={enviarMensaje} style={{padding:"10px 14px",borderRadius:"10px",background:chatCargando?"#1a0a2a":"#6a3a9a",color:"#fff",border:"none",fontSize:"16px",flexShrink:0}}>➤</button>
                </div>
                <div style={{fontSize:"10px",color:"#4a3a6a",marginTop:"6px",textAlign:"center"}}>Enter para enviar · Shift+Enter para nueva línea</div>
              </div>
            </div>
          );
        })()}
        ...
        {tab==="_calendario" && (()=>{
          ...
          return (
            <div className="fade">
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"#3a8ad0",marginBottom:"6px"}}>📅 Calendario</div>
              <div style={{fontSize:"13px",color:"#5a7a9a",marginBottom:"18px"}}>{esEjecutor?"TUS TAREAS POR FECHA":"TODAS LAS TAREAS POR FECHA"}</div>

              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"14px"}}>
                <button onClick={()=>setCalMes(m=>{ const d=new Date(m.year,m.month-1,1); return {year:d.getFullYear(),month:d.getMonth()}; })} style={{padding:"8px 14px",borderRadius:"8px",background:"#0e1e2e",color:"#5a9ad0",border:"1px solid #1a3a5a"}}>‹</button>
                <div style={{fontSize:"15px",color:"#cce0f0"}}>{nombreMes.charAt(0).toUpperCase()+nombreMes.slice(1)}</div>
                <button onClick={()=>setCalMes(m=>{ const d=new Date(m.year,m.month+1,1); return {year:d.getFullYear(),month:d.getMonth()}; })} style={{padding:"8px 14px",borderRadius:"8px",background:"#0e1e2e",color:"#5a9ad0",border:"1px solid #1a3a5a"}}>›</button>
              </div>

              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"4px",marginBottom:"6px"}}>
                {["L","M","X","J","V","S","D"].map(d=><div key={d} style={{textAlign:"center",fontSize:"11px",color:"#4a6a8a"}}>{d}</div>)}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"4px",marginBottom:"18px"}}>
                {celdas.map((fecha,idx)=>{
                  ...
                  const dotColor = estado==="atrasada"?"#c0392b":estado==="completada"?"#3a8a5a":estado==="pendiente"?"#c8a020":null;
                  return (
                    <button key={fecha} onClick={()=>setCalDia(fecha)} style={{
                      ...
                      background:esSeleccionado?"#1a3a5a":esHoy?"#0e2a3a":"#0a1620",
                      color:esHoy?"#fff":"#9ab",
                      border:esSeleccionado?"1px solid #3a8ad0":"1px solid #1a2a3a"
                    }}>
              ...
              <div style={{fontSize:"13px",color:"#8ab0d0",marginBottom:"10px"}}>{calDia}{calDia===hoy?" · hoy":""}</div>
              {tareasDelDia.length===0 ? (
                <div style={{textAlign:"center",padding:"30px 20px",color:"#3a5a7a",fontSize:"13px"}}>Sin tareas este día</div>
              ) : (
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                    ...
                    return (
                      <div key={grupo.key} style={{border:`1px solid ${grupo.color}44`,borderRadius:"14px",overflow:"hidden"}}>
                        <div onClick={()=>setGrupoTareasCalAbiertos(prev=>({...prev,[grupo.key]:!prev[grupo.key]}))} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",background:abierto?`${grupo.color}14`:"transparent"}}>
                          <span style={{fontSize:"13px",fontWeight:"600",color:grupo.color}}>{grupo.label} <span style={{fontSize:"11px",color:"#6a8aa0",fontWeight:"400"}}>· {grupo.tareas.length}</span></span>
                          <span style={{fontSize:"13px",color:grupo.color,transform:abierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                        </div>
                        {abierto && (
                          <div style={{padding:"4px 14px 14px",display:"flex",flexDirection:"column",gap:"10px"}}>
                ...
                return (
                  <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                    {dias.map(dia=>{
                      ...
                      return (
                        <div key={dia} style={{border:"1px solid #1abc9c33",borderRadius:"14px",overflow:"hidden"}}>
                          <div onClick={()=>setDiaRelevamientoAbierto(abierto?"":dia)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:abierto?"#0e2422":"transparent"}}>
                            <div>
                              <div style={{fontSize:"14px",color:"#dff0e8",fontWeight:"600"}}>{dia}</div>
                              <div style={{fontSize:"11px",color:"#5a9a90",marginTop:"3px"}}>{items.length} registro{items.length!==1?"s":""}{conDiferencia>0 && <span style={{color:"#c0392b"}}> · {conDiferencia} con diferencia</span>}</div>
                            </div>
                            <span style={{fontSize:"13px",color:"#1abc9c",transform:abierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                          </div>
                          {abierto && (
                            <div style={{padding:"4px 14px 14px",display:"flex",flexDirection:"column",gap:"10px"}}>
                              {items.map(r=>{
                                ...
                                return (
                                  <div key={r.id} style={{background:"#0e1a18",borderRadius:"10px",padding:"12px 14px",borderLeft:`4px solid ${diff===0?"#3a8a5a":"#c0392b"}`}}>
                                    <div style={{fontSize:"13px",color:"#dff0e8",marginBottom:"6px"}}>{sp?.nombre||"—"}{sub?` › ${sub.nombre}`:""}{g?` · ${g.nombre}`:""}</div>
                                    <div style={{display:"flex",gap:"16px",marginBottom:"6px"}}>
                                      <div><span style={{fontSize:"11px",color:"#5a9a90"}}>Contado: </span><span style={{fontSize:"14px",fontWeight:"700",color:"#1abc9c"}}>{r.cantidad_contada}</span></div>
                                      <div><span style={{fontSize:"11px",color:"#5a9a90"}}>Sistema: </span><span style={{fontSize:"14px",color:"#9ab"}}>{sistema}</span></div>
                                      {diff!==0 && <div><span style={{fontSize:"11px",color:"#c0392b"}}>Diferencia: </span><span style={{fontSize:"14px",fontWeight:"700",color:"#c0392b"}}>{diff>0?"+":""}{diff}</span></div>}
                                    </div>
                                    {r.notas && <div style={{fontSize:"12px",color:"#a0c0b8",marginBottom:"6px"}}>{r.notas}</div>}
                                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                                      <div style={{fontSize:"10px",color:"#5a8a7a"}}>{usuariosMap[r.usuario_id]||"—"}</div>
                                      {!esEjecutor && (
              ...
              editingGenetica ? (
                <div style={card({borderColor:"#1a5a7a",background:"#0e1e28"})}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
                      <div>
                        <div style={{...lbl("#5a8aaa"),marginBottom:"6px"}}>DÍAS FLORACIÓN MÍN</div>
                      ...
                      <div>
                        <div style={{...lbl("#5a8aaa"),marginBottom:"6px"}}>DÍAS FLORACIÓN MÁX</div>
              ...
              {geneticas
                ...
                <div key={g.id} style={{background:"#162416",border:`1px solid ${g.color}55`,borderRadius:"14px",padding:"18px",borderLeft:`5px solid ${g.color}`,opacity:g.activa===false?0.55:1}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                    ...
                    <div style={{background:"#0e180e",border:`1px solid ${g.color}88`,borderRadius:"10px",padding:"8px 14px",textAlign:"center",minWidth:"72px"}}>
                      {g.dias_floracion_min ? (
                        <>
                          ...
                          <div style={{fontSize:"10px",color:"#4a6a4a",marginTop:"3px"}}>días de flora</div>
                        </>
                      ) : g.semanas_floracion ? (
                        <>
                          <div style={{fontSize:"18px",fontWeight:"700",color:g.color,lineHeight:1}}>{g.semanas_floracion}</div>
                          <div style={{fontSize:"10px",color:"#4a6a4a",marginTop:"3px"}}>semanas</div>
                        </>
                      ) : (
                        <div style={{fontSize:"11px",color:"#3a5a3a"}}>—</div>
                      )}
                    </div>
                  </div>
                  ...
                  {!esEjecutor && (
                    <div style={{display:"flex",gap:"6px",marginTop:"12px"}}>
                      <button onClick={()=>setEditingGenetica({...g, semanas_floracion:g.semanas_floracion||"", dias_floracion_min:g.dias_floracion_min||"", dias_floracion_max:g.dias_floracion_max||""})} style={{flex:1,padding:"8px",borderRadius:"8px",fontSize:"12px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>✏️ Editar</button>
                ...
                <>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                    {gruposCosecha.map(grupo=>{
                      const color = grupo.g?.color || "#8aaa20";
                      ...
                      return (
                        <div key={grupo.key} style={{border:`1px solid ${color}44`,borderRadius:"14px",overflow:"hidden"}}>
                          <div onClick={()=>setCosechaGrupoAbierto(prev=>({...prev,[grupo.key]:!prev[grupo.key]}))} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:abierto?`${color}14`:"transparent"}}>
                            <span style={{fontSize:"14px",fontWeight:"600",color}}>{grupo.nombre} <span style={{fontSize:"11px",color:"#7a9a6a",fontWeight:"400"}}>· {grupo.items.length} cosecha{grupo.items.length!==1?"s":""} · {grupo.totalG.toFixed(0)}g · {grupo.promedioG.toFixed(1)}g/planta</span></span>
                          ...
                          {abierto && (
                            <div style={{padding:"4px 14px 14px",display:"flex",flexDirection:"column",gap:"10px"}}>
                              {grupo.items.map(c=>{
                                ...
                                return (
                                  <div key={c.id} style={{background:"#141e10",border:`1px solid ${color}33`,borderRadius:"14px",padding:"16px",borderLeft:`4px solid ${color}`}}>
                                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                                      <div>
                                        <div style={{fontSize:"15px",fontWeight:"600",color:"#e0f0d0",marginBottom:"4px"}}>{c.numero_lote?loteLabel(c.numero_lote, sufijoDeLote(c)):"—"}</div>
        ...
        {tab==="_misala" && !esEjecutor && (
          <div className="fade">
            ...
            <div style={{marginBottom:"14px",border:"1px solid #0e6a5a",borderRadius:"12px",background:"#0a1e1a",overflow:"hidden"}}>
              <div onClick={()=>setDatosMaestrosAbierto(!datosMaestrosAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px"}}>
                <span style={{...lbl("#16a085"),margin:0}}>🗂 DATOS MAESTROS</span>
                <span style={{fontSize:"13px",color:"#16a085",transform:datosMaestrosAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
              </div>
              {datosMaestrosAbierto && (
              <div style={{padding:"0 10px 12px"}}>

                {/* ── Tipos de tarea (hijo) ── */}
                <div style={{border:"1px solid #0e5a4a",borderRadius:"10px",background:"#08150f",overflow:"hidden",marginBottom:"10px"}}>
                  <div onClick={()=>setTiposTareaAbierto(!tiposTareaAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px"}}>
                    <span style={{fontSize:"14px",fontWeight:"600",color:"#3ac0a0"}}>📋 Tipos de tarea <span style={{fontSize:"11px",color:"#5a9a8a",fontWeight:"400"}}>· {tiposTarea.length}</span></span>
                    <span style={{fontSize:"12px",color:"#3ac0a0",transform:tiposTareaAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                  </div>
                  {tiposTareaAbierto && (
                  ...
                  {editingTipoTarea ? (
                    <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                      ...
                      <div>
                        <div style={{...lbl("#5aa090"),marginBottom:"6px"}}>SECTORES HABILITADOS <span style={{fontWeight:"400",color:"#4a7a6a"}}>(ninguno = todos)</span></div>
                        <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
                          {sectoresPadre.map(sp=>{
                            const ids = editingTipoTarea.sectores_padre_ids || [];
                            const marcado = ids.includes(sp.id);
                            return (
                              <label key={sp.id} style={{display:"flex",alignItems:"center",gap:"10px",fontSize:"13px",color:marcado?"#cff0e4":"#7a9a8a",background:marcado?"#0e2a20":"#0a1a14",borderRadius:"8px",padding:"9px 12px",border:"1px solid "+(marcado?"#1a7a5a":"#1a3a2a"),cursor:"pointer"}}>
                      ...
                      <div style={{display:"flex",gap:"10px"}}>
                        <button disabled={savingTipoTarea || !editingTipoTarea.nombre} onClick={saveTipoTarea} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"13px",fontWeight:"600",background:"#0e8a6a",color:"#fff",border:"none"}}>{savingTipoTarea?"Guardando...":"✓ Guardar"}</button>
                        <button onClick={()=>setEditingTipoTarea(null)} style={{padding:"10px 16px",borderRadius:"8px",fontSize:"13px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
                      </div>
                    </div>
                  ) : (
                    <button onClick={()=>setEditingTipoTarea(emptyTipoTarea())} style={{width:"100%",padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"#0a2420",color:"#16a085",border:"1px dashed #0e6a5a",marginBottom:"12px"}}>+ Nuevo tipo de tarea</button>
                  )}
                  {tiposTarea.length>0 && (
                    <div style={{display:"flex",flexDirection:"column",gap:"8px",marginTop:"12px"}}>
                      {tiposTarea.map(tt=>{
                        ...
                        return (
                          <div key={tt.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#0a1814",borderRadius:"10px",padding:"10px 14px",opacity:tt.activo===false?0.5:1}}>
                            <div>
                              <span style={{fontSize:"14px",color:"#dff0e8"}}>{tt.icono} {tt.nombre}</span>
                              <div style={{fontSize:"11px",color:"#5a9a8a"}}>{nombres}</div>
                            </div>
                            <div style={{display:"flex",gap:"6px"}}>
                              <button onClick={()=>setEditingTipoTarea({...tt, sectores_padre_ids: (tt.sectores_padre_ids && tt.sectores_padre_ids.length) ? [...tt.sectores_padre_ids] : (tt.sector_padre_id ? [tt.sector_padre_id] : [])})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"11px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>✏️</button>
                              <button onClick={()=>toggleActivoTipoTarea(tt)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"11px",background:"#2a2a10",color:"#c8a020",border:"1px solid #4a4a20"}}>{tt.activo===false?"↺":"⏸"}</button>
                              <button onClick={()=>delTipoTarea(tt)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"11px",background:"#2a1010",color:"#aa5050",border:"1px solid #4a2020"}}>🗑</button>
                            </div>
                          </div>
                        );
                      })}
                ...
                {/* ── Genéticas (hijo) ── */}
                <div style={{border:"1px solid #1a5a7a",borderRadius:"10px",background:"#08131a",overflow:"hidden"}}>
                  <div onClick={()=>setGeneticasBoxAbierto(!geneticasBoxAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px"}}>
                    <span style={{fontSize:"14px",fontWeight:"600",color:"#3a9ad4"}}>🧬 Genéticas <span style={{fontSize:"11px",color:"#5a8aaa",fontWeight:"400"}}>· {geneticas.length}</span></span>
                    <span style={{fontSize:"12px",color:"#3a9ad4",transform:geneticasBoxAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                  </div>
                  {geneticasBoxAbierto && (
                  <div style={{padding:"0 12px 12px"}}>
                    <div style={{display:"flex",flexDirection:"column",gap:"6px",marginBottom:"10px"}}>
                      {geneticas.length===0 && <div style={{fontSize:"12px",color:"#4a6a7a",fontStyle:"italic"}}>Sin genéticas cargadas todavía.</div>}
                      {[...geneticas].sort((a,b)=>(a.nombre||"").localeCompare(b.nombre||"")).map(g=>(
                        <div key={g.id} style={{display:"flex",alignItems:"center",gap:"8px",fontSize:"13px",color:"#cfe4f0",background:"#0a1a24",borderRadius:"8px",padding:"8px 12px"}}>
                          <span style={{width:"10px",height:"10px",borderRadius:"50%",background:g.color||"#3a9ad4",flexShrink:0}}/>
                          {g.nombre}
                        </div>
                      ))}
                    </div>
                    <button onClick={()=>setTab("_geneticas")} style={{width:"100%",padding:"11px",borderRadius:"9px",fontSize:"13px",fontWeight:"600",background:"#0e2838",color:"#3a9ad4",border:"1px solid #1a5a7a"}}>🧬 Administrar genéticas</button>
                  </div>
                  )}
                </div>
            ...
            <div style={card({borderColor:"#6a3a8a",background:"#1a0e22"})}>
              <div onClick={()=>setUsuariosSalaAbierto(!usuariosSalaAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:usuariosSalaAbierto?"12px":"0"}}>
                <span style={{...lbl("#a86ad0"),margin:0}}>🛠 ADMINISTRACIÓN <span style={{color:"#7a5a9a",fontWeight:"400"}}>· Usuarios · {usuariosLista.length}</span></span>
                <span style={{fontSize:"13px",color:"#a86ad0",transform:usuariosSalaAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
              </div>
              ...
              {editingUsuario ? (
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                  <div>
                    <div style={{...lbl("#a888c8"),marginBottom:"6px"}}>NOMBRE</div>
                    <input value={editingUsuario.nombre} onChange={e=>setEditingUsuario({...editingUsuario,nombre:e.target.value})} style={inp()}/>
                  </div>
                  <div>
                    <div style={{...lbl("#a888c8"),marginBottom:"6px"}}>EMAIL</div>
                    <input type="email" value={editingUsuario.email} onChange={e=>setEditingUsuario({...editingUsuario,email:e.target.value})} style={inp()}/>
                  </div>
                  {!editingUsuario.id && (
                    <div style={{fontSize:"11px",color:"#7a5a9a"}}>Se crea como Ejecutor en tu Sala. Solo el Administrador puede crear Master Growers nuevos.</div>
                  )}
                  <div style={{display:"flex",gap:"10px"}}>
                    <button disabled={savingUsuario || !editingUsuario.nombre || !editingUsuario.email} onClick={saveUsuario} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"13px",fontWeight:"600",background:"#7a3aaa",color:"#fff",border:"1px solid #9a5aca"}}>{savingUsuario?"Guardando...":"✓ Guardar"}</button>
                    <button onClick={()=>setEditingUsuario(null)} style={{padding:"10px 16px",borderRadius:"8px",fontSize:"13px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
                  </div>
                </div>
              ) : (
                <button onClick={()=>setEditingUsuario(emptyUsuario())} style={{width:"100%",padding:"10px",borderRadius:"8px",fontSize:"13px",background:"#1a0e22",color:"#a86ad0",border:"2px dashed #6a3a8a",marginBottom:"4px"}}>+ Nuevo usuario</button>
              )}

              <div style={{display:"flex",flexDirection:"column",gap:"10px",marginTop:"12px"}}>
                {usuariosLista.map(u=>(
                  <div key={u.id} style={{background:"#140a1a",borderRadius:"10px",padding:"10px 14px",opacity:u.activo===false?0.5:1}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:u.rol==="ejecutor"?"8px":"0"}}>
                      <div>
                        ...
                        <span style={{fontSize:"10px",color:"#8a6aaa",marginLeft:"8px"}}>{u.rol==="ejecutor"?"EJECUTOR":"MASTER GROWER"}</span>
                        {u.activo===false && <span style={{fontSize:"10px",color:"#aa5050",marginLeft:"8px"}}>DESACTIVADO</span>}
                      </div>
                      <div style={{display:"flex",gap:"6px"}}>
                        <button onClick={()=>setEditingUsuario({id:u.id, nombre:u.nombre, email:u.email, rol:u.rol})} style={{padding:"5px 9px",borderRadius:"6px",fontSize:"10px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>✏️</button>
                        {u.id!==usuario.id && (
                          <button onClick={()=>toggleActivoUsuario(u)} style={{padding:"5px 9px",borderRadius:"6px",fontSize:"10px",background:"#2a1010",color:"#aa5050",border:"1px solid #4a2020"}}>{u.activo===false?"↺ Activar":"⏸ Desactivar"}</button>
                        )}
                      </div>
                    </div>
                    {u.rol==="ejecutor" && (
                      <button onClick={()=>togglePermisoRelevamiento(u)} style={{padding:"6px 10px",borderRadius:"6px",fontSize:"10px",background:u.permisos?.relevamiento?"#1a4a3a":"#2a1a3a",color:u.permisos?.relevamiento?"#3a9a7a":"#9a7aba",border:`1px solid ${u.permisos?.relevamiento?"#2a7a5a":"#4a3a6a"}`}}>🔍 {u.permisos?.relevamiento?"Relevamiento: ON":"Relevamiento: OFF"}</button>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div style={{marginBottom:"14px",border:"1px solid #3a2a4a",borderRadius:"12px",background:"#120c1a",overflow:"hidden"}}>
              <div onClick={()=>setEstructuraAbierto(!estructuraAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:"#140e1c"}}>
                <span style={{...lbl("#c090e0"),margin:0}}>🧱 ESTRUCTURA <span style={{color:"#8a6a9a",fontWeight:"400"}}>· Sectores y subsectores</span></span>
                <span style={{fontSize:"13px",color:"#c090e0",transform:estructuraAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
              </div>
              ...
              <div style={card({borderColor:"#5a3a7a",background:"#1a1428"})}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                  ...
                  {sectoresPadre.find(sp=>sp.id===editingSub.sector_padre_id)?.nombre==="Vegetativo" && (
                    <label style={{display:"flex",alignItems:"flex-start",gap:"10px",fontSize:"13px",color:"#c0a8d0",background:"#1a1024",borderRadius:"8px",padding:"10px 12px",border:"1px solid #3a2a4a"}}>
                      ...
                      <span>Permite avanzar a Floración<br/><span style={{fontSize:"11px",color:"#8a6a9a"}}>Si lo destildás, los lotes en este subsector no van a poder cortar directo a flora (ej: 150cc todavía muy chico).</span></span>
                    </label>
                  )}
                  <div style={{display:"flex",gap:"10px"}}>
            ...
            {sectoresPadre.map(sp=>{
              ...
              <div key={sp.id} style={card({borderColor:"#3a2a4a"})}>
                <div onClick={()=>setSectorMiSalaAbierto(abierto?"":sp.id)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:abierto?"14px":"0"}}>
                  <span style={{fontSize:"17px",fontWeight:"600",color:"#c090e0"}}>{sp.nombre} <span style={{fontSize:"11px",color:"#8a6a9a",fontWeight:"400"}}>· {subsDeEste.length}</span></span>
                  <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                    {abierto && <button onClick={(e)=>{e.stopPropagation(); setEditingSub(emptySubsector(sp.id));}} style={{fontSize:"12px",padding:"6px 14px",borderRadius:"14px",background:"#2a1a3a",color:"#c090e0",border:"1px dashed #6a3a8a"}}>+ Agregar</button>}
                    <span style={{fontSize:"13px",color:"#8a6a9a",transform:abierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                  </div>
                </div>
                {abierto && (
                <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                  {subsDeEste.map(sub=>(
                    <div key={sub.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#111e11",borderRadius:"8px",padding:"10px 14px"}}>
                      <div>
                        ...
                        {sp.nombre==="Vegetativo" && sub.puede_avanzar_etapa===false && <div style={{fontSize:"10px",color:"#aa7a40"}}>⏸ no habilitada para pasar a Floración</div>}
                      </div>
                      <div style={{display:"flex",gap:"6px"}}>
                        <button onClick={()=>setEditingSub({id:sub.id, sector_padre_id:sub.sector_padre_id, nombre:sub.nombre, capacidad_unidades:sub.capacidad_unidades||"", puede_avanzar_etapa:sub.puede_avanzar_etapa})} style={{fontSize:"11px",padding:"6px 10px",borderRadius:"6px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>✏️</button>
                  ...
                  {subsDeEste.length===0 && <div style={{fontSize:"12px",color:"#3a5a3a",fontStyle:"italic"}}>Sin subsectores</div>}
                </div>
                )}
              </div>
              );
        ...
        {tab==="_reportes" && !esEjecutor && (()=>{
          ...
          const BarraStacked = ({items, total}) => (
            <div>
              <div style={{display:"flex",height:"22px",borderRadius:"6px",overflow:"hidden",marginBottom:"8px"}}>
                {items.map((it,i)=><div key={i} style={{width:`${(it.cant/total*100).toFixed(1)}%`,background:it.g.color||"#5a8a5a"}}/>)}
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:"4px"}}>
                {items.map((it,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:"12px"}}>
                    <span style={{color:it.g.color||"#9aaa9a"}}>● {it.g.nombre}</span>
          ...
          return (
            <div className="fade">
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"#5a9ad0",marginBottom:"6px"}}>📊 Reportes</div>
              <div style={{fontSize:"12px",color:"#5a7a8a",marginBottom:"16px"}}>Tocá cada reporte para abrirlo</div>

              <Reporte id="distribucion" titulo="🧬 Distribución de genéticas (total activo)" color="#7ec850" vacio={globalItems.length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin plantas activas.</div>}>
                <BarraStacked items={globalItems} total={globalTotal}/>
              </Reporte>

              <Reporte id="subsector" titulo="📍 Genéticas por subsector" color="#a77dc2" vacio={subsConPlantas.length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin plantas en subsectores.</div>}>
                <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
                  {subsConPlantas.map((s,i)=>(
                    <div key={i}>
                      <div style={{fontSize:"13px",color:"#dff0cf",marginBottom:"6px"}}>{s.nombre} <span style={{fontSize:"11px",color:"#7a9a7a"}}>· {s.sp} · {s.total} plantas</span></div>
              ...
              <Reporte id="produccion" titulo="⚖️ Producción por genética (g/planta)" color="#c8a020" vacio={produccionOrdenada.length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin cosechas registradas todavía.</div>}>
                {produccionOrdenada.map((x,i)=>(
                  <div key={i} style={{marginBottom:"10px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",marginBottom:"4px"}}>
                      <span style={{color:x.g.color||"#c8a020"}}>{x.g.nombre}</span>
                      <span style={{color:"#e0c060",fontWeight:"600"}}>{x.porPlanta.toFixed(1)} g/planta <span style={{fontSize:"10px",color:"#8a7a4a",fontWeight:"400"}}>({x.gramos}g · {x.plantas} pl.)</span></span>
                    </div>
                    <div style={{height:"8px",background:"#1a1408",borderRadius:"4px",overflow:"hidden"}}><div style={{width:`${(x.porPlanta/maxProd*100)}%`,height:"100%",background:"#c8a020"}}/></div>
                  </div>
                ))}
              </Reporte>

              <Reporte id="mortandad" titulo="🥀 Mortandad por genética (bajas)" color="#c0392b" vacio={bajasOrdenadas.length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin bajas registradas.</div>}>
                {bajasOrdenadas.map((x,i)=>(
                  <div key={i} style={{marginBottom:"10px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",marginBottom:"4px"}}>
                      <span style={{color:x.g.color||"#d08070"}}>{x.g.nombre}</span>
                      <span style={{color:"#e07060",fontWeight:"600"}}>{x.bajas} bajas</span>
                    </div>
                    <div style={{height:"8px",background:"#1a0808",borderRadius:"4px",overflow:"hidden"}}><div style={{width:`${(x.bajas/maxBaja*100)}%`,height:"100%",background:"#c0392b"}}/></div>
                  </div>
                ))}
              </Reporte>

              <Reporte id="ocupacion" titulo="📦 Ocupación de subsectores" color="#3a8ad0" vacio={ocupacion.length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin subsectores con capacidad o plantas.</div>}>
                {ocupacion.map((o,i)=>{
                  const pct = o.cap>0 ? Math.min(100, o.plantas/o.cap*100) : 0;
                  const col = pct>=90?"#c0392b":pct>=70?"#c8a020":"#3a8a5a";
                  return (
                    <div key={i} style={{marginBottom:"10px"}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",marginBottom:"4px"}}>
                        <span style={{color:"#cce0d0"}}>{o.nombre} <span style={{fontSize:"10px",color:"#5a7a7a"}}>· {o.sp}</span></span>
                        <span style={{color:col}}>{o.plantas}{o.cap>0?` / ${o.cap}`:""}</span>
                      </div>
                      {o.cap>0 && <div style={{height:"8px",background:"#0a1a1e",borderRadius:"4px",overflow:"hidden"}}><div style={{width:`${pct}%`,height:"100%",background:col}}/></div>}
                    </div>
                  );
                })}
              </Reporte>

              <Reporte id="prom_genetica" titulo="⏱ Tiempo promedio por etapa (por genética)" color="#16a085" vacio={Object.keys(promedios).length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Todavía no hay etapas terminadas. Un promedio aparece recién cuando un lote sale de una etapa con un movimiento real (avance o cosecha) — los lotes de la carga inicial que no se movieron no cuentan.</div>}>
                <div style={{fontSize:"11px",color:"#5a7a7a",marginBottom:"10px",lineHeight:"1.4"}}>Solo cuenta etapas que el lote <b>terminó</b> (salió de ellas con un movimiento). Los ciclos en curso y la carga inicial sin movimientos no se promedian.</div>
                <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                  {Object.entries(promedios).map(([gid,etapas])=>{
                    ...
                    return (
                      <div key={gid}>
                        <div style={{fontSize:"13px",color:g.color||"#16a085",marginBottom:"6px"}}>{g.nombre}</div>
                        {sectoresPadre.map(sp=>{
                          const e = etapas[sp.nombre];
                          if(!e) return null;
                          return <div key={sp.id} style={{display:"flex",justifyContent:"space-between",fontSize:"12px",marginBottom:"3px"}}><span style={{color:"#8aaa9a"}}>{sp.nombre}</span><span style={{color:"#bcd"}}>{Math.round(e.suma/e.n)} días <span style={{color:"#5a7a7a"}}>(prom. {e.n})</span></span></div>;
                        })}
                      </div>
                    );
                  })}
              ...
              <Reporte id="info_lotes" titulo="📋 Reporte info. LOTES" color="#e8834a" vacio={lotes.length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin lotes cargados.</div>}>
                ...
                <div style={{fontSize:"11px",color:"#8a6a4a",marginBottom:"8px"}}>{lotesInfo.length} lote{lotesInfo.length!==1?"s":""}</div>
                <div style={{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"12px"}}>
                  {lotesInfo.map(({l,s,spL,st,g})=>(
                    <div key={l.id} style={{background:"#1e1610",borderRadius:"8px",padding:"9px 11px",border:"1px solid #4a3a2a"}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:"12px",marginBottom:"3px"}}>
                        <span style={{color:g?.color||"#c0a080",fontWeight:"600"}}>{g?.nombre||"Sin genética"}{l.numero_lote?` · ${loteLabel(l.numero_lote, sufijoDeLote(l))}`:""}</span>
                        <span style={{color:"#b09070"}}>{l.cantidad_plantas} pl.</span>
                      </div>
                      <div style={{fontSize:"10px",color:"#8a7a6a",marginBottom:"4px"}}>{l.estado} · {s?.nombre||"—"} ({spL?.nombre||"—"}) · inicio {l.fecha_inicio||"—"}</div>
                      <div style={{display:"flex",gap:"8px",flexWrap:"wrap",fontSize:"10px"}}>
                        <span style={{color:"#7ab070"}}>Enr {st.dias_enraizado!=null?st.dias_enraizado:"—"}d</span>
                        <span style={{color:"#7ab070"}}>Veg {st.dias_vegetativo!=null?st.dias_vegetativo:"—"}d</span>
                        <span style={{color:"#a77dc2"}}>Flo {st.dias_floracion!=null?st.dias_floracion:"—"}d</span>
                        <span style={{color:"#c8a020"}}>Sec {(l.dias_secado!=null?l.dias_secado:(st.dias_secado!=null?st.dias_secado:"—"))}d</span>
                        {l.estado==="cosechado" && <span style={{color:"#e0c060"}}>⚖ {l.peso_cosechado_gramos||0}g</span>}
                      </div>
                    </div>
                  ))}
                </div>
                <button onClick={descargarTSVLotes} disabled={lotesInfo.length===0} style={{width:"100%",padding:"11px",borderRadius:"9px",fontSize:"13px",background:"#3a2a1a",color:"#e8a860",border:"1px solid #6a4a2a",fontWeight:"600"}}>⬇ Descargar TSV ({lotesInfo.length} lotes)</button>
              </Reporte>
      ...
      {editingLote && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
          <div style={{background:"#182818",border:"1px solid #2a5a2a",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"19px",color:"#7ec850",marginBottom:"16px"}}>{editingLote.id ? `Editar lote${editingLote.numero_lote?` · ${loteLabel(editingLote.numero_lote)}`:""}` : "Nueva planta / lote"}</div>
            <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
              <div>
      ...
      {editingTarea && (()=>{
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200,overflowY:"auto"}}>
            <div style={{background:"#0a1e1a",border:"1px solid #0e6a5a",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px",margin:"auto"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"19px",color:"#16a085",marginBottom:"16px"}}>{editingTarea.id?"Editar tarea":"Nueva tarea"}</div>
              <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                <div>
                ...
                <div>
                  ...
                  {tiposDisponibles.length===0 ? (
                    <div style={{fontSize:"12px",color:"#8a7a40",background:"#2a2410",borderRadius:"8px",padding:"10px 12px"}}>No hay tipos de tarea para este subsector — agregalos en ⚙️ Mi Sala.</div>
                  ) : (
                ...
                <div style={{display:"flex",gap:"10px"}}>
                  <button disabled={savingTarea || !editingTarea.titulo || !editingTarea.fecha_programada || !editingTarea.tipo_id} onClick={saveTarea} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:savingTarea?"#0e4a3a":"#0e8a6a",color:"#fff",border:"1px solid #16a085"}}>{savingTarea?"Guardando...":"✓ Guardar"}</button>
      ...
      {movingLote && (()=>{
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"#182818",border:"1px solid #2a5a2a",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"19px",color:"#7ec850",marginBottom:"6px"}}>{modoRetroceso ? "↩ Volver a Vegetativo" : modoMover ? "↔ Mover lote" : "Avanzar etapa"}</div>
              <div style={{fontSize:"13px",color:"#8aaa8a",marginBottom:"16px"}}>{g?.nombre} · {movingLote.cantidad_plantas} plantas{modoRetroceso ? ` → ${subDestinoFijo?.nombre}` : !modoMover && ` → ${spDestinoNatural?.nombre}`}</div>
              {modoMover && <div style={{fontSize:"11px",color:"#7a9ad0",marginBottom:"14px"}}>Para corregir un error de ubicación. No se puede volver a Enraizado, ni pasar a una maceta más chica dentro de Vegetativo.</div>}
              {modoRetroceso && <div style={{fontSize:"11px",color:"#7aa080",marginBottom:"14px"}}>Todavía no entró en 12/12, así que no cuenta como floración real — vuelve a Vegetativo, a Vege 5L (el paso inmediatamente anterior). No se puede ir más atrás.</div>}
              <div style={{...lbl("#5a9a5a"),marginBottom:"6px"}}>CANTIDAD A {modoRetroceso?"VOLVER":modoMover?"MOVER":"AVANZAR"}</div>
              ...
              <div style={{fontSize:"11px",color:"#5a8a5a",marginBottom:"16px"}}>{(parseInt(movingMovida)||0) < movingLote.cantidad_plantas && (parseInt(movingMovida)||0)>0 ? `Quedan ${movingLote.cantidad_plantas-(parseInt(movingMovida)||0)} plantas en ${spOrigen?.nombre} — se crea un lote hermano en el destino` : `${modoRetroceso?"Vuelve":modoMover?"Mueve":"Avanza"} el lote completo`}</div>
              {modoRetroceso ? (
                <div style={{marginBottom:"16px"}}>
                  <div style={{...lbl("#5a9a5a"),marginBottom:"6px"}}>SUBSECTOR DESTINO</div>
                  <div style={{...inp(),background:"#0e2a1a",color:"#7aa080"}}>{subDestinoFijo?.nombre||"—"}</div>
                </div>
              ) : (
                <>
                  <div style={{...lbl("#5a9a5a"),marginBottom:"6px"}}>SUBSECTOR DESTINO</div>
              ...
              <div style={{display:"flex",gap:"10px"}}>
                <button disabled={!destinoSub || savingMovida || (parseInt(movingMovida)||0)<=0 || (parseInt(movingMovida)||0)>movingLote.cantidad_plantas} onClick={confirmarMovimiento} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"#3a6a10",color:"#dff0cf",border:"1px solid #5a9a20"}}>{savingMovida?"Moviendo...":"✓ Confirmar"}</button>
                <button onClick={cerrar} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
              </div>
            </div>
          </div>
        );
      ...
      {(()=>{
        ...
        return (
          <>
            ...
            <div style={{position:"fixed",left:0,right:0,bottom:0,background:"#152015",borderRadius:"18px 18px 0 0",borderTop:"1px solid #2a4a2a",transform:masAbierto?"translateY(0)":"translateY(100%)",transition:"transform .25s ease",zIndex:160,padding:"10px 20px 28px",maxHeight:"70vh",overflow:"auto"}}>
              <div style={{width:"36px",height:"4px",background:"#3a4a3a",borderRadius:"3px",margin:"6px auto 16px"}}/>
              {[["CULTIVO",grupoCultivo],["REGISTROS",grupoRegistros],["SISTEMA",grupoSistema]].filter(([,g])=>g.length>0).map(([label,grupo])=>(
                <div key={label}>
                  <div style={{fontSize:"10px",letterSpacing:"2px",color:"#5a8a7a",margin:"14px 0 8px"}}>{label}</div>
                  {grupo.map(t=>(
                    <div key={t.key} onClick={()=>irA(t.key)} style={{display:"flex",alignItems:"center",gap:"10px",padding:"11px 6px",fontSize:"14px",color:tab===t.key?t.col:"#cce0d0",background:tab===t.key?`${t.col}1a`:"transparent",borderRadius:"8px",borderBottom:"1px solid #1f2f1f"}}>{t.label}</div>
                  ))}
                </div>
              ))}
            </div>

            <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#101c10",borderTop:"1px solid #2a4a2a",display:"flex",padding:"6px 4px 10px",zIndex:140}}>
              {sectoresPadre.map(sp=>{
                const c = colorDeFase(sp.orden);
                const activo = tab===sp.id;
                return (
                  <button key={sp.id} onClick={()=>setTab(sp.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:"3px",padding:"6px 2px",background:"none",border:"none",color:activo?c:"#5a7a5a",fontSize:"10px"}}>
                    <span style={{fontSize:"15px"}}>{iconoDeFase(sp.orden)}</span>
                    <span>{sp.nombre}</span>
                    <span style={{width:activo?"7px":"5px",height:activo?"7px":"5px",borderRadius:"50%",background:activo?c:"#3a5a3a",boxShadow:activo?`0 0 6px ${c}`:"none"}}/>
                  </button>
                );
              })}
              <button onClick={()=>setMasAbierto(true)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:"3px",padding:"6px 2px",background:"none",border:"none",color:masActivo?"#7ec850":"#5a7a5a",fontSize:"10px"}}>
                <span style={{fontSize:"15px"}}>☰</span>
                <span>Más</span>
                <span style={{width:masActivo?"7px":"5px",height:masActivo?"7px":"5px",borderRadius:"50%",background:masActivo?"#7ec850":"#3a5a3a",boxShadow:masActivo?"0 0 6px #7ec850":"none"}}/>
              </button>
            </div>
          </>
        );
