  ...
  const saveTarea = async () => {
    ...
    try {
      ...
      if(editingTarea.id) {
        ...
        const filas = subIds.map(sid=>({...base, sector_id: sid||null, grupo_id: grupoId}));
  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"var(--c074)",minHeight:"100vh",color:"var(--c019)"}}>
                          ...
                          return (
                            <div key={t.id} style={{background:"var(--c076)",borderRadius:"9px",padding:"10px 12px",borderLeft:"3px solid "+(atr?"var(--c010)":TAREA_PRIORIDAD_COLOR[t.prioridad]||"var(--c057)")}}>
                              <div style={{display:"flex",justifyContent:"space-between",gap:"8px",alignItems:"flex-start"}}>
                                <span style={{fontSize:"13px",color:"var(--c060)",fontWeight:"600"}}>{tObj?tObj.icono+" ":""}{t.titulo}</span>
      ...
      {editingTarea && (()=>{
        ...
        // Con checklist, unimos los tipos disponibles de todos los subsectores elegidos (sin duplicar).
        const tiposDisponibles = subIdsElegidos.length===0 ? tiposTareaParaSector(null) :
          [...new Map(subIdsElegidos.flatMap(sid=>tiposTareaParaSector(sid)).map(tt=>[tt.id,tt])).values()];
