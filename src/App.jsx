...
function RaizApp() {
  ...
  useEffect(()=>{
    if(!usuario) return;
    if(usuario.rol==="administrador") cargarPanelAdmin();
    else cargarTodo();
  },[usuario]);
  ...
  // ══ PANEL ADMINISTRADOR — ve y gestiona todas las salas, no pertenece a ninguna ══
  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
      ...
      {(()=>{
        ...
        const grupoCultivo = tabsExtra.filter(t=>["_tareas","_calendario","_relevamientos"].includes(t.key));
