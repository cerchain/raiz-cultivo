...
function RaizApp() {
  ...
  const crearEjecutorEntidad = async () => {
    ...
    try {
      ...
      // CRÍTICO con la RLS nueva: el acceso del ejecutor sale de usuario_salas,
      // no de sala_id. Sin esta fila, el ejecutor no vería NADA.
      const insUS = await supabase.from("usuario_salas").insert({ usuario_id: insU.data.id, sala_id: creandoEjecutorEntidad.salaId });
      if(insUS.error) console.error("usuario_salas:", insUS.error);
