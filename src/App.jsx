  ...
  const guardarUsuarioEntidad = async () => {
    ...
    try {
      ...
      if(emailCambio && orig?.auth_id){
        ...
        let out = {}; try { out = await resp.json(); } catch(_) {}
        if(!resp.ok){ setStatus("error: "+String(out.error||"no se pudo actualizar el email").slice(0,90)); setGuardandoUsuarioEntidad(false); setTimeout(()=>setStatus(""),6000); return; }
      } else {
