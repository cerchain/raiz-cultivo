...
export default function App() {
  ...
  const emptyRelevamiento = (sectorId) => ({ sector_id: sectorId||"", lote_id:"", cantidad_contada:"", notas:"" });
  ...
  const guardarRelevamiento = async () => {
    ...
    try {
      await supabase.from("relevamientos").insert({
        ...
        lote_id: editingRelevamiento.lote_id || null,
        cantidad_contada: parseInt(editingRelevamiento.cantidad_contada)||0,
        usuario_id: usuario.id,
    ...
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
      ...
      <div style={{padding:"20px 20px 48px"}}>
        ...
        {tab==="_relevamientos" && (()=>{
          const lotesDelSector = (secId) => lotes.filter(l=>l.sector_actual_id===secId && l.estado==="activo");
          const recuento = r => {
            if(r.lote_id){ const l = lotes.find(x=>x.id===r.lote_id); return l ? l.cantidad_plantas : null; }
            return lotesDelSector(r.sector_id).reduce((a,l)=>a+l.cantidad_plantas,0);
          };
          return (
            <div className="fade">
                  ...
                  <div style={card({borderColor:"#0e8a7a",background:"#0a1e1c"})}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                      <div>
                        ...
                        <select value={editingRelevamiento.sector_id} onChange={e=>setEditingRelevamiento({...editingRelevamiento,sector_id:e.target.value, lote_id:""})} style={inp()}>
                      ...
                      {editingRelevamiento.sector_id && lotesDelSector(editingRelevamiento.sector_id).length>0 && (
                        <div>
                          <div style={{...lbl("#5aa098"),marginBottom:"6px"}}>LOTE (OPCIONAL — vacío = todo el subsector)</div>
                          <select value={editingRelevamiento.lote_id} onChange={e=>setEditingRelevamiento({...editingRelevamiento,lote_id:e.target.value})} style={inp()}>
                            <option value="">Todo el subsector</option>
                            {lotesDelSector(editingRelevamiento.sector_id).map(l=>{
                              const g = geneticas.find(x=>x.id===l.genetica_id);
                              return <option key={l.id} value={l.id}>{g?.nombre||"Sin genética"} (sistema: {l.cantidad_plantas})</option>;
                            })}
                          </select>
                        </div>
                      )}
                      <div>
                        <div style={{...lbl("#5aa098"),marginBottom:"6px"}}>CANTIDAD CONTADA (REAL)</div>
                        <input type="number" min="0" value={editingRelevamiento.cantidad_contada} onChange={e=>setEditingRelevamiento({...editingRelevamiento,cantidad_contada:e.target.value})} style={inp()}/>
                      </div>
                      <div>
              ...
              {relevamientos.length===0 ? (
                ...
                <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                  {relevamientos.map(r=>{
                    ...
                    const diff = sistema!==null ? r.cantidad_contada - sistema : null;
                    return (
                      <div key={r.id} style={{background:"#0e1a18",border:"1px solid #1abc9c33",borderRadius:"14px",padding:"16px",borderLeft:`4px solid ${diff===0?"#3a8a5a":diff!==null?"#c0392b":"#5a7a9a"}`}}>
                        ...
                        <div style={{display:"flex",gap:"16px",marginBottom:"6px"}}>
                          ...
                          {sistema!==null && <div><span style={{fontSize:"11px",color:"#5a9a90"}}>Sistema: </span><span style={{fontSize:"15px",color:"#9ab"}}>{sistema}</span></div>}
                          {diff!==null && diff!==0 && <div><span style={{fontSize:"11px",color:"#c0392b"}}>Diferencia: </span><span style={{fontSize:"15px",fontWeight:"700",color:"#c0392b"}}>{diff>0?"+":""}{diff}</span></div>}
                        </div>
