      ...
      <div style={{padding:"20px 20px 110px"}}>
        ...
        {tab==="_reportes" && !esEjecutor && (()=>{
          ...
          const tiemposPorLote = (lote) => {
            ...
            const tramos = [];
            // Tramo desde alta hasta el primer movimiento (etapa de origen del primer mov)
            ...
            // Tramos entre movimientos consecutivos
            for(let i=0;i<movs.length;i++){
              ...
              if(spDest) tramos.push({ etapa: spDest.nombre, dias: diasEntre(movs[i].fecha, finTramo), estimado: i===movs.length-1 });
            }
            // Agrupar por etapa (puede haber repetidas)
            const porEtapa = {};
          ...
          const seccion = (titulo, color) => ({fontSize:"13px",fontWeight:"600",color,letterSpacing:"1px",margin:"22px 0 10px"});

          return (
            <div className="fade">
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"#5a9ad0",marginBottom:"6px"}}>📊 Reportes</div>
              <div style={{fontSize:"12px",color:"#5a7a8a",marginBottom:"4px"}}>TIEMPOS · DISTRIBUCIÓN · PRODUCCIÓN · MORTANDAD · OCUPACIÓN</div>

              {/* ── DISTRIBUCIÓN GLOBAL ── */}
              <div style={seccion("🧬 DISTRIBUCIÓN DE GENÉTICAS (TOTAL ACTIVO)","#7ec850")}></div>
              {globalItems.length>0 ? (
                <div style={card({borderColor:"#2a4a2a"})}><BarraStacked items={globalItems} total={globalTotal}/></div>
              ) : <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin plantas activas.</div>}

              {/* ── % POR SUBSECTOR ── */}
              <div style={seccion("📍 GENÉTICAS POR SUBSECTOR","#a77dc2")}></div>
              {subsConPlantas.length>0 ? subsConPlantas.map((s,i)=>(
                <div key={i} style={card({borderColor:"#3a2a4a"})}>
                  <div style={{fontSize:"14px",color:"#dff0cf",marginBottom:"4px"}}>{s.nombre} <span style={{fontSize:"11px",color:"#7a9a7a"}}>· {s.sp} · {s.total} plantas</span></div>
                  <BarraStacked items={s.items} total={s.total}/>
                </div>
              )) : <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin plantas en subsectores.</div>}

              {/* ── PRODUCCIÓN POR GENÉTICA ── */}
              <div style={seccion("⚖️ PRODUCCIÓN POR GENÉTICA (GRAMOS COSECHADOS)","#c8a020")}></div>
              {produccionOrdenada.length>0 ? (
                <div style={card({borderColor:"#4a3a10"})}>
              ...
              ) : <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin cosechas registradas todavía.</div>}

              {/* ── MORTANDAD ── */}
              <div style={seccion("🥀 MORTANDAD POR GENÉTICA (BAJAS)","#c0392b")}></div>
              {bajasOrdenadas.length>0 ? (
                <div style={card({borderColor:"#4a1a1a"})}>
              ...
              ) : <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin bajas registradas.</div>}

              {/* ── OCUPACIÓN ── */}
              <div style={seccion("📦 OCUPACIÓN DE SUBSECTORES","#3a8ad0")}></div>
              {ocupacion.length>0 ? (
                <div style={card({borderColor:"#1a3a4a"})}>
                  {ocupacion.map((o,i)=>{
              ...
              ) : <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin subsectores con capacidad o plantas.</div>}

              {/* ── TIEMPO POR ETAPA: PROMEDIO POR GENÉTICA ── */}
              <div style={seccion("⏱ TIEMPO PROMEDIO POR ETAPA (POR GENÉTICA)","#16a085")}></div>
              {Object.keys(promedios).length>0 ? Object.entries(promedios).map(([gid,etapas])=>{
                const g = geneticas.find(x=>x.id===gid);
                if(!g) return null;
                return (
                  <div key={gid} style={card({borderColor:"#0e4a3a"})}>
                    <div style={{fontSize:"14px",color:g.color||"#16a085",marginBottom:"8px"}}>{g.nombre}</div>
                    {sectoresPadre.map(sp=>{
              ...
              }) : <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin datos de movimientos todavía.</div>}

              {/* ── TIEMPO POR ETAPA: POR LOTE ── */}
              <div style={seccion("⏱ TIEMPO POR ETAPA (POR LOTE)","#16a085")}></div>
              {lotesConHistorial.length>0 ? lotesConHistorial.map(l=>{
                const g = geneticas.find(x=>x.id===l.genetica_id);
                const tiempos = tiemposPorLote(l);
                const hayEstimado = tiempos.some(t=>t.estimado);
                return (
                  <div key={l.id} style={card({borderColor:"#0e3a3a"})}>
                    <div style={{fontSize:"13px",color:g?.color||"#16a085",marginBottom:"6px"}}>{g?.nombre||"Sin genética"}{l.numero_lote?` · ${loteLabel(l.numero_lote, sufijoDeLote(l))}`:""}</div>
                    {tiempos.map((t,i)=>(
                      <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:"12px",marginBottom:"3px"}}>
                        <span style={{color:"#8aaa9a"}}>{t.etapa}</span>
                        <span style={{color:"#bcd"}}>{t.dias} días {t.estimado && <span style={{color:"#a0843a"}}>≈ est.</span>}</span>
                      </div>
                    ))}
                    {hayEstimado && <div style={{fontSize:"10px",color:"#7a6a3a",marginTop:"6px"}}>≈ estimado: este lote no tiene historial completo de movimientos, se calcula desde su fecha de alta.</div>}
                  </div>
                );
              }) : <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin lotes para analizar.</div>}
