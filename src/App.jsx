...
export default function App() {
  ...
  // ── LOTES (alta y edición de plantas en subsector) ──
  const saveLote = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        estado: editingLote.estado || "activo",
        notas: editingLote.notas || ""
      };

      if(editingLote.id){
        // ── EDICIÓN de lote existente ──
        const orig = editingLote._original || {};
        const cantidadAnterior = orig.cantidad_plantas ?? nuevaCantidad;
        const estadoAnterior = orig.estado ?? "activo";

        await supabase.from("lotes").update(payload).eq("id", editingLote.id);

        if(estadoAnterior!=="descartado" && payload.estado==="descartado"){
          // Baja total del lote
          await supabase.from("movimientos_lote").insert({
            lote_id: editingLote.id,
            sector_origen_id: editingLote.sector_actual_id,
            sector_destino_id: null,
            cantidad_movida: cantidadAnterior,
            usuario_id: usuario.id,
            tipo: "baja",
            motivo: editingLote.motivo || null
          });
        } else if(payload.estado!=="descartado" && nuevaCantidad < cantidadAnterior){
          // Baja parcial (mortandad, descarte de algunas plantas) sin sacar el lote de circulación
          await supabase.from("movimientos_lote").insert({
            lote_id: editingLote.id,
            sector_origen_id: editingLote.sector_actual_id,
            sector_destino_id: editingLote.sector_actual_id,
            cantidad_movida: cantidadAnterior - nuevaCantidad,
            usuario_id: usuario.id,
            tipo: "baja_parcial",
            motivo: editingLote.motivo || null
          });
                          ...
                          return (
                            <div key={l.id} style={{background:"#111e11",borderRadius:"8px",padding:"10px 12px",border:`1px solid ${g?.color||"#2a4a2a"}44`}}>
                              ...
                              <div style={{display:"flex",gap:"6px"}}>
                                {!esEjecutor && (
                                  <button onClick={()=>setEditingLote({...l, cantidad_plantas:String(l.cantidad_plantas), motivo:"", _original:{cantidad_plantas:l.cantidad_plantas, estado:l.estado}})} style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>✏️</button>
                                )}
                                {!esEjecutor && sigPadre && (
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
          <div style={{background:"#182818",border:"1px solid #2a5a2a",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
              ...
              {editingLote.id && (
                <div>
                  <div style={{...lbl("#5a9a5a"),marginBottom:"6px"}}>ESTADO</div>
                  <select value={editingLote.estado||"activo"} onChange={e=>setEditingLote({...editingLote,estado:e.target.value})} style={inp()}>
                    <option value="activo">Activo</option>
                    <option value="descartado">Descartado (baja total)</option>
                  </select>
                </div>
              )}
              {editingLote.id && (editingLote.estado==="descartado" || parseInt(editingLote.cantidad_plantas||0) < (editingLote._original?.cantidad_plantas ?? Infinity)) && (
                <div>
                  <div style={{...lbl("#c8a020"),marginBottom:"6px"}}>MOTIVO DE LA BAJA</div>
                  <input value={editingLote.motivo||""} onChange={e=>setEditingLote({...editingLote,motivo:e.target.value})} placeholder="Ej: plaga, muerte, error de carga..." style={inp()}/>
                </div>
              )}
              <div>
