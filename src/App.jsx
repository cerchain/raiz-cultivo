...
function RaizApp() {
  ...
  const [rFiltroEstado, setRFiltroEstado] = useState("todos"); // todos|aplicado|pendiente
  ...
  const saveTarea = async () => {
    ...
    try {
      ...
      if(editingTarea.id) {
        ...
        // Nueva tarea: checklist de subsectores. 1 subsector = 1 fila sin grupo_id.
        // 2+ subsectores = 1 fila por subsector, todas con el mismo grupo_id (Opción B).
        // Cada fila recibe SU PROPIO numero_tarea (son tareas independientes con su propio estado).
        ...
        for(const sid of subIds){
          ...
          filas.push({...base, sector_id: sid||null, grupo_id: grupoId, numero_tarea: numRpc.data});
        }
  ...
  const emptyRelevamiento = (sectorId) => ({ sector_id: sectorId||"", sector_ids: sectorId?[sectorId]:[], notas:"", porSub:{} });
  ...
  const guardarRelevamiento = async () => {
    ...
    subIds.forEach(sid=>{
      ...
      lineas.filter(l=>l.genetica_id && l.cantidad!=="").forEach(l=>items.push({sid, linea:l}));
    });
                          ...
                          {sectoresPadre.map(sp=>(
                            <div key={sp.id} style={{marginBottom:"6px"}}>
                              ...
                              {subsectoresDe(sp.id).map(sub=>(
                                <label key={sub.id} style={{display:"flex",alignItems:"center",gap:"8px",fontSize:"13px",color:"var(--c060)",padding:"4px 0",cursor:"pointer"}}>
                                  <input type="checkbox" checked={subIdsElegidos.includes(sub.id)} onChange={()=>toggleSubRel(sub.id)}/>
                                  {sub.nombre}
                                </label>
                              ))}
                            </div>
                    ...
                    <div style={{maxHeight:"180px",overflowY:"auto",border:"1px solid var(--c053)",borderRadius:"10px",padding:"8px 10px"}}>
                      {sectoresPadre.map(sp=>(
                        <div key={sp.id} style={{marginBottom:"6px"}}>
                          ...
                          {subsectoresDe(sp.id).map(sub=>(
                            <label key={sub.id} style={{display:"flex",alignItems:"center",gap:"8px",fontSize:"13px",color:"var(--c060)",padding:"4px 0",cursor:"pointer"}}>
                              <input type="checkbox" checked={subIdsElegidos.includes(sub.id)} onChange={()=>toggleSub(sub.id)}/>
                              {sub.nombre}
                            </label>
                          ))}
                        </div>
