...
const card = (x={}) => ({ background:"var(--c072)", border:"1px solid var(--c024)", borderRadius:"14px", padding:"18px", marginBottom:"16px", ...x });
const lbl  = (c="var(--c143)") => ({ fontSize:"11px", color:c, letterSpacing:"2.5px", display:"block", marginBottom:"8px", fontWeight:"600" });
const inp  = (x={}) => ({ background:"var(--c054)", border:"1px solid var(--c024)", borderRadius:"8px", color:"var(--c018)", fontSize:"14px", padding:"8px 12px", width:"100%", fontFamily:"'IBM Plex Mono',monospace", outline:"none", ...x });

const TIPO_LABEL = { indica:"Índica 🟣", sativa:"Sativa 🟢", hibrido:"Híbrido ⚖️" };
const TIPO_COLOR = { indica:"var(--c055)", sativa:"var(--c144)", hibrido:"var(--c098)" };

// Paleta atada al ciclo de la planta, no a colores sueltos por feature
// ═══════════════════════════════════════════════════════════════
// SISTEMA DE TEMAS — cada color de la app es un token con dos valores:
//   [ oscuro, claro ]   El oscuro es EXACTAMENTE el de siempre (cero cambios).
// El claro se generó invirtiendo luminosidad y forzando contraste WCAG AA
// (>=4.5:1) en todos los textos. Los botones sólidos conservan su saturación
// para que el texto blanco encima siga siendo legible.
// Se aplica con CSS custom properties: cambiar de tema NO re-renderiza nada,
// solo cambia el atributo data-theme del <html>.
// ═══════════════════════════════════════════════════════════════
const PALETA = {
  c001:["#7ec850","#477a26"],
  c002:["#2a2a2a","#d5d5d5"],
  ...
  c184:["#1a2418","#ebefeb"],
  c185:["#2a4a30","#e7ede8"],
  c186:["#5a4a2044","#6b572444"],
  c187:["#c0a868","#816a2b"],
  c188:["#3a2a10","#f2ede6"],
  c189:["#6a5020","#6e5321"],
  c190:["#162816","#eaf0ea"],
  c191:["#0e241e","#e9f2ef"],
  c192:["#16a08533","#117e6833"],
  c193:["#dfe8e0","#374f39"],
  c194:["#0e5a3a","#137c50"],
  c195:["#3a6a5a","#2c745c"],
  c196:["#7a6a9a","#422b71"],
...
const CSS_TEMAS =
  ...
  "html[data-theme='claro'] input,html[data-theme='claro'] select,html[data-theme='claro'] textarea{color-scheme:light}" +
  "html[data-theme='oscuro'] input,html[data-theme='oscuro'] select,html[data-theme='oscuro'] textarea{color-scheme:dark}";
...
let TEMA_ACTIVO = "oscuro";   // se sincroniza en el useEffect del tema
...
const TAREA_PRIORIDAD_COLOR = { alta:"var(--c009)", media:"var(--c003)", baja:"var(--c035)" };
const TAREA_ESTADO_LABEL = { pendiente:"Pendiente", en_progreso:"En progreso", completada:"Completada", atrasada:"Atrasada" };
const TAREA_ESTADO_COLOR = { pendiente:"var(--c056)", en_progreso:"var(--c003)", completada:"var(--c035)", atrasada:"var(--c009)" };
...
class ErrorBoundary extends Component {
  ...
  render(){
    if(this.state.error){
      return (
        <div style={{background:"var(--c073)",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:"16px",padding:"30px",fontFamily:"'IBM Plex Mono',monospace",color:"var(--c018)",textAlign:"center"}}>
          <div style={{fontSize:"40px"}}>🌱💥</div>
          <div style={{fontSize:"16px",color:"var(--c057)"}}>Algo se rompió en pantalla</div>
          <div style={{fontSize:"12px",color:"var(--c074)",maxWidth:"320px"}}>Tus datos en Supabase no se tocaron — esto es solo un error de la interfaz. Recargá la página.</div>
          <button onClick={()=>window.location.reload()} style={{padding:"12px 24px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"var(--c040)",color:"var(--c018)",border:"1px solid var(--c041)"}}>↺ Recargar</button>
        </div>
      );
    }
...
function RaizApp() {
  ...
  // Tema: "oscuro" | "claro" | "sistema". Persiste en usuarios.tema (viaja entre
  // celular/tablet/PC). localStorage es solo un cache para que no parpadee al abrir.
  ...
  // Aplica el tema al <html>. Cambiar data-theme reescribe todas las CSS vars
  // de una, sin re-render. Si la preferencia es "sistema", escucha los cambios del SO.
  useEffect(()=>{
    const aplicar = () => {
      const t = resolverTema(temaPref);
      TEMA_ACTIVO = t;
      document.documentElement.setAttribute("data-theme", t);
      setTemaTick(x=>x+1);   // fuerza re-render de lo que usa colorDeFase (no es CSS var)
    };
    aplicar();
  ...
  const cambiarTema = async (pref) => {
    setTemaPref(pref);                       // instantáneo, sin esperar a la red
    if(!usuario?.id) return;
    setGuardandoTema(true);
    const r = await supabase.from("usuarios").update({ tema: pref }).eq("id", usuario.id);
    if(r.error) setStatus("no se pudo guardar el tema: " + r.error.message.slice(0,60));
    else setUsuario(u => u ? {...u, tema: pref} : u);
    setGuardandoTema(false);
    if(r.error) setTimeout(()=>setStatus(""), 5000);
  };
      ...
      try {
        ...
        if(authUser){
          ...
          if(r.data && r.data.activo!==false){
            setUsuario(r.data);
            if(r.data.tema) setTemaPref(r.data.tema);   // el tema viaja con el usuario entre dispositivos
          }
  ...
  const agruparTareasPorSector = (lista) => {
    ...
    if(grupos["_sin"]) ordenados.push({key:"_sin", label:"Sin sector asignado", color:"var(--c058)", tareas:grupos["_sin"]});
    return ordenados;
  };
  ...
  const TareaCard = ({t}) => {
    ...
    return (
      <div style={{background:"var(--c075)",border:`1px solid ${TAREA_ESTADO_COLOR[estadoMostrado]}55`,borderRadius:"14px",padding:"16px",borderLeft:`4px solid ${TAREA_PRIORIDAD_COLOR[t.prioridad]||"var(--c056)"}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}>
          <div>
            <div style={{fontSize:"15px",fontWeight:"600",color:"var(--c059)"}}>{t.titulo}</div>
            <div style={{fontSize:"12px",color:"var(--c076)",marginTop:"2px"}}>{tipoObj ? `${tipoObj.icono} ${tipoObj.nombre}` : (TAREA_TIPO_LABEL[t.tipo] || t.tipo || "Sin tipo")}</div>
          </div>
        ...
        {(sp||g) && <div style={{fontSize:"12px",color:"var(--c145)",marginBottom:"6px"}}>{sp && `${sp.nombre} › ${sub.nombre}`}{g && ` · ${g.nombre}`}</div>}
        {t.descripcion && <div style={{fontSize:"13px",color:"var(--c146)",marginBottom:"8px",lineHeight:"1.5"}}>{t.descripcion}</div>}
        <div style={{display:"flex",justifyContent:"space-between",fontSize:"11px",color:"var(--c036)",marginBottom:"10px"}}>
        ...
        <div style={{display:"flex",gap:"6px",marginBottom:"10px"}}>
          {esMia && t.estado==="pendiente" && (
            <button onClick={()=>updateEstadoTarea(t,"en_progreso")} style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"var(--c077)",color:"var(--c003)",border:"1px solid var(--c078)"}}>▶ Iniciar</button>
          )}
          {esMia && (t.estado==="pendiente"||t.estado==="en_progreso") && (
            <button onClick={()=>updateEstadoTarea(t,"completada")} style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"var(--c099)",color:"var(--c035)",border:"1px solid var(--c147)"}}>✓ Completar</button>
          )}
          {!esEjecutor && (
            <button onClick={()=>setEditingTarea({...t, lote_id:t.lote_id||"", sector_id:t.sector_id||"", asignado_a:t.asignado_a||"", descripcion:t.descripcion||"", tipo_id:t.tipo_id||""})} style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>✏️</button>
          )}
          {!esEjecutor && (
            <button onClick={()=>delTarea(t)} style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>🗑</button>
          )}
        </div>
        <button onClick={()=>setTareaExpandida(expandida?null:t.id)} style={{fontSize:"11px",color:"var(--c148)",background:"none",border:"none",padding:0}}>{expandida?"▲ Ocultar":"▼"} 💬 {comentarios.length} comentario{comentarios.length!==1?"s":""}</button>
        {expandida && (
          <div style={{marginTop:"10px",paddingTop:"10px",borderTop:"1px solid var(--c149)"}}>
            {comentarios.map(c=>(
              <div key={c.id} style={{marginBottom:"8px",fontSize:"12px"}}>
                <span style={{color:"var(--c020)",fontWeight:"600"}}>{usuariosMap[c.usuario_id]||"—"}</span>
                <span style={{color:"var(--c150)",marginLeft:"6px",fontSize:"10px"}}>{(c.creado_en||"").slice(0,16).replace("T"," ")}</span>
                <div style={{color:"var(--c151)",marginTop:"2px"}}>{c.texto}</div>
              </div>
            ))}
            <div style={{display:"flex",gap:"6px",marginTop:"8px"}}>
              ...
              <button onClick={()=>addComentario(t.id)} style={{padding:"8px 14px",borderRadius:"8px",fontSize:"12px",background:"var(--c027)",color:"#fff",border:"none"}}>Enviar</button>
            </div>
          </div>
        )}
      </div>
  ...
  if(!loaded)  return <div style={{background:"var(--c073)",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Mono',monospace",color:"var(--c152)",fontSize:"14px"}}>Cargando...</div>;
  ...
  if(esAdminEntidad && adminEntidadModo==="entidad"){
    if(!entidadLoaded) return <div style={{background:"var(--c012)",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Mono',monospace",color:"var(--c017)",fontSize:"14px"}}>Cargando panel de entidad...</div>;
    ...
    return (
      <div style={{background:"var(--c012)",minHeight:"100vh",fontFamily:"'IBM Plex Mono',monospace",color:"var(--c013)"}}>
        <div style={{background:"var(--c100)",borderBottom:"2px solid var(--c005)",padding:"18px 20px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"21px",color:"var(--c014)"}}>🏢 {entidadNombre}</div>
              <div style={{fontSize:"11px",color:"var(--c060)",letterSpacing:"2px",marginTop:"4px"}}>{usuario.nombre} · ADMINISTRADOR DE ENTIDAD</div>
            </div>
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"6px"}}>
              {status && <div style={{fontSize:"11px",color:status==="guardado"?"var(--c001)":"var(--c079)",background:"var(--c101)",padding:"4px 10px",borderRadius:"10px"}}>{status}</div>}
              <button onClick={handleLogout} style={{fontSize:"11px",color:"var(--c037)",background:"transparent",border:"1px solid var(--c005)",borderRadius:"14px",padding:"4px 12px"}}>salir</button>
            </div>
          </div>
          {usuario.es_master_grower && misSalasComoMG.length>0 && (
            <button onClick={()=>entrarModoMG()} style={{marginTop:"12px",width:"100%",padding:"11px",borderRadius:"10px",fontSize:"13px",fontWeight:"600",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>
        ...
        {eligiendoSalaMG && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"var(--c016)",border:"1px solid var(--c010)",borderRadius:"16px",padding:"20px",width:"100%",maxWidth:"360px"}}>
              <div style={{fontSize:"13px",color:"var(--c001)",marginBottom:"14px"}}>¿A qué sala querés entrar?</div>
              <div style={{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"12px"}}>
                {eligiendoSalaMG.map(s=>(
                  <button key={s.id} onClick={()=>entrarModoMG(s.id)} style={{padding:"12px",borderRadius:"10px",fontSize:"13px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)",textAlign:"left"}}>{s.nombre}</button>
                ))}
              </div>
              <button onClick={()=>setEligiendoSalaMG(null)} style={{width:"100%",padding:"10px",borderRadius:"10px",fontSize:"12px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
            </div>
          </div>
        )}

        <div style={{padding:"18px"}}>
          {totalAtrasadas>0 && <div style={{background:"var(--c007)",border:"1px solid var(--c080)",borderRadius:"10px",padding:"10px 14px",marginBottom:"16px",fontSize:"12px",color:"var(--c028)"}}>⚠ {totalAtrasadas} tarea{totalAtrasadas!==1?"s":""} atrasada{totalAtrasadas!==1?"s":""} en toda la entidad</div>}

          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"10px",marginBottom:"20px"}}>
            <div style={{background:"var(--c016)",borderRadius:"12px",padding:"14px 10px",textAlign:"center",border:"1px solid var(--c005)"}}>
              <div style={{fontSize:"22px",fontWeight:"700",color:"var(--c014)"}}>{entidadSalas.length}</div>
              <div style={{fontSize:"9px",color:"var(--c017)",marginTop:"4px"}}>SALAS</div>
            </div>
            <div style={{background:"var(--c016)",borderRadius:"12px",padding:"14px 10px",textAlign:"center",border:"1px solid var(--c005)"}}>
              <div style={{fontSize:"22px",fontWeight:"700",color:"var(--c001)"}}>{totalPlantas}</div>
              <div style={{fontSize:"9px",color:"var(--c017)",marginTop:"4px"}}>PLANTAS ACTIVAS</div>
            </div>
            <div style={{background:"var(--c016)",borderRadius:"12px",padding:"14px 10px",textAlign:"center",border:"1px solid var(--c005)"}}>
              <div style={{fontSize:"22px",fontWeight:"700",color:"var(--c003)"}}>{totalGramos.toFixed(0)}g</div>
              <div style={{fontSize:"9px",color:"var(--c017)",marginTop:"4px"}}>PRODUCCIÓN</div>
            </div>
          </div>

          <div style={{marginBottom:"24px",border:"1px solid var(--c005)",borderRadius:"14px",background:"var(--c153)",overflow:"hidden"}}>
            <div onClick={()=>setSalasEntidadAbierto(!salasEntidadAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:"var(--c100)"}}>
              <span style={{fontSize:"11px",color:"var(--c014)",letterSpacing:"2px",fontWeight:"600"}}>🚪 SALAS · {entidadSalas.length}</span>
              <span style={{fontSize:"13px",color:"var(--c014)",transform:salasEntidadAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
            </div>
            {salasEntidadAbierto && (<div style={{padding:"14px"}}>

          {creandoSalaEntidad ? (
            <div style={{background:"var(--c016)",border:"1px dashed var(--c025)",borderRadius:"12px",padding:"14px",marginBottom:"14px"}}>
              <div style={{fontSize:"10px",color:"var(--c014)",letterSpacing:"1px",marginBottom:"8px"}}>NUEVA SALA</div>
              <input value={creandoSalaEntidad.nombre||""} onChange={e=>setCreandoSalaEntidad({...creandoSalaEntidad,nombre:e.target.value})} placeholder="Nombre de la sala" style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c005)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"8px"}}/>
              <div style={{fontSize:"9px",color:"var(--c017)",marginBottom:"6px"}}>MASTER GROWER (opcional, podés asignarlo después)</div>
              <input value={creandoSalaEntidad.mgNombre||""} onChange={e=>setCreandoSalaEntidad({...creandoSalaEntidad,mgNombre:e.target.value})} placeholder="Nombre del MG" style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c005)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"8px"}}/>
              <input value={creandoSalaEntidad.mgEmail||""} onChange={e=>setCreandoSalaEntidad({...creandoSalaEntidad,mgEmail:e.target.value})} placeholder="Email del MG" style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c005)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"10px"}}/>
              <div style={{display:"flex",gap:"8px"}}>
                <button disabled={guardandoSalaEntidad || !creandoSalaEntidad.nombre} onClick={crearSalaEntidad} style={{flex:1,padding:"9px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"var(--c061)",color:"#fff",border:"none"}}>{guardandoSalaEntidad?"Guardando...":"✓ Crear sala"}</button>
                <button onClick={()=>setCreandoSalaEntidad(null)} style={{padding:"9px 14px",borderRadius:"8px",fontSize:"12px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
              </div>
            </div>
          ) : (
            <button onClick={()=>setCreandoSalaEntidad({nombre:"",mgNombre:"",mgEmail:""})} style={{width:"100%",padding:"12px",borderRadius:"10px",fontSize:"12px",fontWeight:"600",background:"var(--c016)",color:"var(--c014)",border:"2px dashed var(--c025)",marginBottom:"14px"}}>+ Nueva Sala</button>
          )}
          <div style={{display:"flex",flexDirection:"column",gap:"10px",marginBottom:"24px"}}>
            {entidadSalas.map(sala=>{
              ...
              const mgColor = mg ? "var(--c081)" : "var(--c009)";
              return (
                <div key={sala.id} style={{background:"var(--c016)",border:"1px solid var(--c005)",borderRadius:"14px",padding:"16px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                    <div>
                      <div style={{fontSize:"15px",fontWeight:"600",color:"var(--c102)"}}>{sala.nombre}</div>
                      <div style={{fontSize:"12px",color:mgColor,marginTop:"3px"}}>{mgLabel}</div>
                    </div>
                    <button onClick={()=>setEliminandoSalaEntidad({sala, confirmText:""})} style={{padding:"4px 9px",borderRadius:"6px",fontSize:"10px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)",whiteSpace:"nowrap"}}>🗑 Eliminar sala</button>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"6px"}}>
                    <div style={{background:"var(--c062)",borderRadius:"8px",padding:"8px 4px",textAlign:"center"}}>
                      <div style={{fontSize:"16px",fontWeight:"700",color:"var(--c001)"}}>{d.plantasActivas}</div>
                      <div style={{fontSize:"8px",color:"var(--c042)",marginTop:"2px"}}>PLANTAS</div>
                    </div>
                    <div style={{background:"var(--c062)",borderRadius:"8px",padding:"8px 4px",textAlign:"center"}}>
                      <div style={{fontSize:"16px",fontWeight:"700",color:d.tareasAtrasadas>0?"var(--c009)":"var(--c003)"}}>{d.tareasPendientes}</div>
                      <div style={{fontSize:"8px",color:"var(--c042)",marginTop:"2px"}}>TAREAS</div>
                    </div>
                    <div style={{background:"var(--c062)",borderRadius:"8px",padding:"8px 4px",textAlign:"center"}}>
                      <div style={{fontSize:"16px",fontWeight:"700",color:"var(--c003)"}}>{d.cosechasTotal}</div>
                      <div style={{fontSize:"8px",color:"var(--c042)",marginTop:"2px"}}>COSECHAS</div>
                    </div>
                    <div style={{background:"var(--c062)",borderRadius:"8px",padding:"8px 4px",textAlign:"center"}}>
                      <div style={{fontSize:"16px",fontWeight:"700",color:"var(--c001)"}}>{d.gramosTotal.toFixed(0)}g</div>
                      <div style={{fontSize:"8px",color:"var(--c042)",marginTop:"2px"}}>GRAMOS</div>
                    </div>
                  </div>
                  {d.tareasAtrasadas>0 && <div style={{marginTop:"8px",fontSize:"10px",color:"var(--c009)"}}>⚠ {d.tareasAtrasadas} tarea{d.tareasAtrasadas!==1?"s":""} atrasada{d.tareasAtrasadas!==1?"s":""}</div>}
                  <div style={{display:"flex",gap:"8px",flexWrap:"wrap",marginTop:"10px"}}>
                    <button onClick={()=>setCreandoEjecutorEntidad({nombre:"",email:"",salaId:sala.id})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"var(--c043)",color:"var(--c001)",border:"1px solid var(--c044)"}}>+ Nuevo Ejecutor para esta sala</button>
                    {!sala.master_grower_id ? (
                      <>
                        <button onClick={()=>setAgregandoMGSala({salaId:sala.id, salaNombre:sala.nombre, nombre:"", email:""})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"var(--c103)",color:"var(--c014)",border:"1px solid var(--c025)"}}>🌱 Agregar Master Grower</button>
                        <button disabled={asignandoComoMG} onClick={()=>asignarmeComoMG(sala)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"var(--c016)",color:"var(--c017)",border:"1px solid var(--c005)"}}>{asignandoComoMG?"Asignando...":"Asignarme a mí"}</button>
                      </>
                    ) : (mg && mg.activo===false) ? (
                      <button onClick={()=>reactivarODesactivarUsuarioEntidad(mg)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"var(--c043)",color:"var(--c001)",border:"1px solid var(--c044)"}}>↺ Reactivar MG ({mg.nombre})</button>
                    ) : (
                      <button disabled style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"var(--c154)",color:"var(--c155)",border:"1px solid var(--c156)",cursor:"default"}}>✓ MG asignado{mg?": "+mg.nombre:""}</button>
                    )}
                  </div>
                </div>
              );
          ...
          {linkInvitacion && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"var(--c016)",border:"1px solid var(--c010)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontSize:"13px",color:"var(--c001)",marginBottom:"10px"}}>✓ Cuenta creada para {linkInvitacion.nombre}</div>
              <div style={{fontSize:"11px",color:"var(--c157)",marginBottom:"12px",lineHeight:"1.6"}}>Todavía no puede loguearse. Mandale este link por WhatsApp o el medio que uses — al abrirlo va a poder poner su propia contraseña.</div>
              <div style={{background:"var(--c012)",border:"1px solid var(--c024)",borderRadius:"8px",padding:"10px 12px",fontSize:"11px",color:"var(--c013)",wordBreak:"break-all",marginBottom:"14px"}}>{linkInvitacion.url}</div>
              <div style={{display:"flex",gap:"8px"}}>
                <button onClick={()=>{navigator.clipboard.writeText(linkInvitacion.url); setStatus("link copiado"); setTimeout(()=>setStatus(""),1800);}} style={{flex:1,padding:"11px",borderRadius:"9px",fontSize:"12px",fontWeight:"600",background:"var(--c040)",color:"var(--c018)",border:"1px solid var(--c041)"}}>📋 Copiar link</button>
                <button onClick={()=>setLinkInvitacion(null)} style={{padding:"11px 16px",borderRadius:"9px",fontSize:"12px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cerrar</button>
              </div>
            </div>
          </div>
        )}
        ...
        {eliminandoSalaEntidad && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"var(--c045)",border:"1px solid var(--c046)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
                <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"18px",color:"var(--c028)",marginBottom:"10px"}}>⚠ Eliminar "{eliminandoSalaEntidad.sala.nombre}"</div>
                {(() => {
                  ...
                  return tieneDatos ? (
                    <div style={{background:"var(--c158)",border:"1px solid var(--c159)",borderRadius:"10px",padding:"10px 12px",fontSize:"11px",color:"var(--c160)",marginBottom:"12px"}}>
                    ...
                    <div style={{fontSize:"12px",color:"var(--c047)",marginBottom:"14px",lineHeight:"1.6"}}>Esta sala está vacía. Se borra definitivamente.</div>
                  );
                })()}
                <div style={{fontSize:"10px",color:"var(--c038)",letterSpacing:"1px",marginBottom:"6px"}}>ESCRIBÍ "{eliminandoSalaEntidad.sala.nombre}" PARA CONFIRMAR</div>
                <input value={eliminandoSalaEntidad.confirmText} onChange={e=>setEliminandoSalaEntidad({...eliminandoSalaEntidad,confirmText:e.target.value})} style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c008)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"14px"}}/>
                <div style={{display:"flex",gap:"10px"}}>
                  <button disabled={borrandoSalaEntidad || eliminandoSalaEntidad.confirmText!==eliminandoSalaEntidad.sala.nombre} onClick={confirmarEliminarSalaEntidad} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"var(--c048)",color:"#fff",border:"1px solid var(--c049)"}}>{borrandoSalaEntidad?"Eliminando...":"🗑 Eliminar definitivamente"}</button>
                  <button onClick={()=>setEliminandoSalaEntidad(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {creandoEjecutorEntidad && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"var(--c016)",border:"1px solid var(--c025)",borderRadius:"16px",padding:"20px",width:"100%",maxWidth:"360px"}}>
                <div style={{fontSize:"11px",color:"var(--c014)",letterSpacing:"1px",marginBottom:"12px"}}>NUEVO EJECUTOR</div>
                <input value={creandoEjecutorEntidad.nombre} onChange={e=>setCreandoEjecutorEntidad({...creandoEjecutorEntidad,nombre:e.target.value})} placeholder="Nombre" style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c005)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"8px"}}/>
                <input value={creandoEjecutorEntidad.email} onChange={e=>setCreandoEjecutorEntidad({...creandoEjecutorEntidad,email:e.target.value})} placeholder="Email" style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c005)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"14px"}}/>
                <div style={{display:"flex",gap:"8px"}}>
                  <button disabled={guardandoEjecutorEntidad || !creandoEjecutorEntidad.nombre || !creandoEjecutorEntidad.email} onClick={crearEjecutorEntidad} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"var(--c061)",color:"#fff",border:"none"}}>{guardandoEjecutorEntidad?"Guardando...":"✓ Crear"}</button>
                  <button onClick={()=>setCreandoEjecutorEntidad(null)} style={{padding:"10px 14px",borderRadius:"8px",fontSize:"12px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}

          <div style={{fontSize:"10px",color:"var(--c017)",letterSpacing:"2px",marginBottom:"12px"}}>USUARIOS DE LA ENTIDAD ({entidadUsuarios.length})</div>
          <div style={{display:"flex",flexDirection:"column",gap:"8px",paddingBottom:"30px"}}>
            {entidadUsuarios.map(u=>{
              ...
              const btnBg = u.activo===false?"var(--c043)":"var(--c007)";
              const btnColor = u.activo===false?"var(--c001)":"var(--c011)";
              const btnBorder = u.activo===false?"var(--c044)":"var(--c008)";
              ...
              return (
                <div key={u.id} style={{background:"var(--c016)",borderRadius:"10px",padding:"10px 14px",opacity:u.activo===false?0.5:1,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:"13px",color:"var(--c161)"}}>{u.nombre}</div>
                    <div style={{fontSize:"10px",color:"var(--c017)",marginTop:"2px"}}>{rolLabel}{u.es_master_grower?" + MG":""}{u.activo===false?" · INACTIVO":""}{!u.auth_id?" · SIN CUENTA CREADA":""}</div>
                    <div style={{fontSize:"10px",color:"var(--c162)",marginTop:"3px"}}>📍 {ubicacion}</div>
                    {u.email && <div style={{fontSize:"10px",color:"var(--c042)",marginTop:"2px"}}>{u.email}</div>}
                  </div>
                  {u.id!==usuario.id && (
                    <div style={{display:"flex",gap:"6px",flexWrap:"wrap",justifyContent:"flex-end"}}>
                      <button onClick={()=>setEditandoUsuarioEntidad({id:u.id, nombre:u.nombre||"", email:u.email||""})} style={{padding:"5px 10px",borderRadius:"6px",fontSize:"10px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>✏️</button>
                      {!u.auth_id && (
                        <button onClick={()=>{navigator.clipboard.writeText(window.location.origin+"/?invite="+u.id); setStatus("link copiado"); setTimeout(()=>setStatus(""),1800);}} style={{padding:"5px 10px",borderRadius:"6px",fontSize:"10px",background:"var(--c103)",color:"var(--c014)",border:"1px solid var(--c025)"}}>📋 Invitación</button>
                      )}
                      ...
                      <button onClick={()=>setEliminandoUsuarioEntidad({usuario:u, confirmText:""})} style={{padding:"5px 10px",borderRadius:"6px",fontSize:"10px",background:"var(--c007)",color:"var(--c082)",border:"1px solid var(--c080)"}}>🗑</button>
                    </div>
                  )}
                </div>
              );
          ...
          {eliminandoUsuarioEntidad && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"var(--c045)",border:"1px solid var(--c046)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
                <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"18px",color:"var(--c028)",marginBottom:"10px"}}>⚠ Eliminar a "{eliminandoUsuarioEntidad.usuario.nombre}"</div>
                <div style={{fontSize:"12px",color:"var(--c047)",marginBottom:"14px",lineHeight:"1.6"}}>Esto borra al usuario definitivamente (no solo lo desactiva). Su historial de tareas y movimientos queda, pero desvinculado. La cuenta de acceso (Auth) sigue existiendo del lado de Supabase — esto no la borra.</div>
                <div style={{fontSize:"10px",color:"var(--c038)",letterSpacing:"1px",marginBottom:"6px"}}>ESCRIBÍ "{eliminandoUsuarioEntidad.usuario.nombre}" PARA CONFIRMAR</div>
                <input value={eliminandoUsuarioEntidad.confirmText} onChange={e=>setEliminandoUsuarioEntidad({...eliminandoUsuarioEntidad,confirmText:e.target.value})} style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c008)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"14px"}}/>
                {status && status.startsWith("error") && <div style={{fontSize:"11px",color:"var(--c028)",marginBottom:"12px"}}>{status}</div>}
                <div style={{display:"flex",gap:"10px"}}>
                  <button disabled={borrandoUsuarioEntidad || eliminandoUsuarioEntidad.confirmText!==eliminandoUsuarioEntidad.usuario.nombre} onClick={confirmarEliminarUsuarioEntidad} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"var(--c048)",color:"#fff",border:"1px solid var(--c049)"}}>{borrandoUsuarioEntidad?"Eliminando...":"🗑 Eliminar definitivamente"}</button>
                  <button onClick={()=>setEliminandoUsuarioEntidad(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {agregandoMGSala && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"var(--c016)",border:"1px solid var(--c025)",borderRadius:"16px",padding:"20px",width:"100%",maxWidth:"360px"}}>
                <div style={{fontSize:"11px",color:"var(--c014)",letterSpacing:"1px",marginBottom:"4px"}}>NUEVO MASTER GROWER</div>
                <div style={{fontSize:"11px",color:"var(--c017)",marginBottom:"12px"}}>para la sala {agregandoMGSala.salaNombre}</div>
                <input value={agregandoMGSala.nombre} onChange={e=>setAgregandoMGSala({...agregandoMGSala,nombre:e.target.value})} placeholder="Nombre del MG" style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c005)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"8px"}}/>
                <input value={agregandoMGSala.email} onChange={e=>setAgregandoMGSala({...agregandoMGSala,email:e.target.value})} placeholder="Email del MG" style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c005)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"14px"}}/>
                <div style={{display:"flex",gap:"8px"}}>
                  <button disabled={guardandoMGSala || !agregandoMGSala.nombre || !agregandoMGSala.email} onClick={crearMGparaSala} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"var(--c061)",color:"#fff",border:"none"}}>{guardandoMGSala?"Guardando...":"✓ Crear MG"}</button>
                  <button onClick={()=>setAgregandoMGSala(null)} style={{padding:"10px 14px",borderRadius:"8px",fontSize:"12px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {editandoUsuarioEntidad && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
              <div style={{background:"var(--c016)",border:"1px solid var(--c025)",borderRadius:"16px",padding:"20px",width:"100%",maxWidth:"360px"}}>
                <div style={{fontSize:"11px",color:"var(--c014)",letterSpacing:"1px",marginBottom:"12px"}}>EDITAR USUARIO</div>
                <div style={{fontSize:"9px",color:"var(--c017)",marginBottom:"4px"}}>NOMBRE</div>
                <input value={editandoUsuarioEntidad.nombre} onChange={e=>setEditandoUsuarioEntidad({...editandoUsuarioEntidad,nombre:e.target.value})} placeholder="Nombre" style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c005)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"10px"}}/>
                <div style={{fontSize:"9px",color:"var(--c017)",marginBottom:"4px"}}>EMAIL</div>
                <input value={editandoUsuarioEntidad.email} onChange={e=>setEditandoUsuarioEntidad({...editandoUsuarioEntidad,email:e.target.value})} placeholder="Email" style={{width:"100%",background:"var(--c012)",border:"1px solid var(--c005)",borderRadius:"8px",color:"var(--c013)",fontSize:"13px",padding:"9px 12px",marginBottom:"6px"}}/>
                <div style={{fontSize:"10px",color:"var(--c104)",background:"var(--c163)",borderRadius:"6px",padding:"7px 10px",marginBottom:"14px",lineHeight:"1.4"}}>Si el usuario ya tiene cuenta, cambiar el email acá <b>también cambia su correo de acceso</b> (login). Queda confirmado al instante, sin mail de verificación. Los que todavía no aceptaron la invitación no tienen cuenta, así que solo se actualiza su ficha.</div>
                {status && status.startsWith("error") && <div style={{fontSize:"11px",color:"var(--c028)",background:"var(--c007)",border:"1px solid var(--c080)",borderRadius:"6px",padding:"8px 10px",marginBottom:"12px",lineHeight:"1.4",wordBreak:"break-word"}}>{status}</div>}
                <div style={{display:"flex",gap:"8px"}}>
                  <button disabled={guardandoUsuarioEntidad || !editandoUsuarioEntidad.nombre} onClick={guardarUsuarioEntidad} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"var(--c083)",color:"#fff",border:"none"}}>{guardandoUsuarioEntidad?"Guardando...":"✓ Guardar"}</button>
                  <button onClick={()=>setEditandoUsuarioEntidad(null)} style={{padding:"10px 14px",borderRadius:"8px",fontSize:"12px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                </div>
              </div>
            </div>
          )}
  ...
  if(usuario.rol==="administrador" && !usuario.entidad_id){
    if(!adminLoaded) return <div style={{background:"var(--c105)",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Mono',monospace",color:"var(--c164)",fontSize:"14px"}}>Cargando panel...</div>;
    return (
      <div style={{background:"var(--c105)",minHeight:"100vh",fontFamily:"'IBM Plex Mono',monospace",color:"var(--c013)"}}>
        <div style={{background:"var(--c106)",borderBottom:"2px solid var(--c005)",padding:"18px 20px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c014)"}}>🛡 Panel Administrador</div>
            <div style={{fontSize:"11px",color:"var(--c060)",letterSpacing:"2px",marginTop:"4px"}}>{usuario.nombre}</div>
          </div>
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"6px"}}>
            {status && <div style={{fontSize:"11px",color:status==="guardado"||status==="sala eliminada"?"var(--c001)":"var(--c079)",background:"var(--c101)",padding:"5px 10px",borderRadius:"10px"}}>{status}</div>}
            <button onClick={handleLogout} style={{fontSize:"11px",color:"var(--c037)",background:"transparent",border:"1px solid var(--c005)",borderRadius:"14px",padding:"4px 12px"}}>salir</button>
          </div>
        </div>
        ...
        <div style={{padding:"20px"}}>
          {editingMGAdmin ? (
            <div style={card({background:"var(--c165)",borderColor:"var(--c025)"})}>
              <span style={lbl("var(--c014)")}>{editingMGAdmin.salaIdExistente?"ASIGNAR MASTER GROWER A SALA SIN DUEÑO":"NUEVO MASTER GROWER + SALA NUEVA"}</span>
              <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                <div>
                  <div style={{...lbl("var(--c084)"),marginBottom:"6px"}}>NOMBRE</div>
                  <input value={editingMGAdmin.nombre} onChange={e=>setEditingMGAdmin({...editingMGAdmin,nombre:e.target.value})} style={inp()}/>
                </div>
                <div>
                  <div style={{...lbl("var(--c084)"),marginBottom:"6px"}}>EMAIL</div>
                ...
                {!editingMGAdmin.salaIdExistente && (
                  <div>
                    <div style={{...lbl("var(--c084)"),marginBottom:"6px"}}>NOMBRE DE LA SALA NUEVA</div>
                ...
                <div style={{display:"flex",gap:"10px"}}>
                  <button disabled={savingMGAdmin || !editingMGAdmin.nombre || !editingMGAdmin.email} onClick={()=>crearMasterGrowerAdmin(editingMGAdmin.salaIdExistente)} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"13px",fontWeight:"600",background:"var(--c061)",color:"#fff",border:"none"}}>{savingMGAdmin?"Guardando...":"✓ Guardar"}</button>
                  <button onClick={()=>setEditingMGAdmin(null)} style={{padding:"10px 16px",borderRadius:"8px",fontSize:"13px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                </div>
              </div>
            </div>
          ) : (
            <button onClick={()=>setEditingMGAdmin({nombre:"",email:"",salaNombre:"",salaIdExistente:null})} style={{width:"100%",padding:"14px",borderRadius:"10px",fontSize:"13px",fontWeight:"600",background:"var(--c106)",color:"var(--c014)",border:"2px dashed var(--c025)",marginBottom:"20px"}}>+ Nuevo Master Grower (Sala nueva)</button>
          )}

          <div style={{fontSize:"13px",color:"var(--c037)",marginBottom:"14px"}}>{salasTodas.length} sala{salasTodas.length!==1?"s":""}</div>

          <div style={card({background:"var(--c166)",borderColor:"var(--c107)"})}>
            <span style={lbl("var(--c081)")}>🏢 ENTIDADES ({todasEntidades.length})</span>
            <div style={{display:"flex",flexDirection:"column",gap:"8px",marginTop:"6px"}}>
              {todasEntidades.map(ent=>{
                ...
                return (
                  <div key={ent.id} style={{background:"var(--c108)",borderRadius:"10px",padding:"10px 12px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px"}}>
                      <span style={{fontSize:"13px",color:"var(--c167)",fontWeight:"600"}}>{ent.nombre}</span>
                      {puedeBorrar ? (
                        <button onClick={()=>setEliminandoEntidad({entidad:ent, confirmText:""})} style={{padding:"5px 10px",borderRadius:"7px",fontSize:"10px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>🗑 Eliminar</button>
                      ) : (
                        <span style={{fontSize:"9px",color:"var(--c017)"}}>no se puede borrar con salas activas</span>
                      )}
                    </div>
                    <div style={{fontSize:"10px",color:"var(--c037)",marginBottom:"8px"}}>{salasDeEsta} sala{salasDeEsta!==1?"s":""} · {usuariosDeEsta} usuario{usuariosDeEsta!==1?"s":""}</div>
                    <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                      <span style={{fontSize:"9px",color:"var(--c037)"}}>EXPIRA:</span>
                      <input type="date" value={ent.fecha_expiracion? ent.fecha_expiracion.slice(0,10):""} onChange={e=>actualizarExpiracionEntidad(ent.id, e.target.value)} style={{background:"var(--c016)",border:"1px solid var(--c005)",borderRadius:"6px",color:"var(--c013)",fontSize:"10px",padding:"4px 8px",colorScheme:"dark"}}/>
                      {ent.fecha_expiracion && <button onClick={()=>actualizarExpiracionEntidad(ent.id, null)} style={{fontSize:"9px",color:"var(--c037)",background:"transparent",border:"none"}}>quitar</button>}
                    </div>
                  </div>
                );
              })}
          ...
          {salasTodas.map(sala=>{
            ...
            return (
              <div key={sala.id} style={card({background:"var(--c168)",borderColor:"var(--c005)"})}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"10px"}}>
                  <div>
                    <div style={{fontSize:"16px",fontWeight:"600",color:"var(--c102)"}}>{sala.nombre}</div>
                    <div style={{fontSize:"12px",color:mg?"var(--c081)":"var(--c009)",marginTop:"3px"}}>{mg?"Master Grower: "+mg.nombre+(mg.activo===false?" (desactivado)":""):"⚠ Sin Master Grower asignado"}</div>
                  </div>
                  <button onClick={()=>setEliminandoSala({sala, confirmText:""})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>🗑 Eliminar sala</button>
                </div>

                {mg && (
                  <button onClick={()=>eliminarMasterGrowerAdmin(mg)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"var(--c109)",color:"var(--c003)",border:"1px solid var(--c169)",marginBottom:"10px"}}>⏸ Quitar como Master Grower</button>
                )}
                {!mg && (
                  <button onClick={()=>setEditingMGAdmin({nombre:"",email:"",salaNombre:"",salaIdExistente:sala.id})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"var(--c043)",color:"var(--c001)",border:"1px solid var(--c044)",marginBottom:"10px"}}>+ Asignar Master Grower</button>
                )}

                <div style={{fontSize:"11px",color:"var(--c060)",marginBottom:"8px"}}>{miembros.length} usuario{miembros.length!==1?"s":""} en esta sala</div>
                <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
                  {miembros.map(u=>(
                    <div key={u.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"var(--c108)",borderRadius:"8px",padding:"8px 12px",opacity:u.activo===false?0.5:1}}>
                      <span style={{fontSize:"12px",color:"var(--c170)"}}>{u.nombre} <span style={{color:"var(--c060)"}}>· {u.rol==="ejecutor"?"Ejecutor":"Master Grower"}{u.activo===false?" · desactivado":""}</span></span>
                      {u.activo===false ? (
                        <button onClick={()=>reactivarUsuarioAdmin(u)} style={{padding:"4px 9px",borderRadius:"6px",fontSize:"10px",background:"var(--c043)",color:"var(--c001)",border:"1px solid var(--c044)"}}>↺ Reactivar</button>
                      ) : (
                        <button onClick={()=>u.rol==="master_grower"?eliminarMasterGrowerAdmin(u):eliminarEjecutorAdmin(u)} style={{padding:"4px 9px",borderRadius:"6px",fontSize:"10px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>Desactivar</button>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {eliminandoEntidad && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"var(--c045)",border:"1px solid var(--c046)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"18px",color:"var(--c028)",marginBottom:"10px"}}>⚠ Eliminar "{eliminandoEntidad.entidad.nombre}"</div>
              <div style={{fontSize:"12px",color:"var(--c047)",marginBottom:"14px",lineHeight:"1.6"}}>Esto borra DEFINITIVAMENTE la entidad y su usuario administrador. No se puede deshacer.</div>
              <div style={{...lbl("var(--c038)"),marginBottom:"6px"}}>ESCRIBÍ "{eliminandoEntidad.entidad.nombre}" PARA CONFIRMAR</div>
              ...
              <div style={{display:"flex",gap:"10px"}}>
                <button disabled={borrandoEntidad || eliminandoEntidad.confirmText!==eliminandoEntidad.entidad.nombre} onClick={confirmarEliminarEntidad} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"var(--c048)",color:"#fff",border:"1px solid var(--c049)"}}>{borrandoEntidad?"Eliminando...":"🗑 Eliminar definitivamente"}</button>
                <button onClick={()=>setEliminandoEntidad(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
              </div>
            </div>
          </div>
        )}
        ...
        {eliminandoSala && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"var(--c045)",border:"1px solid var(--c046)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"18px",color:"var(--c028)",marginBottom:"10px"}}>⚠ Eliminar "{eliminandoSala.sala.nombre}"</div>
              <div style={{fontSize:"12px",color:"var(--c047)",marginBottom:"14px",lineHeight:"1.6"}}>Esto borra DEFINITIVAMENTE la sala y todo su contenido: sectores, lotes, genéticas, tareas, comentarios, relevamientos y los usuarios que pertenecen a ella. No se puede deshacer.</div>
              <div style={{...lbl("var(--c038)"),marginBottom:"6px"}}>ESCRIBÍ "{eliminandoSala.sala.nombre}" PARA CONFIRMAR</div>
              ...
              <div style={{display:"flex",gap:"10px"}}>
                <button disabled={borrandoSala || eliminandoSala.confirmText!==eliminandoSala.sala.nombre} onClick={confirmarEliminarSala} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"var(--c048)",color:"#fff",border:"1px solid var(--c049)"}}>{borrandoSala?"Eliminando...":"🗑 Eliminar definitivamente"}</button>
                <button onClick={()=>setEliminandoSala(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
              </div>
            </div>
          </div>
        )}
  ...
  const tabsExtra = [
    {key:"_tareas",        label:"📋 Tareas",        col:"var(--c015)"},
    {key:"_calendario",    label:"📅 Calendario",    col:"var(--c063)"},
    {key:"_asistente",     label:"🤖 Asistente",     col:"var(--c055)"},
    {key:"_relevamientos", label:"🔍 Relevamientos", col:"var(--c029)"},
    {key:"_bajas",         label:"📉 Bajas",         col:"var(--c009)"}
  ];
  ...
  if(esEjecutor) tabsExtra.splice(3, 0, {key:"_geneticas", label:"🧬 Genéticas", col:"var(--c098)"});
  if(!esEjecutor) tabsExtra.push({key:"_cosechas", label:"📦 Cosechas", col:"var(--c003)"});
  if(!esEjecutor) tabsExtra.push({key:"_reportes", label:"📊 Reportes", col:"var(--c064)"});
  if(!esEjecutor) tabsExtra.push({key:"_misala", label:"⚙️ Mi Sala", col:"var(--c110)"});
  tabsExtra.push({key:"_apariencia", label:"🎨 Apariencia", col:"var(--c001)"});

  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"var(--c073)",minHeight:"100vh",color:"var(--c018)"}}>
      ...
      <style>{"*{box-sizing:border-box;margin:0;padding:0;}button{cursor:pointer;transition:all .15s ease;font-family:'IBM Plex Mono',monospace;}textarea,input,select{outline:none;font-family:'IBM Plex Mono',monospace;}textarea:focus,input:focus,select:focus{border-color:var(--c171)!important;}.fade{animation:fi .22s ease;}@keyframes fi{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:none}}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:var(--c024);border-radius:2px}"}</style>

      <div style={{background:"var(--c054)",borderBottom:"2px solid var(--c024)",padding:"18px 20px 16px",position:"sticky",top:0,zIndex:100}}>
        {usuario.rol==="administrador" && adminEntidadModo==="mg" && (
          <button onClick={volverModoEntidad} style={{width:"100%",marginBottom:"12px",padding:"8px",borderRadius:"9px",fontSize:"12px",background:"var(--c012)",color:"var(--c014)",border:"1px solid var(--c005)",textAlign:"left"}}>← Volver al panel de {entidadNombre}</button>
        )}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c001)"}}>🌱 RAÍZ — Cuaderno de Cultivo</div>
            <div style={{fontSize:"11px",color:"var(--c172)",letterSpacing:"2px",marginTop:"4px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"6px"}}>
            {status==="guardado" && <div style={{fontSize:"12px",color:"var(--c001)",background:"var(--c173)",padding:"5px 14px",borderRadius:"20px",border:"1px solid var(--c174)"}}>✓ guardado</div>}
            {status && status!=="guardado" && <div style={{fontSize:"11px",color:"var(--c079)",background:"var(--c007)",padding:"5px 10px",borderRadius:"10px",border:"1px solid var(--c046)"}}>⚠ {status}</div>}
            <button onClick={handleLogout} style={{fontSize:"11px",color:"var(--c026)",background:"transparent",border:"1px solid var(--c024)",borderRadius:"14px",padding:"4px 12px"}}>salir</button>
          </div>
        </div>
      </div>
      ...
      <div style={{padding:"20px 20px 110px"}}>
        ...
        {sectoresPadre.find(sp=>sp.id===tab) && (()=>{
          ...
          return (
            <div className="fade">
              ...
              <div style={{fontSize:"12px",color:"var(--c026)",marginBottom:"18px"}}>{subs.length} subsector{subs.length!==1?"es":""}</div>

              {subs.length===0 && (
                <div style={{textAlign:"center",padding:"40px 20px",color:"var(--c111)",fontSize:"14px"}}>Sin subsectores configurados aún{!esEjecutor && " — creá uno en ⚙️ Mi Sala"}</div>
              )}
              ...
              {subs.map(sub=>{
                ...
                return (
                  <div key={sub.id} style={card(en1212 ? {borderColor:"var(--c175)", background:"var(--c176)"} : {borderColor:colorDeFase(sp.orden)+"55"})}>
                    <div onClick={()=>toggleSub(sub.id)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",alignItems:"center",gap:"8px",flexWrap:"wrap"}}>
                          <span style={{fontSize:"16px",fontWeight:"600",color:"var(--c018)"}}>{sub.nombre}</span>
                          {sub.capacidad_unidades && <span style={{fontSize:"11px",color:"var(--c026)"}}>cap. {sub.capacidad_unidades}</span>}
                          {en1212 && <span style={{fontSize:"10px",color:"var(--c177)",background:"var(--c112)",padding:"2px 8px",borderRadius:"9px",fontWeight:"600"}}>☀ 12/12</span>}
                          {diasParaCorte!==null && (
                            <span style={{fontSize:"10px",color:diasParaCorte<0?"var(--c082)":"var(--c003)",background:diasParaCorte<0?"var(--c113)":"var(--c178)",padding:"2px 8px",borderRadius:"9px",fontWeight:"600"}}>
                        ...
                        {lotesMaduracion.length>0 && <div style={{fontSize:"11px",color:"var(--c085)",marginTop:"6px"}}>🫙 {lotesMaduracion.length} en maduración</div>}
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:"10px",flexShrink:0,marginLeft:"10px"}}>
                        <span style={{fontSize:"22px",fontWeight:"700",color:totalPlantas>0?"var(--c001)":"var(--c050)"}}>{totalPlantas}</span>
                        <span style={{fontSize:"13px",color:"var(--c026)",display:"inline-block",transform:abierta?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                      </div>
                    </div>
                    ...
                    {sp.nombre==="Floración" && !esEjecutor && (
                      <div style={{background:"var(--c114)",borderRadius:"9px",padding:"10px 12px",marginBottom:"14px",border:"1px solid var(--c030)"}}>
                        <div style={{fontSize:"10px",color:"var(--c051)",letterSpacing:"0.5px",marginBottom:"6px"}}>🌸 12/12 ACTIVADO EN ESTA CARPA DESDE</div>
                        <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                          ...
                          <button onClick={()=>guardarFechaFloracion(sub)} disabled={savingFloracion} style={{padding:"6px 12px",borderRadius:"6px",fontSize:"11px",background:"var(--c030)",color:"var(--c179)",border:"1px solid var(--c115)"}}>✓</button>
                        </div>
                        {!sub.fecha_inicio_floracion && <div style={{fontSize:"10px",color:"var(--c039)",marginTop:"6px"}}>Sin activar — cargá el día que esta carpa pasa a 12/12. Aplica a todos los lotes que tenga adentro.</div>}
                        {sub.fecha_inicio_floracion && <div style={{fontSize:"10px",color:"var(--c039)",marginTop:"6px"}}>Se congela en cada lote al cortar, y esta fecha se resetea sola cuando la carpa quede vacía para el próximo ciclo.</div>}
                      </div>
                    )}
                    {lotesSub.length>0 && (
                      <div style={{display:"flex",flexDirection:"column",gap:"8px",marginTop:"10px",marginBottom:"10px"}}>
                        {lotesSub.map(l=>{
                          const g = geneticas.find(x=>x.id===l.genetica_id);
                          return (
                            <div key={l.id} style={{background:"var(--c054)",borderRadius:"8px",padding:"10px 12px",border:`1px solid ${g?.color||"var(--c024)"}44`}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px"}}>
                                <span style={{fontSize:"13px",color:g?.color||"var(--c086)"}}>{g?.nombre||"Sin genética"}{l.numero_lote?` · ${loteLabel(l.numero_lote, sufijoDeLote(l))}`:""}</span>
                                <span style={{fontSize:"13px",color:"var(--c180)"}}>{l.cantidad_plantas} plantas</span>
                              </div>
                              {sp.nombre!=="Floración" && (
                                <div style={{display:"flex",alignItems:"baseline",gap:"7px",marginBottom:"6px",flexWrap:"wrap"}}>
                                  <span style={{fontSize:"12px",color:"var(--c065)",fontWeight:"600"}}>{Math.max(0,Math.round((new Date()-new Date(l.fecha_ultima_movida||l.fecha_inicio))/86400000))}d en {sp.nombre.toLowerCase()}</span>
                                  <span style={{fontSize:"9px",color:"var(--c181)"}}>desde {l.fecha_ultima_movida||l.fecha_inicio}</span>
                                </div>
                              )}
                              {sp.nombre==="Floración" && (()=>{
                                ...
                                return (
                                  <div style={{display:"flex",alignItems:"baseline",gap:"9px",marginBottom:"6px",flexWrap:"wrap"}}>
                                    {corteTxt && <span style={{fontSize:"12px",color:corteAtrasado?"var(--c082)":"var(--c003)",fontWeight:"600"}}>{corteTxt}</span>}
                                    <span style={{fontSize:"11px",color:"var(--c065)"}}>🌱 {st?st.dias_vegetativo:"—"}d veg</span>
                                    {sub.fecha_inicio_floracion && <span style={{fontSize:"11px",color:"var(--c051)"}}>🌸 {st?st.dias_floracion:"—"}d flo</span>}
                                  </div>
                                );
                              })()}
                              {sp.nombre==="Floración" && sub.fecha_inicio_floracion && (
                                <div style={{fontSize:"11px",color:"var(--c051)",marginBottom:"6px"}}>🌸 En 12/12 desde {sub.fecha_inicio_floracion}</div>
                              )}
                              {sp.nombre==="Floración" && !sub.fecha_inicio_floracion && !esEjecutor && (
                                <div style={{fontSize:"11px",color:"var(--c039)",marginBottom:"6px"}}>🌸 Carpa todavía sin 12/12 activado — cargalo arriba, en el encabezado de {sub.nombre}.</div>
                              )}
                              <div style={{display:"flex",gap:"6px"}}>
                                {!esEjecutor && (
                                  <button onClick={()=>setEditingLote({...l, cantidad_plantas:String(l.cantidad_plantas)})} style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>✏️</button>
                                )}
                                {!esEjecutor && (
                                  <button onClick={()=>setBajaLote({lote:l, cantidad:"1", motivo:""})} style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c007)",color:"var(--c031)",border:"1px solid var(--c008)"}}>🥀</button>
                                )}
                                {!esEjecutor && sigPadre && (
                                  (sp.nombre==="Floración" && !sub.fecha_inicio_floracion) ? (
                                    <button disabled title="Activá el 12/12 de esta carpa antes de poder cortar" style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"var(--c004)",color:"var(--c116)",border:"1px solid var(--c002)",cursor:"not-allowed"}}>→ Avanzar a {sigPadre.nombre} (activá 12/12 primero)</button>
                                  ) : (sp.nombre==="Vegetativo" && sub.puede_avanzar_etapa===false) ? (
                                    <button disabled title="Este tamaño de maceta no está habilitado para pasar a Floración (lo configurás en Mi Sala)" style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"var(--c004)",color:"var(--c116)",border:"1px solid var(--c002)",cursor:"not-allowed"}}>→ Avanzar a {sigPadre.nombre} (maceta no habilitada)</button>
                                  ) : (
                                ...
                                {!esEjecutor && !sigPadre && (
                                  <button onClick={()=>pasarAMaduracion(l)} style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"var(--c182)",color:"var(--c085)",border:"1px solid var(--c117)"}}>🫙 Pasar a Maduración</button>
                                )}
                                {!esEjecutor && !(sp.nombre==="Floración" && sub.fecha_inicio_floracion) && (
                                  <button onClick={()=>{setMovingLote(l); setDestinoSub(""); setMovingMovida(String(l.cantidad_plantas)); setModoMover(true); setModoRetroceso(false);}} title="Mover a otro subsector (corrección)" style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c183)",color:"var(--c118)",border:"1px solid var(--c107)"}}>↔</button>
                                )}
                                {!esEjecutor && sp.nombre==="Floración" && !sub.fecha_inicio_floracion && (
                                  <button onClick={()=>iniciarRetroceso(l)} title="Todavía no entró en 12/12 — volver a Vegetativo (5L)" style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c184)",color:"var(--c087)",border:"1px solid var(--c185)"}}>↩ 5L</button>
                                )}
                              </div>
                              {sp.nombre==="Floración" && sub.fecha_inicio_floracion && (
                                <div style={{fontSize:"10px",color:"var(--c039)",marginTop:"6px"}}>🔒 En 12/12 — no se puede mover hasta el corte (avanzar a Secado).</div>
                              )}
                            </div>
                          );
                        })}
                    ...
                    {lotesMaduracion.length>0 && (
                      <div style={{marginTop:"4px",marginBottom:"10px"}}>
                        <div style={{fontSize:"11px",color:"var(--c085)",marginBottom:"6px",letterSpacing:"0.5px"}}>🫙 EN MADURACIÓN</div>
                        <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                          {lotesMaduracion.map(l=>{
                            const g = geneticas.find(x=>x.id===l.genetica_id);
                            return (
                              <div key={l.id} style={{background:"var(--c119)",borderRadius:"8px",padding:"10px 12px",border:`1px solid var(--c186)`}}>
                                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
                                  <span style={{fontSize:"13px",color:g?.color||"var(--c086)"}}>{g?.nombre||"Sin genética"}{l.numero_lote?` · ${loteLabel(l.numero_lote, sufijoDeLote(l))}`:""}</span>
                                  <span style={{fontSize:"13px",color:"var(--c187)"}}>{l.cantidad_plantas} plantas</span>
                                </div>
                                <div style={{display:"flex",gap:"6px"}}>
                                  {!esEjecutor && (
                                    <button onClick={()=>setBajaLote({lote:l, cantidad:"1", motivo:""})} style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c007)",color:"var(--c031)",border:"1px solid var(--c008)"}}>🥀</button>
                                  ...
                                  {!esEjecutor && (
                                    <button onClick={()=>setFinalizandoLote({lote:l, peso:"", dias_secado:String(l.dias_secado||""), notas:l.notas||""})} style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"var(--c188)",color:"var(--c057)",border:"1px solid var(--c189)"}}>⚖️ Finalizar y pesar</button>
                                  )}
                                </div>
                              </div>
                            );
                    ...
                    {!esEjecutor && (
                      <div style={{display:"flex",gap:"8px"}}>
                        {(sp.nombre==="Enraizado" || sp.nombre==="Vegetativo") && (
                          <button onClick={()=>setEditingLote(emptyLote(sub.id))} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"var(--c190)",color:"var(--c001)",border:"1px dashed var(--c010)"}}>+ Planta/lote</button>
                        )}
                        <button onClick={()=>setEditingTarea(emptyTarea(sub.id))} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"var(--c088)",color:"var(--c015)",border:"1px dashed var(--c027)"}}>+ Tarea</button>
                      </div>
                    )}

                    {tareasDelSub.length>0 && (
                      <div style={{marginTop:"14px"}}>
                        <div onClick={()=>setTareasEnSubAbierto(prev=>({...prev,[sub.id]:!prev[sub.id]}))} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 12px",background:tareasEnSubAbierto[sub.id]?"var(--c191)":"var(--c120)",borderRadius:"9px",border:"1px solid var(--c192)"}}>
                          <span style={{fontSize:"12px",color:"var(--c015)"}}>📋 Tareas planificadas aquí <span style={{color:"var(--c032)"}}>· {tareasDelSub.length}</span></span>
                          <span style={{fontSize:"11px",color:"var(--c015)",transform:tareasEnSubAbierto[sub.id]?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                        </div>
                        {tareasEnSubAbierto[sub.id] && (
                          <div style={{display:"flex",flexDirection:"column",gap:"6px",marginTop:"8px"}}>
                            {tareasDelSub.map(t=>{
                              ...
                              return (
                                <div key={t.id} style={{background:"var(--c075)",borderRadius:"8px",padding:"9px 11px",borderLeft:`3px solid ${TAREA_PRIORIDAD_COLOR[t.prioridad]||"var(--c056)"}`}}>
                                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                                    <span style={{fontSize:"12px",color:"var(--c193)"}}>{t.titulo}</span>
                                  ...
                                  <div style={{fontSize:"10px",color:"var(--c036)",marginTop:"3px"}}>📅 {t.fecha_programada}{t.asignado_a?` · → ${usuariosMap[t.asignado_a]||"—"}`:""}</div>
                                </div>
                              );
                            })}
                          </div>
        ...
        {tab==="_tareas" && (()=>{
          ...
          return (
            <div className="fade">
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c015)",marginBottom:"6px"}}>📋 Tareas</div>
              <div style={{fontSize:"13px",color:"var(--c032)",marginBottom:"18px"}}>{esEjecutor?"TUS TAREAS ASIGNADAS":"TODAS LAS TAREAS"}</div>

              {!esEjecutor && (
                <button onClick={()=>setEditingTarea(emptyTarea())} style={{width:"100%",padding:"14px",borderRadius:"12px",fontSize:"14px",fontWeight:"600",background:"var(--c088)",color:"var(--c015)",border:"2px dashed var(--c027)",marginBottom:"20px"}}>+ Nueva tarea</button>
              )}
              ...
              <div style={{display:"flex",gap:"8px",marginBottom:"12px",flexWrap:"wrap"}}>
                {[["todas","Todas"],["pendiente","Pendientes"],["en_progreso","En progreso"],["completada","Completadas"],["atrasada","Atrasadas"]].map(([k,label])=>(
                  <button key={k} onClick={()=>setTFiltro(k)} style={{padding:"7px 14px",borderRadius:"18px",fontSize:"12px",background:tFiltro===k?"var(--c027)":"var(--c121)",color:tFiltro===k?"#fff":"var(--c076)",border:`1px solid ${tFiltro===k?"var(--c015)":"var(--c122)"}`}}>{label}</button>
                ))}
              </div>

              <div style={{background:"var(--c089)",borderRadius:"12px",padding:"12px 14px",marginBottom:"18px",border:"1px solid var(--c052)"}}>
                <div style={{fontSize:"11px",color:"var(--c032)",letterSpacing:"1px",marginBottom:"10px"}}>FILTRAR POR FECHA</div>
                <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
                  {[["ninguno","Sin filtro"],["exacta","Fecha exacta"],["rango","Rango"]].map(([m,l])=>(
                    <button key={m} onClick={()=>setTModoFecha(m)} style={{flex:1,padding:"7px 6px",borderRadius:"9px",fontSize:"11px",background:tModoFecha===m?"var(--c194)":"var(--c089)",color:tModoFecha===m?"#fff":"var(--c036)",border:`1px solid ${tModoFecha===m?"var(--c015)":"var(--c052)"}`}}>{l}</button>
                  ))}
                </div>
                {tModoFecha==="exacta" && (
                  <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                    <input type="date" value={tFechaExacta} onChange={e=>setTFechaExacta(e.target.value)} style={{...inp(),flex:1}}/>
                    {tFechaExacta && <button onClick={()=>setTFechaExacta("")} style={{padding:"7px 10px",borderRadius:"8px",fontSize:"11px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>✕</button>}
                  </div>
                )}
                {tModoFecha==="rango" && (
                  <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                      <span style={{fontSize:"11px",color:"var(--c032)",width:"36px"}}>Desde</span>
                      <input type="date" value={tFechaDesde} onChange={e=>setTFechaDesde(e.target.value)} style={{...inp(),flex:1}}/>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                      <span style={{fontSize:"11px",color:"var(--c032)",width:"36px"}}>Hasta</span>
                      <input type="date" value={tFechaHasta} onChange={e=>setTFechaHasta(e.target.value)} style={{...inp(),flex:1}}/>
                    </div>
                    {(tFechaDesde||tFechaHasta) && <button onClick={()=>{setTFechaDesde("");setTFechaHasta("");}} style={{padding:"6px",borderRadius:"8px",fontSize:"11px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>✕ Limpiar fechas</button>}
                  </div>
                )}
              </div>

              {tareasFiltradas.length===0 ? (
                <div style={{textAlign:"center",padding:"40px 20px",color:"var(--c195)",fontSize:"14px"}}>Sin tareas{tFiltro!=="todas"?" en este filtro":""}</div>
              ) : (
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                    ...
                    return (
                      <div key={grupo.key} style={{border:`1px solid ${grupo.color}44`,borderRadius:"14px",overflow:"hidden"}}>
                        <div onClick={()=>setGrupoTareasAbiertos(prev=>({...prev,[grupo.key]:!prev[grupo.key]}))} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:abierto?`${grupo.color}14`:"transparent"}}>
                          <span style={{fontSize:"13px",fontWeight:"600",color:grupo.color}}>{grupo.label} <span style={{fontSize:"11px",color:"var(--c076)",fontWeight:"400"}}>· {grupo.tareas.length}</span></span>
        ...
        {tab==="_asistente" && (()=>{
          ...
          return (
            <div className="fade" style={{display:"flex",flexDirection:"column",height:"calc(100vh - 180px)"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c055)",marginBottom:"4px"}}>🤖 Asistente IA</div>
              <div style={{fontSize:"12px",color:"var(--c196)",marginBottom:"14px"}}>Hacé preguntas o subí una foto para diagnóstico</div>

              <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:"10px",paddingBottom:"8px"}}>
                {chatMensajes.length===0 && (
                  <div style={{textAlign:"center",padding:"40px 20px",color:"var(--c123)",fontSize:"13px",lineHeight:"1.7"}}>
                    <div style={{fontSize:"40px",marginBottom:"12px"}}>🌿</div>
                    <div>Podés preguntarme sobre deficiencias, plagas, nutrición, tiempos de floración...</div>
                    <div style={{marginTop:"10px",color:"var(--c066)"}}>También podés adjuntar una foto para que la analice.</div>
                  </div>
                )}
                {chatMensajes.map((m,i)=>(
                  <div key={i} style={{display:"flex",flexDirection:"column",alignItems:m.rol==="usuario"?"flex-end":"flex-start"}}>
                    {m.imagen && (
                      <img src={`data:image/jpeg;base64,${m.imagen}`} alt="foto" style={{maxWidth:"70%",borderRadius:"12px",marginBottom:"4px",border:"1px solid var(--c030)"}}/>
                    )}
                    {m.texto && (
                      <div style={{
                        maxWidth:"85%",padding:"12px 14px",borderRadius:"14px",fontSize:"13px",lineHeight:"1.6",whiteSpace:"pre-wrap",
                        background:m.rol==="usuario"?"var(--c124)":m.rol==="error"?"var(--c113)":"var(--c125)",
                        color:m.rol==="usuario"?"var(--c197)":m.rol==="error"?"var(--c198)":"var(--c090)",
                        borderBottomRightRadius:m.rol==="usuario"?"4px":"14px",
                        borderBottomLeftRadius:m.rol==="usuario"?"14px":"4px",
                        border:m.rol==="asistente"?"1px solid var(--c052)":"none"
                      }}>{m.texto}</div>
                    )}
                    <div style={{fontSize:"10px",color:"var(--c123)",marginTop:"3px"}}>{m.ts}</div>
                  </div>
                ))}
                {chatCargando && (
                  <div style={{display:"flex",alignItems:"flex-start"}}>
                    <div style={{background:"var(--c125)",border:"1px solid var(--c052)",borderRadius:"14px",borderBottomLeftRadius:"4px",padding:"12px 16px",color:"var(--c199)",fontSize:"13px"}}>Analizando...</div>
                  </div>
                )}
                <div ref={chatBottomRef}/>
              </div>

              <div style={{borderTop:"1px solid var(--c124)",paddingTop:"12px",marginTop:"8px"}}>
                {chatImagen && (
                  <div style={{position:"relative",display:"inline-block",marginBottom:"8px"}}>
                    <img src={`data:image/jpeg;base64,${chatImagen}`} alt="preview" style={{height:"70px",borderRadius:"8px",border:"1px solid var(--c066)"}}/>
                    <button onClick={()=>setChatImagen(null)} style={{position:"absolute",top:"-6px",right:"-6px",width:"20px",height:"20px",borderRadius:"50%",background:"var(--c009)",color:"#fff",border:"none",fontSize:"11px",lineHeight:"20px",textAlign:"center",padding:0}}>✕</button>
                  </div>
                )}
                <div style={{display:"flex",gap:"8px",alignItems:"flex-end"}}>
                  <input type="file" accept="image/*" ref={fileInputRef} onChange={seleccionarImagen} style={{display:"none"}}/>
                  <button onClick={()=>fileInputRef.current?.click()} style={{padding:"10px",borderRadius:"10px",background:"var(--c126)",color:"var(--c055)",border:"1px solid var(--c200)",fontSize:"18px",flexShrink:0}}>📷</button>
                  ...
                  <button disabled={chatCargando||(!chatInput.trim()&&!chatImagen)} onClick={enviarMensaje} style={{padding:"10px 14px",borderRadius:"10px",background:chatCargando?"var(--c126)":"var(--c201)",color:"#fff",border:"none",fontSize:"16px",flexShrink:0}}>➤</button>
                </div>
                <div style={{fontSize:"10px",color:"var(--c066)",marginTop:"6px",textAlign:"center"}}>Enter para enviar · Shift+Enter para nueva línea</div>
              </div>
            </div>
          );
        })()}
        ...
        {tab==="_calendario" && (()=>{
          ...
          return (
            <div className="fade">
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c063)",marginBottom:"6px"}}>📅 Calendario</div>
              <div style={{fontSize:"13px",color:"var(--c056)",marginBottom:"18px"}}>{esEjecutor?"TUS TAREAS POR FECHA":"TODAS LAS TAREAS POR FECHA"}</div>

              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"14px"}}>
                <button onClick={()=>setCalMes(m=>{ const d=new Date(m.year,m.month-1,1); return {year:d.getFullYear(),month:d.getMonth()}; })} style={{padding:"8px 14px",borderRadius:"8px",background:"var(--c127)",color:"var(--c064)",border:"1px solid var(--c091)"}}>‹</button>
                <div style={{fontSize:"15px",color:"var(--c202)"}}>{nombreMes.charAt(0).toUpperCase()+nombreMes.slice(1)}</div>
                <button onClick={()=>setCalMes(m=>{ const d=new Date(m.year,m.month+1,1); return {year:d.getFullYear(),month:d.getMonth()}; })} style={{padding:"8px 14px",borderRadius:"8px",background:"var(--c127)",color:"var(--c064)",border:"1px solid var(--c091)"}}>›</button>
              </div>

              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"4px",marginBottom:"6px"}}>
                {["L","M","X","J","V","S","D"].map(d=><div key={d} style={{textAlign:"center",fontSize:"11px",color:"var(--c203)"}}>{d}</div>)}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"4px",marginBottom:"18px"}}>
                {celdas.map((fecha,idx)=>{
                  ...
                  const dotColor = estado==="atrasada"?"var(--c009)":estado==="completada"?"var(--c035)":estado==="pendiente"?"var(--c003)":null;
                  return (
                    <button key={fecha} onClick={()=>setCalDia(fecha)} style={{
                      ...
                      background:esSeleccionado?"var(--c091)":esHoy?"var(--c204)":"var(--c205)",
                      color:esHoy?"#fff":"#9ab",
                      border:esSeleccionado?"1px solid var(--c063)":"1px solid var(--c206)"
                    }}>
              ...
              <div style={{fontSize:"13px",color:"var(--c207)",marginBottom:"10px"}}>{calDia}{calDia===hoy?" · hoy":""}</div>
              {tareasDelDia.length===0 ? (
                <div style={{textAlign:"center",padding:"30px 20px",color:"var(--c208)",fontSize:"13px"}}>Sin tareas este día</div>
              ) : (
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                    ...
                    return (
                      <div key={grupo.key} style={{border:`1px solid ${grupo.color}44`,borderRadius:"14px",overflow:"hidden"}}>
                        <div onClick={()=>setGrupoTareasCalAbiertos(prev=>({...prev,[grupo.key]:!prev[grupo.key]}))} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",background:abierto?`${grupo.color}14`:"transparent"}}>
                          <span style={{fontSize:"13px",fontWeight:"600",color:grupo.color}}>{grupo.label} <span style={{fontSize:"11px",color:"var(--c209)",fontWeight:"400"}}>· {grupo.tareas.length}</span></span>
                          <span style={{fontSize:"13px",color:grupo.color,transform:abierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                        </div>
                        {abierto && (
                          <div style={{padding:"4px 14px 14px",display:"flex",flexDirection:"column",gap:"10px"}}>
        ...
        {tab==="_relevamientos" && (()=>{
          ...
          return (
            <div className="fade">
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c029)",marginBottom:"6px"}}>🔍 Relevamientos</div>
              <div style={{fontSize:"13px",color:"var(--c067)",marginBottom:"18px"}}>CONTEO REAL DE PLANTAS POR SUBSECTOR</div>

              {!puedeRelevar && <div style={{fontSize:"12px",color:"var(--c021)",background:"var(--c092)",borderRadius:"8px",padding:"10px 14px",marginBottom:"16px"}}>No tenés permiso de relevamiento — pedile al Master Grower que te lo active en ⚙️ Mi Sala. Podés ver el historial igual.</div>}

              {puedeRelevar && (
                editingRelevamiento ? (
                  <div style={card({borderColor:"var(--c093)",background:"var(--c210)"})}>
                    <span style={lbl("var(--c029)")}>NUEVO RELEVAMIENTO</span>
                    <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                      <div>
                        <div style={{...lbl("var(--c068)"),marginBottom:"6px"}}>SUBSECTOR</div>
                      ...
                      <div>
                        <div style={{...lbl("var(--c068)"),marginBottom:"6px"}}>GENÉTICAS CONTADAS</div>
                        {editingRelevamiento.lineas.map((linea,idx)=>(
                          <div key={idx} style={{display:"flex",gap:"6px",marginBottom:"8px"}}>
                            ...
                            {editingRelevamiento.lineas.length>1 && (
                              <button onClick={()=>setEditingRelevamiento({...editingRelevamiento,lineas:editingRelevamiento.lineas.filter((_,i)=>i!==idx)})} style={{padding:"0 12px",borderRadius:"8px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>✕</button>
                            )}
                          </div>
                        ))}
                        <button onClick={()=>setEditingRelevamiento({...editingRelevamiento,lineas:[...editingRelevamiento.lineas,{genetica_id:"",cantidad:""}]})} style={{fontSize:"12px",color:"var(--c068)",background:"none",border:"1px dashed var(--c211)",borderRadius:"8px",padding:"7px 12px",width:"100%"}}>+ Agregar otra genética</button>
                        <div style={{fontSize:"11px",color:"var(--c036)",marginTop:"6px"}}>Solo genéticas ya cargadas — para sumar una genética nueva al catálogo, eso lo hace el Master Grower en 🧬 Genéticas. Si encontrás una que no está en este subsector, igual se crea el lote como "pendiente de aprobación".</div>
                      </div>
                      <div>
                        <div style={{...lbl("var(--c068)"),marginBottom:"6px"}}>NOTAS</div>
                      ...
                      <div style={{display:"flex",gap:"10px"}}>
                        <button disabled={savingRelevamiento || !editingRelevamiento.sector_id || editingRelevamiento.lineas.every(l=>!l.genetica_id || l.cantidad==="")} onClick={guardarRelevamiento} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:savingRelevamiento?"var(--c212)":"var(--c093)",color:"#fff",border:"1px solid var(--c029)"}}>{savingRelevamiento?"Guardando...":"✓ Guardar"}</button>
                        <button onClick={()=>setEditingRelevamiento(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                      </div>
                    </div>
                  </div>
                ) : (
                  <button onClick={()=>setEditingRelevamiento(emptyRelevamiento())} style={{width:"100%",padding:"14px",borderRadius:"12px",fontSize:"14px",fontWeight:"600",background:"var(--c213)",color:"var(--c029)",border:"2px dashed var(--c093)",marginBottom:"20px"}}>+ Nuevo relevamiento</button>
                )
              )}

              {relevamientos.length===0 ? (
                <div style={{textAlign:"center",padding:"40px 20px",color:"var(--c214)",fontSize:"14px"}}>Sin relevamientos registrados</div>
              ) : (()=>{
                const porDia = {};
                ...
                return (
                  <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                    {dias.map(dia=>{
                      ...
                      return (
                        <div key={dia} style={{border:"1px solid var(--c215)",borderRadius:"14px",overflow:"hidden"}}>
                          <div onClick={()=>setDiaRelevamientoAbierto(abierto?"":dia)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:abierto?"var(--c216)":"transparent"}}>
                            <div>
                              <div style={{fontSize:"14px",color:"var(--c059)",fontWeight:"600"}}>{dia}</div>
                              <div style={{fontSize:"11px",color:"var(--c067)",marginTop:"3px"}}>{items.length} registro{items.length!==1?"s":""}{conDiferencia>0 && <span style={{color:"var(--c009)"}}> · {conDiferencia} con diferencia</span>}</div>
                            </div>
                            <span style={{fontSize:"13px",color:"var(--c029)",transform:abierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                          </div>
                          {abierto && (
                            <div style={{padding:"4px 14px 14px",display:"flex",flexDirection:"column",gap:"10px"}}>
                              {items.map(r=>{
                                ...
                                return (
                                  <div key={r.id} style={{background:"var(--c075)",borderRadius:"10px",padding:"12px 14px",borderLeft:`4px solid ${diff===0?"var(--c035)":"var(--c009)"}`}}>
                                    <div style={{fontSize:"13px",color:"var(--c059)",marginBottom:"6px"}}>{sp?.nombre||"—"}{sub?` › ${sub.nombre}`:""}{g?` · ${g.nombre}`:""}</div>
                                    <div style={{display:"flex",gap:"16px",marginBottom:"6px"}}>
                                      <div><span style={{fontSize:"11px",color:"var(--c067)"}}>Contado: </span><span style={{fontSize:"14px",fontWeight:"700",color:"var(--c029)"}}>{r.cantidad_contada}</span></div>
                                      <div><span style={{fontSize:"11px",color:"var(--c067)"}}>Sistema: </span><span style={{fontSize:"14px",color:"#9ab"}}>{sistema}</span></div>
                                      {diff!==0 && <div><span style={{fontSize:"11px",color:"var(--c009)"}}>Diferencia: </span><span style={{fontSize:"14px",fontWeight:"700",color:"var(--c009)"}}>{diff>0?"+":""}{diff}</span></div>}
                                    </div>
                                    {r.notas && <div style={{fontSize:"12px",color:"var(--c217)",marginBottom:"6px"}}>{r.notas}</div>}
                                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                                      <div style={{fontSize:"10px",color:"var(--c036)"}}>{usuariosMap[r.usuario_id]||"—"}</div>
                                      {!esEjecutor && (
                                        <button onClick={()=>delRelevamiento(r.id)} style={{padding:"4px 10px",borderRadius:"6px",fontSize:"10px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>🗑</button>
                                      )}
                                    </div>
                                  </div>
                                );
        ...
        {tab==="_geneticas" && (
          <div className="fade">
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c033)",marginBottom:"6px"}}>🧬 Genéticas y Fenotipos</div>
            <div style={{fontSize:"13px",color:"var(--c022)",marginBottom:"18px"}}>{geneticas.length} ENTRADAS</div>

            {!esEjecutor && (
              editingGenetica ? (
                <div style={card({borderColor:"var(--c069)",background:"var(--c218)"})}>
                  <span style={lbl("var(--c033)")}>{editingGenetica.id?"EDITAR GENÉTICA":"NUEVA GENÉTICA"}</span>
                  <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                    <div>
                      <div style={{...lbl("var(--c022)"),marginBottom:"6px"}}>NOMBRE</div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
                      <div>
                        <div style={{...lbl("var(--c022)"),marginBottom:"6px"}}>TIPO</div>
                      ...
                      <div>
                        <div style={{...lbl("var(--c022)"),marginBottom:"6px"}}>BANCO</div>
                        <input value={editingGenetica.banco} onChange={e=>setEditingGenetica({...editingGenetica,banco:e.target.value})} style={inp()}/>
                      </div>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
                      <div>
                        <div style={{...lbl("var(--c022)"),marginBottom:"6px"}}>DÍAS FLORACIÓN MÍN</div>
                      ...
                      <div>
                        <div style={{...lbl("var(--c022)"),marginBottom:"6px"}}>DÍAS FLORACIÓN MÁX</div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr",gap:"12px"}}>
                      <div>
                        <div style={{...lbl("var(--c022)"),marginBottom:"6px"}}>COLOR</div>
                    ...
                    <div style={{display:"flex",gap:"10px"}}>
                      <button disabled={savingGenetica || !editingGenetica.nombre} onClick={saveGenetica} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:savingGenetica?"var(--c219)":"var(--c220)",color:"#fff",border:"1px solid var(--c221)"}}>{savingGenetica?"Guardando...":"✓ Guardar"}</button>
                      <button onClick={()=>setEditingGenetica(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                    </div>
                  </div>
                </div>
              ) : (
                <button onClick={()=>setEditingGenetica(emptyGenetica())} style={{width:"100%",padding:"14px",borderRadius:"12px",fontSize:"14px",fontWeight:"600",background:"var(--c222)",color:"var(--c033)",border:"2px dashed var(--c069)",marginBottom:"20px"}}>+ Nueva genética</button>
              )
            )}

            <div style={{display:"flex",gap:"8px",marginBottom:"12px",flexWrap:"wrap"}}>
              {["all","indica","sativa","hibrido"].map(t=>(
                <button key={t} onClick={()=>setGFilter(t)} style={{padding:"8px 18px",borderRadius:"22px",fontSize:"13px",background:gFilter===t?(t==="all"?"var(--c223)":TIPO_COLOR[t]):"var(--c121)",color:gFilter===t?"#fff":"var(--c224)",border:`1px solid ${gFilter===t?(t==="all"?"var(--c225)":TIPO_COLOR[t]):"var(--c122)"}`,fontWeight:gFilter===t?"600":"400"}}>{t==="all"?"Todas":TIPO_LABEL[t]}</button>
              ))}
            </div>
            {!esEjecutor && (
              <div style={{display:"flex",gap:"8px",marginBottom:"20px",flexWrap:"wrap"}}>
                {[["activas","Activas"],["inactivas","Inactivas"],["todas","Todas"]].map(([k,label])=>(
                  <button key={k} onClick={()=>setGActivaFilter(k)} style={{padding:"6px 14px",borderRadius:"18px",fontSize:"12px",background:gActivaFilter===k?"var(--c226)":"var(--c227)",color:gActivaFilter===k?"var(--c228)":"var(--c229)",border:`1px solid ${gActivaFilter===k?"var(--c230)":"var(--c002)"}`}}>{label}</button>
                ))}
              </div>
            )}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
              {geneticas
                ...
                .map(g=>(
                <div key={g.id} style={{background:"var(--c231)",border:`1px solid ${g.color}55`,borderRadius:"14px",padding:"18px",borderLeft:`5px solid ${g.color}`,opacity:g.activa===false?0.55:1}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                    <div>
                      <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"5px",flexWrap:"wrap"}}>
                        <span style={{fontSize:"17px",fontWeight:"600",color:"var(--c129)"}}>{g.nombre}</span>
                        {g.fenotipo && <span style={{fontSize:"10px",background:`${g.color}33`,color:g.color,padding:"2px 8px",borderRadius:"10px"}}>FENOTIPO</span>}
                        {g.activa===false && <span style={{fontSize:"10px",background:"var(--c130)",color:"var(--c003)",padding:"2px 8px",borderRadius:"10px"}}>INACTIVA</span>}
                      </div>
                      <div style={{fontSize:"13px",color:TIPO_COLOR[g.tipo]}}>{TIPO_LABEL[g.tipo]}</div>
                    </div>
                    <div style={{background:"var(--c232)",border:`1px solid ${g.color}88`,borderRadius:"10px",padding:"8px 14px",textAlign:"center",minWidth:"72px"}}>
                      {g.dias_floracion_min ? (
                        <>
                          ...
                          <div style={{fontSize:"10px",color:"var(--c131)",marginTop:"3px"}}>días de flora</div>
                        </>
                      ) : g.semanas_floracion ? (
                        <>
                          <div style={{fontSize:"18px",fontWeight:"700",color:g.color,lineHeight:1}}>{g.semanas_floracion}</div>
                          <div style={{fontSize:"10px",color:"var(--c131)",marginTop:"3px"}}>semanas</div>
                        </>
                      ) : (
                        <div style={{fontSize:"11px",color:"var(--c050)"}}>—</div>
                      )}
                    </div>
                  </div>
                  {g.banco && <div style={{fontSize:"12px",color:"var(--c104)",marginBottom:"8px"}}>🏷 {g.banco}</div>}
                  {g.sabor && <div style={{fontSize:"14px",color:"var(--c233)",marginBottom:"6px",lineHeight:"1.6"}}>🍋 {g.sabor}</div>}
                  {g.efecto && <div style={{fontSize:"14px",color:"var(--c234)",lineHeight:"1.6",marginBottom:!esEjecutor?"12px":"0"}}>✨ {g.efecto}</div>}
                  {!esEjecutor && (
                    <div style={{display:"flex",gap:"6px",marginTop:"12px"}}>
                      <button onClick={()=>setEditingGenetica({...g, semanas_floracion:g.semanas_floracion||"", dias_floracion_min:g.dias_floracion_min||"", dias_floracion_max:g.dias_floracion_max||""})} style={{flex:1,padding:"8px",borderRadius:"8px",fontSize:"12px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>✏️ Editar</button>
                      <button onClick={()=>toggleActivaGenetica(g)} style={{flex:1,padding:"8px",borderRadius:"8px",fontSize:"12px",background:"var(--c077)",color:"var(--c003)",border:"1px solid var(--c078)"}}>{g.activa===false?"↺ Activar":"⏸ Desactivar"}</button>
                    </div>
                  )}
                </div>
              ))}
        ...
        {tab==="_cosechas" && !esEjecutor && (
          <div className="fade">
            ...
            <div style={{fontSize:"13px",color:"var(--c235)",marginBottom:"20px"}}>POR GENÉTICA · FECHA · SECADO · PRODUCCIÓN</div>

            {!esEjecutor && (
              <div>
                {editingCosecha ? (
                  <div style={card({borderColor:"var(--c236)",background:"var(--c237)"})}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                      ...
                      <div>
                        <div style={{...lbl("var(--c021)"),marginBottom:"6px"}}>GENÉTICA</div>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
                        <div>
                          <div style={{...lbl("var(--c021)"),marginBottom:"6px"}}>PLANTAS</div>
                        ...
                        <div>
                          <div style={{...lbl("var(--c021)"),marginBottom:"6px"}}>FECHA CORTE</div>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
                        <div>
                          <div style={{...lbl("var(--c021)"),marginBottom:"6px"}}>DÍAS SECADO</div>
                        ...
                        <div>
                          <div style={{...lbl("var(--c021)"),marginBottom:"6px"}}>GRAMOS</div>
                      ...
                      <div>
                        <div style={{...lbl("var(--c021)"),marginBottom:"6px"}}>NOTAS</div>
                      ...
                      <div style={{display:"flex",gap:"10px"}}>
                        <button disabled={savingCosecha} onClick={saveCosecha} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:savingCosecha?"var(--c132)":"var(--c040)",color:"var(--c018)",border:"1px solid var(--c041)"}}>{savingCosecha?"Guardando...":"✓ Guardar"}</button>
                        <button onClick={()=>setEditingCosecha(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                      </div>
                    </div>
                  </div>
                ) : null}
            ...
            {(()=>{
              ...
              if(cosechas.length===0 && !editingCosecha){
                return <div style={{textAlign:"center",padding:"40px 20px",color:"var(--c111)",fontSize:"14px"}}>Sin cosechas registradas aún</div>;
              }
              const porGenetica = {};
              cosechas.forEach(c=>{
              ...
              return (
                <>
                  {cosechas.length>0 && (
                    <div style={card({background:"var(--c238)",borderColor:"var(--c239)"})}>
                      <span style={lbl("var(--c133)")}>RESUMEN ACUMULADO</span>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"12px",textAlign:"center"}}>
                        <div><div style={{fontSize:"28px",fontWeight:"700",color:"var(--c003)"}}>{cosechas.length}</div><div style={{fontSize:"11px",color:"var(--c094)",marginTop:"3px"}}>cosechas</div></div>
                        <div><div style={{fontSize:"28px",fontWeight:"700",color:"var(--c001)"}}>{cosechas.reduce((a,c)=>a+(parseFloat(c.peso_cosechado_gramos)||0),0).toFixed(0)}g</div><div style={{fontSize:"11px",color:"var(--c094)",marginTop:"3px"}}>producción total</div></div>
                        <div><div style={{fontSize:"28px",fontWeight:"700",color:"var(--c240)"}}>{cosechas.reduce((a,c)=>a+(c.cantidad_plantas||0),0)}</div><div style={{fontSize:"11px",color:"var(--c094)",marginTop:"3px"}}>plantas totales</div></div>
                      </div>
                    </div>
                  )}
                  <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                    {gruposCosecha.map(grupo=>{
                      const color = grupo.g?.color || "var(--c133)";
                      ...
                      return (
                        <div key={grupo.key} style={{border:`1px solid ${color}44`,borderRadius:"14px",overflow:"hidden"}}>
                          <div onClick={()=>setCosechaGrupoAbierto(prev=>({...prev,[grupo.key]:!prev[grupo.key]}))} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:abierto?`${color}14`:"transparent"}}>
                            <span style={{fontSize:"14px",fontWeight:"600",color}}>{grupo.nombre} <span style={{fontSize:"11px",color:"var(--c134)",fontWeight:"400"}}>· {grupo.items.length} cosecha{grupo.items.length!==1?"s":""} · {grupo.totalG.toFixed(0)}g · {grupo.promedioG.toFixed(1)}g/planta</span></span>
                          ...
                          {abierto && (
                            <div style={{padding:"4px 14px 14px",display:"flex",flexDirection:"column",gap:"10px"}}>
                              {grupo.items.map(c=>{
                                ...
                                return (
                                  <div key={c.id} style={{background:"var(--c241)",border:`1px solid ${color}33`,borderRadius:"14px",padding:"16px",borderLeft:`4px solid ${color}`}}>
                                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                                      <div>
                                        <div style={{fontSize:"15px",fontWeight:"600",color:"var(--c129)",marginBottom:"4px"}}>{c.numero_lote?loteLabel(c.numero_lote, sufijoDeLote(c)):"—"}</div>
                                        <div style={{fontSize:"12px",color:"var(--c026)"}}>{s?.nombre||"—"} · {c.cantidad_plantas} plantas</div>
                                      </div>
                                      <div style={{textAlign:"right"}}>
                                        {c.fecha_inicio && <div style={{fontSize:"13px",color:"var(--c003)",marginBottom:"2px"}}>✂️ {c.fecha_inicio}</div>}
                                        {c.dias_secado && <div style={{fontSize:"12px",color:"var(--c074)"}}>💨 {c.dias_secado} días</div>}
                                      </div>
                                    </div>
                                    {c.peso_cosechado_gramos>0 && <div style={{background:"var(--c242)",borderRadius:"8px",padding:"10px 14px",marginBottom:"10px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:"13px",color:"var(--c134)"}}>Producción</span><span style={{fontSize:"22px",fontWeight:"700",color:"var(--c001)"}}>{c.peso_cosechado_gramos} g</span></div>}
                                    {c.notas && <div style={{fontSize:"13px",color:"var(--c243)",lineHeight:"1.6",marginBottom:"10px"}}>{c.notas}</div>}
                                    {!esEjecutor && (
                                      <div style={{display:"flex",gap:"8px"}}>
                                        <button onClick={()=>setEditingCosecha({...c, peso_cosechado_gramos:c.peso_cosechado_gramos||"", dias_secado:c.dias_secado||""})} style={{flex:1,padding:"8px",borderRadius:"8px",fontSize:"12px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>✏️ Editar</button>
                                        <button onClick={()=>delCosecha(c.id)} style={{padding:"8px 14px",borderRadius:"8px",fontSize:"12px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>🗑</button>
                                      </div>
                                    )}
                                  </div>
                                );
        ...
        {tab==="_bajas" && (()=>{
          ...
          return (
            <div className="fade">
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c009)",marginBottom:"6px"}}>📉 Bajas y Mortandad</div>
              <div style={{fontSize:"13px",color:"var(--c244)",marginBottom:"20px"}}>HISTORIAL · MOTIVO · ETAPA AL MOMENTO DE LA BAJA</div>

              {bajas.length===0 ? (
                <div style={{textAlign:"center",padding:"40px 20px",color:"var(--c245)",fontSize:"14px"}}>Sin bajas registradas — buena señal</div>
              ) : (
                <>
                  <div style={card({background:"var(--c045)",borderColor:"var(--c246)"})}>
                    <span style={lbl("var(--c009)")}>RESUMEN</span>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",textAlign:"center"}}>
                      <div><div style={{fontSize:"28px",fontWeight:"700",color:"var(--c031)"}}>{bajas.length}</div><div style={{fontSize:"11px",color:"var(--c135)",marginTop:"3px"}}>eventos de baja</div></div>
                      <div><div style={{fontSize:"28px",fontWeight:"700",color:"var(--c031)"}}>{totalPlantas}</div><div style={{fontSize:"11px",color:"var(--c135)",marginTop:"3px"}}>plantas perdidas</div></div>
                    </div>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                    {bajas.map((m,idx)=>{
                      ...
                      const sp = sub ? sectoresPadre.find(x=>x.id===sub.sector_padre_id) : null;
                      return (
                        <div key={m.id||idx} style={{background:"var(--c247)",border:"1px solid var(--c248)",borderRadius:"14px",padding:"16px",borderLeft:`4px solid ${g?.color||"var(--c136)"}`}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}>
                            <div>
                              <div style={{fontSize:"15px",fontWeight:"600",color:"var(--c249)"}}>{g?.nombre||"—"}</div>
                              <div style={{fontSize:"12px",color:"var(--c250)",marginTop:"2px"}}>{sp?.nombre||"—"}{sub?` › ${sub.nombre}`:""}</div>
                            </div>
                            <div style={{background:"var(--c251)",border:"1px solid var(--c252)",borderRadius:"8px",padding:"6px 12px",textAlign:"center"}}>
                              <div style={{fontSize:"18px",fontWeight:"700",color:"var(--c031)"}}>{m.cantidad_movida}</div>
                              <div style={{fontSize:"10px",color:"var(--c253)"}}>{m.tipo==="baja"?"lote completo":"plantas"}</div>
                            </div>
                          </div>
                          {m.motivo && <div style={{fontSize:"13px",color:"var(--c047)",lineHeight:"1.6",marginBottom:"6px"}}>📋 {m.motivo}</div>}
                          <div style={{fontSize:"11px",color:"var(--c254)"}}>{(m.fecha||"").slice(0,10)} · {usuariosMap[m.usuario_id]||"—"}</div>
                        </div>
                      );
                    })}
                  </div>
        ...
        {tab==="_misala" && !esEjecutor && (
          <div className="fade">
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c110)",marginBottom:"6px"}}>⚙️ Mi Sala</div>
            <div style={{fontSize:"13px",color:"var(--c053)",marginBottom:"20px"}}>ADMINISTRACIÓN DE SECTORES Y SUBSECTORES</div>

            <div style={{marginBottom:"14px",border:"1px solid var(--c027)",borderRadius:"12px",background:"var(--c137)",overflow:"hidden"}}>
              <div onClick={()=>setDatosMaestrosAbierto(!datosMaestrosAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px"}}>
                <span style={{...lbl("var(--c015)"),margin:0}}>🗂 DATOS MAESTROS</span>
                <span style={{fontSize:"13px",color:"var(--c015)",transform:datosMaestrosAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
              </div>
              {datosMaestrosAbierto && (
              <div style={{padding:"0 10px 12px"}}>

                {/* ── Tipos de tarea (hijo) ── */}
                <div style={{border:"1px solid var(--c255)",borderRadius:"10px",background:"var(--c256)",overflow:"hidden",marginBottom:"10px"}}>
                  <div onClick={()=>setTiposTareaAbierto(!tiposTareaAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px"}}>
                    <span style={{fontSize:"14px",fontWeight:"600",color:"var(--c138)"}}>📋 Tipos de tarea <span style={{fontSize:"11px",color:"var(--c032)",fontWeight:"400"}}>· {tiposTarea.length}</span></span>
                    <span style={{fontSize:"12px",color:"var(--c138)",transform:tiposTareaAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                  </div>
                  {tiposTareaAbierto && (
                  ...
                  {editingTipoTarea ? (
                    <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                      ...
                      <div>
                        <div style={{...lbl("var(--c020)"),marginBottom:"6px"}}>SECTORES HABILITADOS <span style={{fontWeight:"400",color:"var(--c257)"}}>(ninguno = todos)</span></div>
                        <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
                          {sectoresPadre.map(sp=>{
                            const ids = editingTipoTarea.sectores_padre_ids || [];
                            const marcado = ids.includes(sp.id);
                            return (
                              <label key={sp.id} style={{display:"flex",alignItems:"center",gap:"10px",fontSize:"13px",color:marcado?"var(--c258)":"var(--c259)",background:marcado?"var(--c260)":"var(--c089)",borderRadius:"8px",padding:"9px 12px",border:"1px solid "+(marcado?"var(--c261)":"var(--c052)"),cursor:"pointer"}}>
                      ...
                      <div style={{display:"flex",gap:"10px"}}>
                        <button disabled={savingTipoTarea || !editingTipoTarea.nombre} onClick={saveTipoTarea} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"13px",fontWeight:"600",background:"var(--c083)",color:"#fff",border:"none"}}>{savingTipoTarea?"Guardando...":"✓ Guardar"}</button>
                        <button onClick={()=>setEditingTipoTarea(null)} style={{padding:"10px 16px",borderRadius:"8px",fontSize:"13px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                      </div>
                    </div>
                  ) : (
                    <button onClick={()=>setEditingTipoTarea(emptyTipoTarea())} style={{width:"100%",padding:"10px",borderRadius:"8px",fontSize:"12px",fontWeight:"600",background:"var(--c088)",color:"var(--c015)",border:"1px dashed var(--c027)",marginBottom:"12px"}}>+ Nuevo tipo de tarea</button>
                  )}
                  {tiposTarea.length>0 && (
                    <div style={{display:"flex",flexDirection:"column",gap:"8px",marginTop:"12px"}}>
                      {tiposTarea.map(tt=>{
                        ...
                        return (
                          <div key={tt.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"var(--c120)",borderRadius:"10px",padding:"10px 14px",opacity:tt.activo===false?0.5:1}}>
                            <div>
                              <span style={{fontSize:"14px",color:"var(--c059)"}}>{tt.icono} {tt.nombre}</span>
                              <div style={{fontSize:"11px",color:"var(--c032)"}}>{nombres}</div>
                            </div>
                            <div style={{display:"flex",gap:"6px"}}>
                              <button onClick={()=>setEditingTipoTarea({...tt, sectores_padre_ids: (tt.sectores_padre_ids && tt.sectores_padre_ids.length) ? [...tt.sectores_padre_ids] : (tt.sector_padre_id ? [tt.sector_padre_id] : [])})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>✏️</button>
                              <button onClick={()=>toggleActivoTipoTarea(tt)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c077)",color:"var(--c003)",border:"1px solid var(--c078)"}}>{tt.activo===false?"↺":"⏸"}</button>
                              <button onClick={()=>delTipoTarea(tt)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>🗑</button>
                            </div>
                          </div>
                        );
                      })}
                ...
                {/* ── Genéticas (hijo) ── */}
                <div style={{border:"1px solid var(--c069)",borderRadius:"10px",background:"var(--c262)",overflow:"hidden"}}>
                  <div onClick={()=>setGeneticasBoxAbierto(!geneticasBoxAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px"}}>
                    <span style={{fontSize:"14px",fontWeight:"600",color:"var(--c033)"}}>🧬 Genéticas <span style={{fontSize:"11px",color:"var(--c022)",fontWeight:"400"}}>· {geneticas.length}</span></span>
                    <span style={{fontSize:"12px",color:"var(--c033)",transform:geneticasBoxAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                  </div>
                  {geneticasBoxAbierto && (
                  <div style={{padding:"0 12px 12px"}}>
                    <div style={{display:"flex",flexDirection:"column",gap:"6px",marginBottom:"10px"}}>
                      {geneticas.length===0 && <div style={{fontSize:"12px",color:"var(--c263)",fontStyle:"italic"}}>Sin genéticas cargadas todavía.</div>}
                      {[...geneticas].sort((a,b)=>(a.nombre||"").localeCompare(b.nombre||"")).map(g=>(
                        <div key={g.id} style={{display:"flex",alignItems:"center",gap:"8px",fontSize:"13px",color:"var(--c264)",background:"var(--c265)",borderRadius:"8px",padding:"8px 12px"}}>
                          <span style={{width:"10px",height:"10px",borderRadius:"50%",background:g.color||"var(--c033)",flexShrink:0}}/>
                          {g.nombre}
                        </div>
                      ))}
                    </div>
                    <button onClick={()=>setTab("_geneticas")} style={{width:"100%",padding:"11px",borderRadius:"9px",fontSize:"13px",fontWeight:"600",background:"var(--c266)",color:"var(--c033)",border:"1px solid var(--c069)"}}>🧬 Administrar genéticas</button>
                  </div>
                  )}
                </div>
            ...
            <div style={card({borderColor:"var(--c070)",background:"var(--c139)"})}>
              <div onClick={()=>setUsuariosSalaAbierto(!usuariosSalaAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:usuariosSalaAbierto?"12px":"0"}}>
                <span style={{...lbl("var(--c095)"),margin:0}}>🛠 ADMINISTRACIÓN <span style={{color:"var(--c039)",fontWeight:"400"}}>· Usuarios · {usuariosLista.length}</span></span>
                <span style={{fontSize:"13px",color:"var(--c095)",transform:usuariosSalaAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
              </div>
              ...
              {editingUsuario ? (
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                  <div>
                    <div style={{...lbl("var(--c140)"),marginBottom:"6px"}}>NOMBRE</div>
                    <input value={editingUsuario.nombre} onChange={e=>setEditingUsuario({...editingUsuario,nombre:e.target.value})} style={inp()}/>
                  </div>
                  <div>
                    <div style={{...lbl("var(--c140)"),marginBottom:"6px"}}>EMAIL</div>
                    <input type="email" value={editingUsuario.email} onChange={e=>setEditingUsuario({...editingUsuario,email:e.target.value})} style={inp()}/>
                  </div>
                  {!editingUsuario.id && (
                    <div style={{fontSize:"11px",color:"var(--c039)"}}>Se crea como Ejecutor en tu Sala. Solo el Administrador puede crear Master Growers nuevos.</div>
                  )}
                  <div style={{display:"flex",gap:"10px"}}>
                    <button disabled={savingUsuario || !editingUsuario.nombre || !editingUsuario.email} onClick={saveUsuario} style={{flex:1,padding:"10px",borderRadius:"8px",fontSize:"13px",fontWeight:"600",background:"var(--c267)",color:"#fff",border:"1px solid var(--c268)"}}>{savingUsuario?"Guardando...":"✓ Guardar"}</button>
                    <button onClick={()=>setEditingUsuario(null)} style={{padding:"10px 16px",borderRadius:"8px",fontSize:"13px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                  </div>
                </div>
              ) : (
                <button onClick={()=>setEditingUsuario(emptyUsuario())} style={{width:"100%",padding:"10px",borderRadius:"8px",fontSize:"13px",background:"var(--c139)",color:"var(--c095)",border:"2px dashed var(--c070)",marginBottom:"4px"}}>+ Nuevo usuario</button>
              )}

              <div style={{display:"flex",flexDirection:"column",gap:"10px",marginTop:"12px"}}>
                {usuariosLista.map(u=>(
                  <div key={u.id} style={{background:"var(--c269)",borderRadius:"10px",padding:"10px 14px",opacity:u.activo===false?0.5:1}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:u.rol==="ejecutor"?"8px":"0"}}>
                      <div>
                        <span style={{fontSize:"14px",color:"var(--c270)"}}>{u.nombre}</span>
                        <span style={{fontSize:"10px",color:"var(--c271)",marginLeft:"8px"}}>{u.rol==="ejecutor"?"EJECUTOR":"MASTER GROWER"}</span>
                        {u.activo===false && <span style={{fontSize:"10px",color:"var(--c011)",marginLeft:"8px"}}>DESACTIVADO</span>}
                      </div>
                      <div style={{display:"flex",gap:"6px"}}>
                        <button onClick={()=>setEditingUsuario({id:u.id, nombre:u.nombre, email:u.email, rol:u.rol})} style={{padding:"5px 9px",borderRadius:"6px",fontSize:"10px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>✏️</button>
                        {u.id!==usuario.id && (
                          <button onClick={()=>toggleActivoUsuario(u)} style={{padding:"5px 9px",borderRadius:"6px",fontSize:"10px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>{u.activo===false?"↺ Activar":"⏸ Desactivar"}</button>
                        )}
                      </div>
                    </div>
                    {u.rol==="ejecutor" && (
                      <button onClick={()=>togglePermisoRelevamiento(u)} style={{padding:"6px 10px",borderRadius:"6px",fontSize:"10px",background:u.permisos?.relevamiento?"var(--c272)":"var(--c141)",color:u.permisos?.relevamiento?"var(--c273)":"var(--c274)",border:`1px solid ${u.permisos?.relevamiento?"var(--c275)":"var(--c066)"}`}}>🔍 {u.permisos?.relevamiento?"Relevamiento: ON":"Relevamiento: OFF"}</button>
                    )}
                  </div>
                ))}
              </div>
            ...
            {lotes.filter(l=>l.estado==="pendiente_aprobacion").length>0 && (
              <div style={card({borderColor:"var(--c276)",background:"var(--c277)"})}>
                <span style={lbl("var(--c057)")}>🆕 PENDIENTES DE APROBACIÓN (detectados por relevamiento)</span>
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                  {lotes.filter(l=>l.estado==="pendiente_aprobacion").map(l=>{
                    ...
                    const sp = sub ? sectoresPadre.find(x=>x.id===sub.sector_padre_id) : null;
                    return (
                      <div key={l.id} style={{background:"var(--c278)",borderRadius:"10px",padding:"12px 14px"}}>
                        <div style={{fontSize:"14px",color:"var(--c279)",marginBottom:"4px"}}>{g?.nombre||"Sin genética"} · {l.cantidad_plantas} plantas</div>
                        <div style={{fontSize:"12px",color:"var(--c280)",marginBottom:"8px"}}>{sp?.nombre||"—"}{sub?` › ${sub.nombre}`:""}</div>
                        {l.notas && <div style={{fontSize:"11px",color:"var(--c281)",marginBottom:"8px"}}>{l.notas}</div>}
                        <div style={{display:"flex",gap:"6px"}}>
                          <button onClick={()=>aprobarLotePendiente(l)} style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"var(--c282)",color:"var(--c001)",border:"1px solid var(--c283)"}}>✓ Aprobar</button>
                          <button onClick={()=>setEditingLote({...l, cantidad_plantas:String(l.cantidad_plantas)})} style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>✏️ Editar y aprobar</button>
                          <button onClick={()=>delLote(l.id)} style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>🗑</button>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div style={{marginBottom:"14px",border:"1px solid var(--c030)",borderRadius:"12px",background:"var(--c284)",overflow:"hidden"}}>
              <div onClick={()=>setEstructuraAbierto(!estructuraAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:"var(--c285)"}}>
                <span style={{...lbl("var(--c071)"),margin:0}}>🧱 ESTRUCTURA <span style={{color:"var(--c053)",fontWeight:"400"}}>· Sectores y subsectores</span></span>
                <span style={{fontSize:"13px",color:"var(--c071)",transform:estructuraAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
              </div>
              {estructuraAbierto && (<div style={{padding:"10px"}}>
            {editingSub && (
              <div style={card({borderColor:"var(--c115)",background:"var(--c286)"})}>
                <span style={lbl("var(--c287)")}>{editingSub.id?"EDITAR SUBSECTOR":"NUEVO SUBSECTOR"}</span>
                <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                  <div>
                    <div style={{...lbl("var(--c096)"),marginBottom:"6px"}}>SECTOR PADRE</div>
                  ...
                  <div>
                    <div style={{...lbl("var(--c096)"),marginBottom:"6px"}}>NOMBRE DEL SUBSECTOR</div>
                  ...
                  <div>
                    <div style={{...lbl("var(--c096)"),marginBottom:"6px"}}>CAPACIDAD (opcional)</div>
                  ...
                  {sectoresPadre.find(sp=>sp.id===editingSub.sector_padre_id)?.nombre==="Vegetativo" && (
                    <label style={{display:"flex",alignItems:"flex-start",gap:"10px",fontSize:"13px",color:"var(--c288)",background:"var(--c114)",borderRadius:"8px",padding:"10px 12px",border:"1px solid var(--c030)"}}>
                      ...
                      <span>Permite avanzar a Floración<br/><span style={{fontSize:"11px",color:"var(--c053)"}}>Si lo destildás, los lotes en este subsector no van a poder cortar directo a flora (ej: 150cc todavía muy chico).</span></span>
                    </label>
                  )}
                  <div style={{display:"flex",gap:"10px"}}>
                    <button disabled={savingSub || !editingSub.sector_padre_id || !editingSub.nombre} onClick={saveSub} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:savingSub?"var(--c030)":"var(--c070)",color:"#fff",border:"1px solid var(--c289)"}}>{savingSub?"Guardando...":"✓ Guardar"}</button>
                    <button onClick={()=>setEditingSub(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {sectoresPadre.map(sp=>{
              ...
              return (
              <div key={sp.id} style={card({borderColor:"var(--c030)"})}>
                <div onClick={()=>setSectorMiSalaAbierto(abierto?"":sp.id)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:abierto?"14px":"0"}}>
                  <span style={{fontSize:"17px",fontWeight:"600",color:"var(--c071)"}}>{sp.nombre} <span style={{fontSize:"11px",color:"var(--c053)",fontWeight:"400"}}>· {subsDeEste.length}</span></span>
                  <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                    {abierto && <button onClick={(e)=>{e.stopPropagation(); setEditingSub(emptySubsector(sp.id));}} style={{fontSize:"12px",padding:"6px 14px",borderRadius:"14px",background:"var(--c141)",color:"var(--c071)",border:"1px dashed var(--c070)"}}>+ Agregar</button>}
                    <span style={{fontSize:"13px",color:"var(--c053)",transform:abierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
                  </div>
                </div>
                {abierto && (
                <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                  {subsDeEste.map(sub=>(
                    <div key={sub.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"var(--c054)",borderRadius:"8px",padding:"10px 14px"}}>
                      <div>
                        <div style={{fontSize:"14px",color:"var(--c018)"}}>{sub.nombre}</div>
                        {sub.capacidad_unidades && <div style={{fontSize:"11px",color:"var(--c026)"}}>capacidad: {sub.capacidad_unidades}</div>}
                        {sp.nombre==="Vegetativo" && sub.puede_avanzar_etapa===false && <div style={{fontSize:"10px",color:"var(--c290)"}}>⏸ no habilitada para pasar a Floración</div>}
                      </div>
                      <div style={{display:"flex",gap:"6px"}}>
                        <button onClick={()=>setEditingSub({id:sub.id, sector_padre_id:sub.sector_padre_id, nombre:sub.nombre, capacidad_unidades:sub.capacidad_unidades||"", puede_avanzar_etapa:sub.puede_avanzar_etapa})} style={{fontSize:"11px",padding:"6px 10px",borderRadius:"6px",background:"var(--c019)",color:"var(--c001)",border:"1px solid var(--c010)"}}>✏️</button>
                        <button onClick={()=>delSub(sub.id)} style={{fontSize:"11px",padding:"6px 10px",borderRadius:"6px",background:"var(--c007)",color:"var(--c011)",border:"1px solid var(--c008)"}}>🗑</button>
                      </div>
                    </div>
                  ))}
                  {subsDeEste.length===0 && <div style={{fontSize:"12px",color:"var(--c050)",fontStyle:"italic"}}>Sin subsectores</div>}
                </div>
                )}
              </div>
              );
          ...
          <div style={{padding:"16px"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
              {[
                ...
                return (
                  <div key={o.k} onClick={()=>cambiarTema(o.k)} style={{cursor:"pointer",borderRadius:"12px",padding:"14px 16px",border:"2px solid "+(sel?"var(--c001)":"var(--c002)"),background:sel?"var(--c026)":"var(--c004)"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:"15px",fontWeight:"600",color:sel?"var(--c001)":"var(--c006)"}}>{o.t}</span>
                      {sel && <span style={{fontSize:"14px",color:"var(--c001)"}}>✓</span>}
                    </div>
                    <div style={{fontSize:"11px",color:"var(--c024)",marginTop:"4px"}}>{o.d}</div>
                  </div>
                );
              })}
            </div>
            {guardandoTema && <div style={{fontSize:"11px",color:"var(--c024)",marginTop:"12px"}}>Guardando…</div>}
            <div style={{fontSize:"11px",color:"var(--c024)",marginTop:"18px",lineHeight:"1.5",background:"var(--c004)",borderRadius:"8px",padding:"10px 12px"}}>
              Los colores que vos elegiste para tus genéticas no cambian con el tema — son tuyos. Si alguno queda poco legible en modo claro, podés editarlo en Genéticas.
            </div>
          </div>
        )}
        ...
        {tab==="_reportes" && !esEjecutor && (()=>{
          ...
          const BarraStacked = ({items, total}) => (
            <div>
              <div style={{display:"flex",height:"22px",borderRadius:"6px",overflow:"hidden",marginBottom:"8px"}}>
                {items.map((it,i)=><div key={i} style={{width:`${(it.cant/total*100).toFixed(1)}%`,background:it.g.color||"var(--c026)"}}/>)}
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:"4px"}}>
                {items.map((it,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:"12px"}}>
                    <span style={{color:it.g.color||"var(--c086)"}}>● {it.g.nombre}</span>
                    <span style={{color:"#9ab"}}>{it.cant} · {(it.cant/total*100).toFixed(0)}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          return (
            <div className="fade">
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c064)",marginBottom:"6px"}}>📊 Reportes</div>
              <div style={{fontSize:"12px",color:"var(--c291)",marginBottom:"16px"}}>Tocá cada reporte para abrirlo</div>

              <Reporte id="distribucion" titulo="🧬 Distribución de genéticas (total activo)" color="var(--c001)" vacio={globalItems.length===0 && <div style={{fontSize:"13px",color:"var(--c023)"}}>Sin plantas activas.</div>}>
                <BarraStacked items={globalItems} total={globalTotal}/>
              </Reporte>

              <Reporte id="subsector" titulo="📍 Genéticas por subsector" color="var(--c051)" vacio={subsConPlantas.length===0 && <div style={{fontSize:"13px",color:"var(--c023)"}}>Sin plantas en subsectores.</div>}>
                <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
                  {subsConPlantas.map((s,i)=>(
                    <div key={i}>
                      <div style={{fontSize:"13px",color:"var(--c018)",marginBottom:"6px"}}>{s.nombre} <span style={{fontSize:"11px",color:"var(--c074)"}}>· {s.sp} · {s.total} plantas</span></div>
              ...
              <Reporte id="produccion" titulo="⚖️ Producción por genética (g/planta)" color="var(--c003)" vacio={produccionOrdenada.length===0 && <div style={{fontSize:"13px",color:"var(--c023)"}}>Sin cosechas registradas todavía.</div>}>
                {produccionOrdenada.map((x,i)=>(
                  <div key={i} style={{marginBottom:"10px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",marginBottom:"4px"}}>
                      <span style={{color:x.g.color||"var(--c003)"}}>{x.g.nombre}</span>
                      <span style={{color:"var(--c142)",fontWeight:"600"}}>{x.porPlanta.toFixed(1)} g/planta <span style={{fontSize:"10px",color:"var(--c292)",fontWeight:"400"}}>({x.gramos}g · {x.plantas} pl.)</span></span>
                    </div>
                    <div style={{height:"8px",background:"var(--c293)",borderRadius:"4px",overflow:"hidden"}}><div style={{width:`${(x.porPlanta/maxProd*100)}%`,height:"100%",background:"var(--c003)"}}/></div>
                  </div>
                ))}
              </Reporte>

              <Reporte id="mortandad" titulo="🥀 Mortandad por genética (bajas)" color="var(--c009)" vacio={bajasOrdenadas.length===0 && <div style={{fontSize:"13px",color:"var(--c023)"}}>Sin bajas registradas.</div>}>
                {bajasOrdenadas.map((x,i)=>(
                  <div key={i} style={{marginBottom:"10px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",marginBottom:"4px"}}>
                      <span style={{color:x.g.color||"var(--c031)"}}>{x.g.nombre}</span>
                      <span style={{color:"var(--c294)",fontWeight:"600"}}>{x.bajas} bajas</span>
                    </div>
                    <div style={{height:"8px",background:"var(--c295)",borderRadius:"4px",overflow:"hidden"}}><div style={{width:`${(x.bajas/maxBaja*100)}%`,height:"100%",background:"var(--c009)"}}/></div>
                  </div>
                ))}
              </Reporte>

              <Reporte id="ocupacion" titulo="📦 Ocupación de subsectores" color="var(--c063)" vacio={ocupacion.length===0 && <div style={{fontSize:"13px",color:"var(--c023)"}}>Sin subsectores con capacidad o plantas.</div>}>
                {ocupacion.map((o,i)=>{
                  const pct = o.cap>0 ? Math.min(100, o.plantas/o.cap*100) : 0;
                  const col = pct>=90?"var(--c009)":pct>=70?"var(--c003)":"var(--c035)";
                  return (
                    <div key={i} style={{marginBottom:"10px"}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",marginBottom:"4px"}}>
                        <span style={{color:"var(--c090)"}}>{o.nombre} <span style={{fontSize:"10px",color:"var(--c058)"}}>· {o.sp}</span></span>
                        <span style={{color:col}}>{o.plantas}{o.cap>0?` / ${o.cap}`:""}</span>
                      </div>
                      {o.cap>0 && <div style={{height:"8px",background:"var(--c296)",borderRadius:"4px",overflow:"hidden"}}><div style={{width:`${pct}%`,height:"100%",background:col}}/></div>}
                    </div>
                  );
                })}
              </Reporte>

              <Reporte id="prom_genetica" titulo="⏱ Tiempo promedio por etapa (por genética)" color="var(--c015)" vacio={Object.keys(promedios).length===0 && <div style={{fontSize:"13px",color:"var(--c023)"}}>Todavía no hay etapas terminadas. Un promedio aparece recién cuando un lote sale de una etapa con un movimiento real (avance o cosecha) — los lotes de la carga inicial que no se movieron no cuentan.</div>}>
                <div style={{fontSize:"11px",color:"var(--c058)",marginBottom:"10px",lineHeight:"1.4"}}>Solo cuenta etapas que el lote <b>terminó</b> (salió de ellas con un movimiento). Los ciclos en curso y la carga inicial sin movimientos no se promedian.</div>
                <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                  {Object.entries(promedios).map(([gid,etapas])=>{
                    ...
                    return (
                      <div key={gid}>
                        <div style={{fontSize:"13px",color:g.color||"var(--c015)",marginBottom:"6px"}}>{g.nombre}</div>
                        {sectoresPadre.map(sp=>{
                          const e = etapas[sp.nombre];
                          if(!e) return null;
                          return <div key={sp.id} style={{display:"flex",justifyContent:"space-between",fontSize:"12px",marginBottom:"3px"}}><span style={{color:"var(--c297)"}}>{sp.nombre}</span><span style={{color:"#bcd"}}>{Math.round(e.suma/e.n)} días <span style={{color:"var(--c058)"}}>(prom. {e.n})</span></span></div>;
                        })}
                      </div>
                    );
                  })}
              ...
              <Reporte id="info_lotes" titulo="📋 Reporte info. LOTES" color="var(--c298)" vacio={lotes.length===0 && <div style={{fontSize:"13px",color:"var(--c023)"}}>Sin lotes cargados.</div>}>
                ...
                <div style={{fontSize:"11px",color:"var(--c299)",marginBottom:"8px"}}>{lotesInfo.length} lote{lotesInfo.length!==1?"s":""}</div>
                <div style={{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"12px"}}>
                  {lotesInfo.map(({l,s,spL,st,g})=>(
                    <div key={l.id} style={{background:"var(--c300)",borderRadius:"8px",padding:"9px 11px",border:"1px solid var(--c301)"}}>
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:"12px",marginBottom:"3px"}}>
                        <span style={{color:g?.color||"var(--c302)",fontWeight:"600"}}>{g?.nombre||"Sin genética"}{l.numero_lote?` · ${loteLabel(l.numero_lote, sufijoDeLote(l))}`:""}</span>
                        <span style={{color:"var(--c303)"}}>{l.cantidad_plantas} pl.</span>
                      </div>
                      <div style={{fontSize:"10px",color:"var(--c304)",marginBottom:"4px"}}>{l.estado} · {s?.nombre||"—"} ({spL?.nombre||"—"}) · inicio {l.fecha_inicio||"—"}</div>
                      <div style={{display:"flex",gap:"8px",flexWrap:"wrap",fontSize:"10px"}}>
                        <span style={{color:"var(--c065)"}}>Enr {st.dias_enraizado!=null?st.dias_enraizado:"—"}d</span>
                        <span style={{color:"var(--c065)"}}>Veg {st.dias_vegetativo!=null?st.dias_vegetativo:"—"}d</span>
                        <span style={{color:"var(--c051)"}}>Flo {st.dias_floracion!=null?st.dias_floracion:"—"}d</span>
                        <span style={{color:"var(--c003)"}}>Sec {(l.dias_secado!=null?l.dias_secado:(st.dias_secado!=null?st.dias_secado:"—"))}d</span>
                        {l.estado==="cosechado" && <span style={{color:"var(--c142)"}}>⚖ {l.peso_cosechado_gramos||0}g</span>}
                      </div>
                    </div>
                  ))}
                </div>
                <button onClick={descargarTSVLotes} disabled={lotesInfo.length===0} style={{width:"100%",padding:"11px",borderRadius:"9px",fontSize:"13px",background:"var(--c130)",color:"var(--c305)",border:"1px solid var(--c306)",fontWeight:"600"}}>⬇ Descargar TSV ({lotesInfo.length} lotes)</button>
              </Reporte>
      ...
      {editingLote && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
          <div style={{background:"var(--c072)",border:"1px solid var(--c010)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
              <div>
                <div style={{...lbl("var(--c034)"),marginBottom:"6px"}}>GENÉTICA</div>
              ...
              <div>
                <div style={{...lbl("var(--c034)"),marginBottom:"6px"}}>CANTIDAD DE PLANTAS</div>
                ...
                {editingLote.id && <div style={{fontSize:"11px",color:"var(--c023)",marginTop:"4px"}}>Esto es para corregir un error de carga. Para una baja real (muerte, descarte) usá el botón 🥀 en la tarjeta del lote — así queda registrada con motivo.</div>}
              </div>
              <div>
                <div style={{...lbl("var(--c034)"),marginBottom:"6px"}}>FECHA INICIO</div>
              ...
              <div>
                <div style={{...lbl("var(--c034)"),marginBottom:"6px"}}>NOTAS</div>
              ...
              <div style={{display:"flex",gap:"10px",marginTop:"4px"}}>
                <button disabled={savingLote || !editingLote.genetica_id || editingLote.cantidad_plantas===""} onClick={saveLote} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:savingLote?"var(--c132)":"var(--c040)",color:"var(--c018)",border:"1px solid var(--c041)"}}>{savingLote?"Guardando...":(editingLote.id?"✓ Guardar cambios":"✓ Crear lote")}</button>
                <button onClick={()=>setEditingLote(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
              </div>
              {editingLote.id && (
                <button onClick={()=>delLote(editingLote.id)} style={{padding:"10px",borderRadius:"10px",fontSize:"12px",background:"transparent",color:"var(--c136)",border:"1px solid var(--c008)"}}>🗑 Borrar lote por completo (solo errores de carga)</button>
              )}
            </div>
          </div>
        </div>
      ...
      {bajaLote && (()=>{
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"var(--c307)",border:"1px solid var(--c008)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"19px",color:"var(--c031)",marginBottom:"6px"}}>🥀 Dar de baja</div>
              <div style={{fontSize:"13px",color:"var(--c308)",marginBottom:"16px"}}>{g?.nombre||"—"} · {bajaLote.lote.cantidad_plantas} plantas activas</div>
              <div style={{...lbl("var(--c038)"),marginBottom:"6px"}}>CANTIDAD A DAR DE BAJA</div>
              ...
              <div style={{...lbl("var(--c038)"),marginBottom:"6px"}}>MOTIVO</div>
              ...
              {esTotal && <div style={{fontSize:"12px",color:"var(--c003)",background:"var(--c109)",borderRadius:"8px",padding:"8px 12px",marginBottom:"14px"}}>⚠ Esto da de baja el lote completo — desaparece de "activos" y queda solo en el historial.</div>}
              <div style={{display:"flex",gap:"10px"}}>
                <button disabled={savingBaja || cant<=0 || cant>bajaLote.lote.cantidad_plantas} onClick={confirmBaja} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:savingBaja?"var(--c309)":"var(--c048)",color:"#fff",border:"1px solid var(--c049)"}}>{savingBaja?"Guardando...":(esTotal?"✓ Confirmar baja total":"✓ Confirmar baja parcial")}</button>
                <button onClick={()=>setBajaLote(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
              </div>
            </div>
          </div>
        );
      ...
      {finalizandoLote && (()=>{
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"var(--c119)",border:"1px solid var(--c117)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"19px",color:"var(--c057)",marginBottom:"6px"}}>⚖️ Finalizar y pesar</div>
              <div style={{fontSize:"13px",color:"var(--c310)",marginBottom:"16px"}}>{g?.nombre||"—"} · {finalizandoLote.lote.cantidad_plantas} plantas</div>
              <div style={{...lbl("var(--c097)"),marginBottom:"6px"}}>PESO COSECHADO (GRAMOS)</div>
              ...
              <div style={{...lbl("var(--c097)"),marginBottom:"6px"}}>DÍAS DE SECADO (OPCIONAL)</div>
              ...
              <div style={{...lbl("var(--c097)"),marginBottom:"6px"}}>NOTAS</div>
              ...
              <div style={{fontSize:"12px",color:"var(--c021)",background:"var(--c092)",borderRadius:"8px",padding:"8px 12px",marginBottom:"14px"}}>Esto cierra el lote y lo manda a 📦 Cosechas — ya no aparece como activo en ningún subsector.</div>
              <div style={{display:"flex",gap:"10px"}}>
                <button disabled={savingFinal || !finalizandoLote.peso} onClick={confirmFinalizar} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:savingFinal?"var(--c112)":"var(--c311)",color:"var(--c312)",border:"1px solid var(--c313)"}}>{savingFinal?"Guardando...":"✓ Confirmar"}</button>
                <button onClick={()=>setFinalizandoLote(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
              </div>
            </div>
          </div>
        );
      ...
      {editingTarea && (()=>{
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200,overflowY:"auto"}}>
            <div style={{background:"var(--c137)",border:"1px solid var(--c027)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px",margin:"auto"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"19px",color:"var(--c015)",marginBottom:"16px"}}>{editingTarea.id?"Editar tarea":"Nueva tarea"}</div>
              <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                <div>
                  <div style={{...lbl("var(--c020)"),marginBottom:"6px"}}>TÍTULO</div>
                ...
                <div>
                  <div style={{...lbl("var(--c020)"),marginBottom:"6px"}}>SUBSECTOR</div>
                ...
                <div>
                  <div style={{...lbl("var(--c020)"),marginBottom:"6px"}}>TIPO</div>
                  {tiposDisponibles.length===0 ? (
                    <div style={{fontSize:"12px",color:"var(--c021)",background:"var(--c092)",borderRadius:"8px",padding:"10px 12px"}}>No hay tipos de tarea para este subsector — agregalos en ⚙️ Mi Sala.</div>
                  ) : (
                ...
                {editingTarea.sector_id && lotesDelSector.length>0 && (
                  <div>
                    <div style={{...lbl("var(--c020)"),marginBottom:"6px"}}>LOTE (OPCIONAL)</div>
                ...
                <div>
                  <div style={{...lbl("var(--c020)"),marginBottom:"6px"}}>DESCRIPCIÓN</div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
                  <div>
                    <div style={{...lbl("var(--c020)"),marginBottom:"6px"}}>FECHA PROGRAMADA</div>
                  ...
                  <div>
                    <div style={{...lbl("var(--c020)"),marginBottom:"6px"}}>PRIORIDAD</div>
                ...
                <div>
                  <div style={{...lbl("var(--c020)"),marginBottom:"6px"}}>ASIGNAR A</div>
                ...
                <div style={{display:"flex",gap:"10px"}}>
                  <button disabled={savingTarea || !editingTarea.titulo || !editingTarea.fecha_programada || !editingTarea.tipo_id} onClick={saveTarea} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:savingTarea?"var(--c314)":"var(--c083)",color:"#fff",border:"1px solid var(--c015)"}}>{savingTarea?"Guardando...":"✓ Guardar"}</button>
                  <button onClick={()=>setEditingTarea(null)} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
                </div>
              </div>
            </div>
          </div>
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"var(--c072)",border:"1px solid var(--c010)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              ...
              <div style={{fontSize:"13px",color:"var(--c315)",marginBottom:"16px"}}>{g?.nombre} · {movingLote.cantidad_plantas} plantas{modoRetroceso ? ` → ${subDestinoFijo?.nombre}` : !modoMover && ` → ${spDestinoNatural?.nombre}`}</div>
              {modoMover && <div style={{fontSize:"11px",color:"var(--c118)",marginBottom:"14px"}}>Para corregir un error de ubicación. No se puede volver a Enraizado, ni pasar a una maceta más chica dentro de Vegetativo.</div>}
              {modoRetroceso && <div style={{fontSize:"11px",color:"var(--c087)",marginBottom:"14px"}}>Todavía no entró en 12/12, así que no cuenta como floración real — vuelve a Vegetativo, a Vege 5L (el paso inmediatamente anterior). No se puede ir más atrás.</div>}
              <div style={{...lbl("var(--c034)"),marginBottom:"6px"}}>CANTIDAD A {modoRetroceso?"VOLVER":modoMover?"MOVER":"AVANZAR"}</div>
              ...
              <div style={{fontSize:"11px",color:"var(--c026)",marginBottom:"16px"}}>{(parseInt(movingMovida)||0) < movingLote.cantidad_plantas && (parseInt(movingMovida)||0)>0 ? `Quedan ${movingLote.cantidad_plantas-(parseInt(movingMovida)||0)} plantas en ${spOrigen?.nombre} — se crea un lote hermano en el destino` : `${modoRetroceso?"Vuelve":modoMover?"Mueve":"Avanza"} el lote completo`}</div>
              {modoRetroceso ? (
                <div style={{marginBottom:"16px"}}>
                  <div style={{...lbl("var(--c034)"),marginBottom:"6px"}}>SUBSECTOR DESTINO</div>
                  <div style={{...inp(),background:"var(--c099)",color:"var(--c087)"}}>{subDestinoFijo?.nombre||"—"}</div>
                </div>
              ) : (
                <>
                  <div style={{...lbl("var(--c034)"),marginBottom:"6px"}}>SUBSECTOR DESTINO</div>
              ...
              <div style={{display:"flex",gap:"10px"}}>
                <button disabled={!destinoSub || savingMovida || (parseInt(movingMovida)||0)<=0 || (parseInt(movingMovida)||0)>movingLote.cantidad_plantas} onClick={confirmarMovimiento} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"var(--c040)",color:"var(--c018)",border:"1px solid var(--c041)"}}>{savingMovida?"Moviendo...":"✓ Confirmar"}</button>
                <button onClick={cerrar} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"var(--c004)",color:"var(--c006)",border:"1px solid var(--c002)"}}>Cancelar</button>
              </div>
            </div>
          </div>
        );
      ...
      {(()=>{
        ...
        const grupoSistema = tabsExtra.filter(t=>t.key==="_misala");
        ...
        return (
          <>
            ...
            <div style={{position:"fixed",left:0,right:0,bottom:0,background:"var(--c316)",borderRadius:"18px 18px 0 0",borderTop:"1px solid var(--c024)",transform:masAbierto?"translateY(0)":"translateY(100%)",transition:"transform .25s ease",zIndex:160,padding:"10px 20px 28px",maxHeight:"70vh",overflow:"auto"}}>
              <div style={{width:"36px",height:"4px",background:"var(--c317)",borderRadius:"3px",margin:"6px auto 16px"}}/>
              {[["CULTIVO",grupoCultivo],["REGISTROS",grupoRegistros],["SISTEMA",grupoSistema]].filter(([,g])=>g.length>0).map(([label,grupo])=>(
                <div key={label}>
                  <div style={{fontSize:"10px",letterSpacing:"2px",color:"var(--c036)",margin:"14px 0 8px"}}>{label}</div>
                  {grupo.map(t=>(
                    <div key={t.key} onClick={()=>irA(t.key)} style={{display:"flex",alignItems:"center",gap:"10px",padding:"11px 6px",fontSize:"14px",color:tab===t.key?t.col:"var(--c090)",background:tab===t.key?`${t.col}1a`:"transparent",borderRadius:"8px",borderBottom:"1px solid var(--c318)"}}>{t.label}</div>
                  ))}
                </div>
              ))}
            </div>

            <div style={{position:"fixed",bottom:0,left:0,right:0,background:"var(--c319)",borderTop:"1px solid var(--c024)",display:"flex",padding:"6px 4px 10px",zIndex:140}}>
              {sectoresPadre.map(sp=>{
                const c = colorDeFase(sp.orden);
                const activo = tab===sp.id;
                return (
                  <button key={sp.id} onClick={()=>setTab(sp.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:"3px",padding:"6px 2px",background:"none",border:"none",color:activo?c:"var(--c023)",fontSize:"10px"}}>
                    <span style={{fontSize:"15px"}}>{iconoDeFase(sp.orden)}</span>
                    <span>{sp.nombre}</span>
                    <span style={{width:activo?"7px":"5px",height:activo?"7px":"5px",borderRadius:"50%",background:activo?c:"var(--c050)",boxShadow:activo?`0 0 6px ${c}`:"none"}}/>
                  </button>
                );
              })}
              <button onClick={()=>setMasAbierto(true)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:"3px",padding:"6px 2px",background:"none",border:"none",color:masActivo?"var(--c001)":"var(--c023)",fontSize:"10px"}}>
                <span style={{fontSize:"15px"}}>☰</span>
                <span>Más</span>
                <span style={{width:masActivo?"7px":"5px",height:masActivo?"7px":"5px",borderRadius:"50%",background:masActivo?"var(--c001)":"var(--c050)",boxShadow:masActivo?"0 0 6px var(--c001)":"none"}}/>
              </button>
            </div>
          </>
        );
