...
export default function App() {
  ...
  const delGenetica = async (g) => {
    const enUso = lotes.some(l=>l.genetica_id===g.id);
    if(enUso){
      setStatus("no se puede borrar: tiene lotes asociados — desactivala en su lugar");
      setTimeout(()=>setStatus(""),4500);
      return;
    }
    if(!window.confirm(`¿Borrar "${g.nombre}" del catálogo? No se puede deshacer.`)) return;
    try { await supabase.from("geneticas").delete().eq("id", g.id); cargarTodo(); }
    catch(e) { setStatus("error al borrar genética"); setTimeout(()=>setStatus(""),3000); }
  };
                ...
                <div key={g.id} style={{background:"#162416",border:`1px solid ${g.color}55`,borderRadius:"14px",padding:"18px",borderLeft:`5px solid ${g.color}`,opacity:g.activa===false?0.55:1}}>
                  ...
                  {!esEjecutor && (
                    <div style={{display:"flex",gap:"6px",marginTop:"12px"}}>
                      ...
                      <button onClick={()=>delGenetica(g)} style={{padding:"8px 14px",borderRadius:"8px",fontSize:"12px",background:"#2a1010",color:"#aa5050",border:"1px solid #4a2020"}}>🗑</button>
                    </div>
                  )}
                </div>
              ))}
