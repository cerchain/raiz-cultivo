  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"var(--c074)",minHeight:"100vh",color:"var(--c019)"}}>
      ...
      {(()=>{
        ...
        const pendientesInventario = esEjecutor ? 0 : lotes.filter(l=>l.estado==="pendiente_aprobacion").length + relevamientos.filter(r=>!r.aplicado).length;
        return (
          <>
                ...
                <div key={label}>
                  ...
                  {grupo.map(t=>(
                    <div key={t.key} onClick={()=>irA(t.key)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:"10px",padding:"11px 6px",fontSize:"14px",color:tab===t.key?t.col:"var(--c091)",background:tab===t.key?`${t.col}1a`:"transparent",borderRadius:"8px",borderBottom:"1px solid var(--c321)"}}>
                      <span>{t.label}</span>
                      {t.key==="_relevamientos" && pendientesInventario>0 && (
                        <span style={{fontSize:"11px",fontWeight:"600",color:"var(--c001)",background:"var(--c020)",border:"1px solid var(--c011)",borderRadius:"10px",padding:"2px 9px"}}>{pendientesInventario} pendiente{pendientesInventario!==1?"s":""}</span>
                      )}
                    </div>
                  ))}
                </div>
