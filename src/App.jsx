...
function RaizApp() {
  ...
  const guardarFechaFloracion = async (lote) => {
    const val = floracionDraft[lote.id];
    if(val===undefined) return;
    setSavingFloracion(true);
    try {
      await supabase.from("lotes").update({fecha_inicio_floracion: val || null}).eq("id", lote.id);
      setFloracionDraft(prev=>{ const n={...prev}; delete n[lote.id]; return n; });
      setStatus("guardado"); setTimeout(()=>setStatus(""),1800);
      cargarTodo();
      ...
      <div style={{padding:"20px 20px 110px"}}>
          ...
          return (
            <div className="fade">
              ...
              {subs.map(sub=>{
                ...
                return (
                  <div key={sub.id} style={card({borderColor:`${colorDeFase(sp.orden)}55`})}>
                          ...
                          return (
                            <div key={l.id} style={{background:"#111e11",borderRadius:"8px",padding:"10px 12px",border:`1px solid ${g?.color||"#2a4a2a"}44`}}>
                              ...
                              {sp.nombre==="Floración" && !esEjecutor && (
                                <div style={{background:"#1a1024",borderRadius:"7px",padding:"8px 10px",marginBottom:"8px",border:"1px solid #3a2a4a"}}>
                                  <div style={{fontSize:"10px",color:"#a77dc2",letterSpacing:"0.5px",marginBottom:"5px"}}>🌸 INICIO DE FLORACIÓN (12/12)</div>
                                  <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                                    <input type="date" value={floracionDraft[l.id]!==undefined?floracionDraft[l.id]:(l.fecha_inicio_floracion||"")} onChange={e=>setFloracionDraft({...floracionDraft,[l.id]:e.target.value})} style={{...inp(),padding:"6px 8px",fontSize:"12px",flex:1}}/>
                                    <button onClick={()=>guardarFechaFloracion(l)} disabled={savingFloracion} style={{padding:"6px 12px",borderRadius:"6px",fontSize:"11px",background:"#3a2a4a",color:"#c8a0e0",border:"1px solid #5a3a7a"}}>✓</button>
                                  </div>
                                  {!l.fecha_inicio_floracion && <div style={{fontSize:"10px",color:"#7a5a9a",marginTop:"5px"}}>Sin definir — cargala el día que la carpa llega a 12/12.</div>}
                                </div>
                              )}
                              {sp.nombre==="Floración" && esEjecutor && l.fecha_inicio_floracion && (
                                <div style={{fontSize:"11px",color:"#a77dc2",marginBottom:"6px"}}>🌸 Flora desde {l.fecha_inicio_floracion}</div>
                              )}
                              <div style={{display:"flex",gap:"6px"}}>
                                ...
                                {!esEjecutor && sigPadre && (
                                  <button onClick={()=>{setMovingLote(l); setDestinoSub(""); setMovingMovida(String(l.cantidad_plantas));}} style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:`${colorDeFase(sigPadre.orden)}22`,color:colorDeFase(sigPadre.orden),border:`1px solid ${colorDeFase(sigPadre.orden)}66`}}>→ Avanzar a {sigPadre.nombre}</button>
                                )}
                                {!esEjecutor && !sigPadre && (
        ...
        {tab==="_reportes" && !esEjecutor && (()=>{
          ...
          const tiemposPorLote = (lote) => {
            ...
            const sub = sectores.find(s=>s.id===lote.sector_actual_id);
            const spActual = sub ? sectoresPadre.find(x=>x.id===sub.sector_padre_id) : null;
            if(movs.length===0){
              // Sin historial: solo sabemos cuánto lleva desde su alta, en su etapa actual
              // Excepción: si está en Floración y tiene fecha 12/12 cargada, ese tramo es exacto
              if(spActual?.nombre==="Floración" && lote.fecha_inicio_floracion){
                return [{ etapa:"Floración", dias: diasEntre(lote.fecha_inicio_floracion, hoy), estimado:false }];
              }
              return [{ etapa: spActual?.nombre||"—", dias: diasEntre(lote.fecha_inicio, hoy), estimado:true }];
            }
            const tramos = [];
            ...
            if(spOrigen) tramos.push({ etapa: spOrigen.nombre, dias: diasEntre(lote.fecha_inicio, movs[0].fecha), estimado:false });
            for(let i=0;i<movs.length;i++){
              ...
              const finTramo = i<movs.length-1 ? movs[i+1].fecha : hoy;
              if(!spDest) continue;
              if(spDest.nombre==="Floración"){
                // La floración real arranca cuando el grower carga 12/12, no al entrar a la carpa
                if(lote.fecha_inicio_floracion){
                  tramos.push({ etapa:"Floración", dias: diasEntre(lote.fecha_inicio_floracion, finTramo), estimado:false });
                } else {
                  tramos.push({ etapa:"Floración", dias: diasEntre(movs[i].fecha, finTramo), estimado:true });
                }
              } else {
                tramos.push({ etapa: spDest.nombre, dias: diasEntre(movs[i].fecha, finTramo), estimado: i===movs.length-1 });
              }
            }
            const porEtapa = {};
            tramos.forEach(t=>{ porEtapa[t.etapa] = porEtapa[t.etapa] || {etapa:t.etapa, dias:0, estimado:false}; porEtapa[t.etapa].dias += t.dias; if(t.estimado) porEtapa[t.etapa].estimado=true; });
            return Object.values(porEtapa);
          };

          // Promedio por genética: promedia los días por etapa de todos sus lotes
          ...
          const lotesConHistorial = lotes.filter(l=>["activo","maduracion","cosechado"].includes(l.estado) && l.genetica_id);
          const promedios = promedioPorGenetica();

          // Producción por genética (gramos cosechados)
          const produccionGen = {};
          lotes.filter(l=>l.estado==="cosechado").forEach(l=>{
            if(!l.genetica_id) return;
            produccionGen[l.genetica_id] = (produccionGen[l.genetica_id]||0) + (l.peso_cosechado_gramos||0);
          });
          const produccionOrdenada = Object.entries(produccionGen).map(([gid,g])=>({g:geneticas.find(x=>x.id===gid),gramos:g})).filter(x=>x.g).sort((a,b)=>b.gramos-a.gramos);
          const maxProd = Math.max(1, ...produccionOrdenada.map(x=>x.gramos));
          ...
          return (
            <div className="fade">
              ...
              <Reporte id="produccion" titulo="⚖️ Producción por genética (gramos)" color="#c8a020" vacio={produccionOrdenada.length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin cosechas registradas todavía.</div>}>
                {produccionOrdenada.map((x,i)=>(
                  <div key={i} style={{marginBottom:"10px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",marginBottom:"4px"}}>
                      <span style={{color:x.g.color||"#c8a020"}}>{x.g.nombre}</span>
                      <span style={{color:"#e0c060",fontWeight:"600"}}>{x.gramos} g</span>
                    </div>
                    <div style={{height:"8px",background:"#1a1408",borderRadius:"4px",overflow:"hidden"}}><div style={{width:`${(x.gramos/maxProd*100)}%`,height:"100%",background:"#c8a020"}}/></div>
                  </div>
                ))}
              </Reporte>
              ...
              <Reporte id="por_lote" titulo="⏱ Tiempo por etapa (por lote)" color="#16a085" vacio={lotesConHistorial.length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin lotes para analizar.</div>}>
                <div style={{fontSize:"10px",color:"#5a7a6a",marginBottom:"8px"}}>E=Enraizado · V=Vegetativo · F=Floración · S=Secado · ≈ estimado</div>
                <div style={{display:"flex",flexDirection:"column",gap:"2px"}}>
                  {lotesConHistorial.map(l=>{
                    const g = geneticas.find(x=>x.id===l.genetica_id);
                    const tiempos = tiemposPorLote(l);
                    const porEtapa = {};
                    tiempos.forEach(t=>{ porEtapa[t.etapa]=t; });
                    const ini = {"Enraizado":"E","Vegetativo":"V","Floración":"F","Secado":"S"};
                    return (
                      <div key={l.id} style={{display:"flex",alignItems:"center",gap:"8px",padding:"8px 6px",borderBottom:"1px solid #142414"}}>
                        <span style={{fontSize:"11px",color:g?.color||"#9aaa9a",flex:"0 0 42%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g?.nombre||"—"}{l.numero_lote?` ${loteLabel(l.numero_lote, sufijoDeLote(l)).replace("Lote N° ","#")}`:""}</span>
                        <div style={{flex:1,display:"flex",gap:"4px",flexWrap:"wrap",justifyContent:"flex-end"}}>
                          {sectoresPadre.map(sp=>{
                            const t = porEtapa[sp.nombre];
                            if(!t) return null;
                            return <span key={sp.id} style={{fontSize:"10px",background:"#142a14",borderRadius:"5px",padding:"2px 6px",color:"#acd0b0",whiteSpace:"nowrap"}}>{ini[sp.nombre]||sp.nombre[0]} {t.dias}d{t.estimado?"≈":""}</span>;
                          })}
                        </div>
                      </div>
                    );
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"#182818",border:"1px solid #2a5a2a",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              ...
              <div style={{...lbl("#5a9a5a"),marginBottom:"6px"}}>CANTIDAD A AVANZAR</div>
              ...
              <div style={{fontSize:"11px",color:"#5a8a5a",marginBottom:"16px"}}>{(parseInt(movingMovida)||0) < movingLote.cantidad_plantas && (parseInt(movingMovida)||0)>0 ? `Quedan ${movingLote.cantidad_plantas-(parseInt(movingMovida)||0)} plantas en ${spOrigen?.nombre} — se crea un lote hermano en el destino` : "Avanza el lote completo"}</div>
              ...
              <div style={{display:"flex",gap:"10px"}}>
                ...
                <button onClick={()=>{setMovingLote(null);setDestinoSub("");setMovingMovida("");}} style={{padding:"12px 18px",borderRadius:"10px",fontSize:"14px",background:"#1a1a1a",color:"#7a7a7a",border:"1px solid #2a2a2a"}}>Cancelar</button>
              </div>
            </div>
          </div>
        );
