...
function RaizApp() {
  ...
  const cargarTodo = async (salaIdOverride) => {
    try {
      const sId = salaIdOverride || usuario.sala_id;
      if(!sId) return;
      setSalaId(sId);
  ...
  const saveTipoTarea = async () => {
    ...
    try {
      ...
      if(editingTipoTarea.id) await supabase.from("tipos_tarea").update(payload).eq("id", editingTipoTarea.id);
      else await supabase.from("tipos_tarea").insert(payload);
  ...
  if(!usuario) return modoAuth==="login"
            ...
            <div style={card({background:"#1a1608",borderColor:"#5a4a20"})}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
                {entidadesSinUso.map(ent=>(
                  <div key={ent.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#0e0e22",borderRadius:"8px",padding:"8px 12px"}}>
