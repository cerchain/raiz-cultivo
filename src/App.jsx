...
function RaizApp() {
  ...
  const TareaCard = ({t}) => {
    ...
    const loteTrasplante = esTareaTrasplanteConLote ? lotes.find(l=>l.sector_actual_id===t.sector_id && l.numero_lote===t.numero_lote_trasplante && l.estado==="activo") : null;
  ...
  const saveTarea = async () => {
    ...
    try {
      ...
      const base = {
        sala_id: salaId,
        titulo: (editingTarea.titulo||"").trim() || tipoObjBase?.nombre || "Tarea",
                    ...
                    {abierto && (
                      <div style={{padding:"10px",display:"flex",flexDirection:"column",gap:"8px"}}>
                        {pendientesSector.map(t=>{
                          ...
                          const loteTraspSector = esTraspSector ? lotes.find(l=>l.sector_actual_id===t.sector_id && l.numero_lote===t.numero_lote_trasplante && l.estado==="activo") : null;
        ...
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200,overflowY:"auto"}}>
            <div style={{background:"var(--c139)",border:"1px solid var(--c028)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px",margin:"auto"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                <div>
                  <div style={{...lbl("var(--c021)"),marginBottom:"6px"}}>TÍTULO (opcional)</div>
                  <input value={editingTarea.titulo} onChange={e=>setEditingTarea({...editingTarea,titulo:e.target.value})} placeholder="Si lo dejás vacío, se usa el nombre del tipo" style={inp()}/>
                </div>
                {esNueva ? (
                  <div>
