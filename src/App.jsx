...
function RaizApp() {
  ...
  const [repLotesFiltro, setRepLotesFiltro] = useState({etapa:"", sub:"", estado:"todos"});
  ...
  const emptyTipoTarea = () => ({ nombre:"", icono:"📋", sector_padre_id:"", activo:true });
  ...
  const saveTipoTarea = async () => {
    ...
    try {
      const payload = {
        ...
        sector_padre_id: editingTipoTarea.sector_padre_id || null,
  ...
  // Tipos disponibles para un subsector dado: los generales (sin sector_padre) + los específicos de su sector padre
  const tiposTareaParaSector = (sectorId) => {
    const sub = sectores.find(s=>s.id===sectorId);
    const spId = sub?.sector_padre_id;
    return tiposTarea.filter(tt => tt.activo!==false && (!tt.sector_padre_id || tt.sector_padre_id===spId));
  };
  ...
  const asignarmeComoMG = async (sala) => {
    setAsignandoComoMG(true);
    try {
    ...
    return (
      <div style={{background:"#0a0e1a",minHeight:"100vh",fontFamily:"'IBM Plex Mono',monospace",color:"#dfe0f0"}}>
        ...
        <div style={{padding:"18px"}}>
          ...
          <div style={{fontSize:"10px",color:"#5a6a9a",letterSpacing:"2px",marginBottom:"12px"}}>SALAS ({entidadSalas.length})</div>
                ...
                <div key={sala.id} style={{background:"#12162a",border:"1px solid #2a2a5a",borderRadius:"14px",padding:"16px"}}>
                  ...
                  <div style={{display:"flex",gap:"8px",flexWrap:"wrap",marginTop:"10px"}}>
                    ...
                    {!sala.master_grower_id && (
                      <button disabled={asignandoComoMG} onClick={()=>asignarmeComoMG(sala)} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"10px",background:"#1a1a2e",color:"#7a8af0",border:"1px solid #3a3a7a"}}>{asignandoComoMG?"Asignando...":"🌱 Asignarme como Master Grower"}</button>
                    )}
                  </div>
                </div>
              );
    ...
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
      ...
      <div style={{padding:"20px 20px 110px"}}>
                          ...
                          return (
                            <div key={l.id} style={{background:"#111e11",borderRadius:"8px",padding:"10px 12px",border:`1px solid ${g?.color||"#2a4a2a"}44`}}>
                              ...
                              {sp.nombre==="Floración" && sub.fecha_inicio_floracion && (()=>{
                                const g2 = geneticas.find(x=>x.id===l.genetica_id);
                                const minD = g2?.dias_floracion_min || (g2?.semanas_floracion ? g2.semanas_floracion*7 : null);
                                const maxD = g2?.dias_floracion_max || minD;
                                if(!minD) return null;
                                const base = new Date(sub.fecha_inicio_floracion);
                                const f1 = new Date(base); f1.setDate(f1.getDate()+minD);
                                const f2 = new Date(base); f2.setDate(f2.getDate()+maxD);
                                const rango = minD===maxD ? f1.toISOString().slice(0,10) : `${f1.toISOString().slice(0,10)} – ${f2.toISOString().slice(0,10)}`;
                                return <div style={{fontSize:"11px",color:"#c8a020",marginBottom:"6px"}}>🗓 Estimado corte: {rango} ({minD===maxD?`${minD}d`:`${minD}–${maxD}d`})</div>;
                              })()}
                              <div style={{display:"flex",gap:"6px"}}>
                                {!esEjecutor && (
        ...
        {tab==="_misala" && !esEjecutor && (
          <div className="fade">
            ...
            <div style={card({borderColor:"#0e6a5a",background:"#0a1e1a"})}>
              <div onClick={()=>setTiposTareaAbierto(!tiposTareaAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:tiposTareaAbierto?"12px":"0"}}>
                <span style={{...lbl("#16a085"),margin:0}}>TIPOS DE TAREA <span style={{color:"#5a9a8a",fontWeight:"400"}}>· {tiposTarea.length}</span></span>
                <span style={{fontSize:"13px",color:"#16a085",transform:tiposTareaAbierto?"rotate(180deg)":"none",transition:"transform .2s"}}>▾</span>
              </div>
              {tiposTareaAbierto && (
              <>
              {editingTipoTarea ? (
                <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                  ...
                  <div>
                    <div style={{...lbl("#5aa090"),marginBottom:"6px"}}>SECTOR (OPCIONAL — vacío = aplica a todos)</div>
                    <select value={editingTipoTarea.sector_padre_id} onChange={e=>setEditingTipoTarea({...editingTipoTarea,sector_padre_id:e.target.value})} style={inp()}>
              ...
              {tiposTarea.length>0 && (
                <div style={{display:"flex",flexDirection:"column",gap:"8px",marginTop:"12px"}}>
                  {tiposTarea.map(tt=>{
                    const sp = tt.sector_padre_id ? sectoresPadre.find(x=>x.id===tt.sector_padre_id) : null;
                    return (
                      <div key={tt.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#0a1814",borderRadius:"10px",padding:"10px 14px",opacity:tt.activo===false?0.5:1}}>
                        <div>
                          ...
                          <div style={{fontSize:"11px",color:"#5a9a8a"}}>{sp?sp.nombre:"Todos los sectores"}</div>
                        </div>
                        <div style={{display:"flex",gap:"6px"}}>
                          <button onClick={()=>setEditingTipoTarea({...tt, sector_padre_id:tt.sector_padre_id||""})} style={{padding:"6px 10px",borderRadius:"7px",fontSize:"11px",background:"#1a2e1a",color:"#7ec850",border:"1px solid #2a5a2a"}}>✏️</button>
            ...
            <div style={card({borderColor:"#6a3a8a",background:"#1a0e22"})}>
              <div onClick={()=>setUsuariosSalaAbierto(!usuariosSalaAbierto)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:usuariosSalaAbierto?"12px":"0"}}>
                <span style={{...lbl("#a86ad0"),margin:0}}>👥 USUARIOS DE TU SALA <span style={{color:"#7a5a9a",fontWeight:"400"}}>· {usuariosLista.length}</span></span>
        ...
        {tab==="_reportes" && !esEjecutor && (()=>{
          const hoy = new Date();
          const diasEntre = (d1, d2) => Math.max(0, Math.round((new Date(d2)-new Date(d1))/86400000));

          // Movimientos de un lote ordenados por fecha (su recorrido por etapas)
          const movsDeLote = (loteId) => movimientos.filter(m=>m.lote_id===loteId).sort((a,b)=>new Date(a.fecha)-new Date(b.fecha));

          // Tiempo por etapa de UN lote: usa movimientos; si no hay historial completo, estima desde fecha_inicio
          const tiemposPorLote = (lote) => {
            const movs = movsDeLote(lote.id);
            const tramos = [];

            // Etapa de origen (Enraizado normalmente) -> primer movimiento: terminada porque hubo un movimiento que la cerró
            if(movs.length>0){
              const primerOrigen = sectores.find(s=>s.id===movs[0].sector_origen_id);
              const spOrigen = primerOrigen ? sectoresPadre.find(x=>x.id===primerOrigen.sector_padre_id) : null;
              if(spOrigen && spOrigen.nombre!=="Floración"){
                tramos.push({ etapa: spOrigen.nombre, dias: diasEntre(lote.fecha_inicio, movs[0].fecha) });
              }
            }

            // Tramos intermedios: solo si hay un movimiento siguiente que cierra la etapa (la última/actual queda afuera)
            for(let i=0;i<movs.length-1;i++){
              const destino = sectores.find(s=>s.id===movs[i].sector_destino_id);
              const spDest = destino ? sectoresPadre.find(x=>x.id===destino.sector_padre_id) : null;
              if(!spDest || spDest.nombre==="Floración") continue; // Floración se calcula aparte, con las fechas congeladas al cortar
              tramos.push({ etapa: spDest.nombre, dias: diasEntre(movs[i].fecha, movs[i+1].fecha) });
            }

            // Floración: solo cuenta si el lote ya fue cortado (tiene las dos fechas congeladas por el corte)
            if(lote.fecha_inicio_floracion && lote.fecha_fin_floracion){
              tramos.push({ etapa:"Floración", dias: diasEntre(lote.fecha_inicio_floracion, lote.fecha_fin_floracion) });
            }

            const porEtapa = {};
            tramos.forEach(t=>{ porEtapa[t.etapa] = porEtapa[t.etapa] || {etapa:t.etapa, dias:0}; porEtapa[t.etapa].dias += t.dias; });
            return Object.values(porEtapa);
          };

          // Promedio por genética: promedia los días por etapa, solo de etapas ya terminadas
          const promedioPorGenetica = () => {
            const acc = {}; // genId -> etapa -> {suma, n}
            lotes.forEach(l=>{
              if(!l.genetica_id) return;
              tiemposPorLote(l).forEach(t=>{
                acc[l.genetica_id] = acc[l.genetica_id]||{};
                acc[l.genetica_id][t.etapa] = acc[l.genetica_id][t.etapa]||{suma:0,n:0};
                acc[l.genetica_id][t.etapa].suma += t.dias;
                acc[l.genetica_id][t.etapa].n += 1;
              });
            });
            return acc;
          };
          ...
          return (
            <div className="fade">
              ...
              <Reporte id="prom_genetica" titulo="⏱ Tiempo promedio por etapa (por genética)" color="#16a085" vacio={Object.keys(promedios).length===0 && <div style={{fontSize:"13px",color:"#5a7a5a"}}>Sin datos de movimientos todavía.</div>}>
                <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
