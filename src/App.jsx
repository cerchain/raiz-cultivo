...
function RaizApp() {
  ...
  const [repLotesFiltro, setRepLotesFiltro] = useState({etapa:"", sub:"", estado:"todos", genetica:""});
    ...
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
      ...
      <div style={{padding:"20px 20px 110px"}}>
        ...
        {tab==="_reportes" && !esEjecutor && (()=>{
          // Promedio por genética, sobre stats_lotes (RPC): misma fuente de verdad que
          // las tarjetas y el Reporte info. LOTES. Solo promedia etapas CERRADAS:
          // la etapa en curso de un lote activo se excluye para no deprimir el promedio.
          const promedioPorGenetica = () => {
            ...
            lotes.forEach(l=>{
              ...
              const st = statsLotes[l.id];
              if(!st) return;
              // Etapas todavía abiertas para este lote (no se promedian)
              let abiertas = [];
              if(["activo","maduracion"].includes(l.estado)){
                const sAct = sectores.find(x=>x.id===l.sector_actual_id);
                const spAct = sAct ? sectoresPadre.find(x=>x.id===sAct.sector_padre_id)?.nombre : null;
                if(spAct==="Floración"){
                  const con1212 = l.fecha_inicio_floracion || sAct?.fecha_inicio_floracion;
                  // Sin 12/12 la planta sigue vegetando: veg abierta. Con 12/12, veg quedó congelada.
                  abiertas = con1212 ? ["Floración"] : ["Vegetativo","Floración"];
                } else if(spAct) abiertas = [spAct];
              }
              const dias = {
              ...
              etapasNom.forEach(et=>{
                if(abiertas.includes(et)) return;   // etapa en curso: no cierra todavía
                if(!(dias[et]>0)) return;           // etapa que este lote no vivió
