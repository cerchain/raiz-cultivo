...
function RaizApp() {
  ...
  // Selector de tema compacto, para los paneles (entidad / super admin) que no
  // tienen el menú "Más" donde vive la pantalla de Apariencia.
  const SelectorTemaMini = () => (
    <div style={{display:"flex",gap:"3px",background:"var(--c103)",borderRadius:"12px",padding:"3px"}}>
      {[["oscuro","🌙"],["claro","☀️"],["sistema","⚙️"]].map(([k,ic])=>(
        <button key={k} onClick={()=>cambiarTema(k)} title={k}
          style={{fontSize:"12px",lineHeight:"1",padding:"5px 8px",borderRadius:"10px",cursor:"pointer",
                  background:temaPref===k?"var(--c015)":"transparent",
                  border:"1px solid "+(temaPref===k?"var(--c015)":"transparent"),
                  opacity:temaPref===k?1:0.55}}>{ic}</button>
      ))}
    </div>
  );
