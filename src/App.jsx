...
function RaizApp() {
  ...
  const [diaRelevamientoAbierto, setDiaRelevamientoAbierto] = useState(""); // true = mover libre (corrección), false = avanzar etapa normal
  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
                              ...
                              <div style={{display:"flex",gap:"6px"}}>
                                ...
                                {!esEjecutor && sigPadre && (
                                  (sp.nombre==="Floración" && !sub.fecha_inicio_floracion) ? (
                                    ...
                                    <button onClick={()=>{setMovingLote(l); setDestinoSub(""); setMovingMovida(String(l.cantidad_plantas)); setModoMover(false);}} style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:`${colorDeFase(sigPadre.orden)}22`,color:colorDeFase(sigPadre.orden),border:`1px solid ${colorDeFase(sigPadre.orden)}66`}}>→ Avanzar a {sigPadre.nombre}</button>
                                  )
                                )}
                                {!esEjecutor && !sigPadre && (
                                ...
                                {!esEjecutor && !(sp.nombre==="Floración" && sub.fecha_inicio_floracion) && (
                                  <button onClick={()=>{setMovingLote(l); setDestinoSub(""); setMovingMovida(String(l.cantidad_plantas)); setModoMover(true);}} title="Mover a otro subsector (corrección)" style={{padding:"7px 10px",borderRadius:"7px",fontSize:"11px",background:"#1a2030",color:"#7a9ad0",border:"1px solid #2a3a5a"}}>↔</button>
                                )}
                              </div>
                              {sp.nombre==="Floración" && sub.fecha_inicio_floracion && (
      ...
      {/* ══ MODAL: MOVER LOTE (avanzar etapa normal, o mover libre como corrección) ══ */}
      {movingLote && (()=>{
        ...
        const cerrar = () => { setMovingLote(null); setDestinoSub(""); setMovingMovida(""); setModoMover(false); };
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"#182818",border:"1px solid #2a5a2a",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"19px",color:"#7ec850",marginBottom:"6px"}}>{modoMover ? "↔ Mover lote" : "Avanzar etapa"}</div>
              <div style={{fontSize:"13px",color:"#8aaa8a",marginBottom:"16px"}}>{g?.nombre} · {movingLote.cantidad_plantas} plantas{!modoMover && ` → ${spDestinoNatural?.nombre}`}</div>
              ...
              <div style={{...lbl("#5a9a5a"),marginBottom:"6px"}}>CANTIDAD A {modoMover?"MOVER":"AVANZAR"}</div>
              ...
              <div style={{fontSize:"11px",color:"#5a8a5a",marginBottom:"16px"}}>{(parseInt(movingMovida)||0) < movingLote.cantidad_plantas && (parseInt(movingMovida)||0)>0 ? `Quedan ${movingLote.cantidad_plantas-(parseInt(movingMovida)||0)} plantas en ${spOrigen?.nombre} — se crea un lote hermano en el destino` : `${modoMover?"Mueve":"Avanza"} el lote completo`}</div>
