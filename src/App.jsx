...
function RaizApp() {
  ...
  const [aceptaCarpaEnFlo, setAceptaCarpaEnFlo] = useState(false); // confirmación explícita para entrar a una carpa ya en 12/12
  ...
  // Una carpa de Floración con el 12/12 YA activado no puede recibir plantas nuevas:
  // todas las plantas de esa carpa comparten el mismo reloj de floración, así que meter
  // una planta a mitad de ciclo le asignaría días de flora que no le corresponden.
  // Se libera sola cuando la carpa se vacía (el 12/12 se resetea).
  // Días que lleva una carpa en 12/12. 0 si arrancó hoy o si no tiene fecha.
  const diasEnFloracionDeCarpa = (sub) => {
    if(!sub?.fecha_inicio_floracion) return 0;
    return Math.max(0, Math.floor((new Date() - new Date(sub.fecha_inicio_floracion+"T00:00:00")) / 86400000));
  };

  const carpaCerradaAlIngreso = (sub) => {
    if(!sub) return false;
    const sp = sectoresPadre.find(x=>x.id===sub.sector_padre_id);
    if(sp?.nombre!=="Floración") return false;
    if(!sub.fecha_inicio_floracion) return false;
    // Si la carpa arrancó 12/12 HOY, el desfasaje es cero: no tiene sentido advertir
    // ni pedir confirmación. Recién a partir del primer día completo se considera cerrada.
    if(diasEnFloracionDeCarpa(sub)===0) return false;
    // Una carpa VACÍA con una fecha de 12/12 colgada de un ciclo anterior NO está
    // cerrada: esa fecha es residuo, no un ciclo en curso. Solo bloquea si de verdad
    // hay plantas adentro compartiendo ese reloj de floración.
    const tienePlantas = lotes.some(l=>l.sector_actual_id===sub.id && (l.estado==="activo"||l.estado==="maduracion") && (l.cantidad_plantas||0)>0);
    return tienePlantas;
  };
  ...
  // Numeración de lotes ATÓMICA en la base (contadores_genetica + siguiente_numero_lote).
  // Antes esto calculaba MAX+1 sobre el array local `lotes`, lo que tenía condición de
  // carrera: dos altas simultáneas de la misma genética calculaban el mismo número.
  // Devuelve null si falla — quien lo llama DEBE cortar y avisar, nunca seguir con un
  // número inventado.
  const siguienteNumeroLote = async (genId) => {
    const r = await supabase.rpc("siguiente_numero_lote", { p_genetica_id: genId });
    if(r.error){ console.error("siguiente_numero_lote:", r.error); return null; }
    return r.data;
  };
  ...
  const saveCosecha = async () => {
    ...
    try {
      ...
      let numCosecha = null;
      if(!editingCosecha.id){
        numCosecha = await siguienteNumeroLote(payload.genetica_id);
        if(numCosecha===null){ setStatus("no se pudo asignar número de lote — probá de nuevo"); setSavingCosecha(false); setTimeout(()=>setStatus(""),6000); return; }
      }
      const rc = editingCosecha.id
        ? await supabase.from("lotes").update(payload).eq("id", editingCosecha.id)
        : await supabase.from("lotes").insert({...payload, numero_lote: numCosecha});
      if(rc.error){ setStatus("error al guardar la cosecha: "+rc.error.message.slice(0,80)); setSavingCosecha(false); setTimeout(()=>setStatus(""),6000); return; }
  ...
  const delCosecha = async (id) => {
    try { const d=await supabase.from("lotes").delete().eq("id", id); if(d.error){ setStatus("no se pudo borrar: "+d.error.message.slice(0,70)); setTimeout(()=>setStatus(""),6000); return; } cargarTodo(); }
  ...
  const saveLote = async () => {
    ...
    try {
      ...
      if(editingLote.id){
        ...
        const updL = await supabase.from("lotes").update(payload).eq("id", editingLote.id);
        if(updL.error){ setStatus("error al guardar el lote: "+updL.error.message.slice(0,80)); setSavingLote(false); setTimeout(()=>setStatus(""),6000); return; }
        if(eraPendiente){
          const mvE = await supabase.from("movimientos_lote").insert({
          ...
          if(mvE.error) console.error("movimiento de aprobación:", mvE.error);
        }
      } else {
        // Alta de lote nuevo — número secuencial propio de esta genética
        const numNuevo = await siguienteNumeroLote(editingLote.genetica_id);
        if(numNuevo===null){ setStatus("no se pudo asignar número de lote — probá de nuevo"); setSavingLote(false); setTimeout(()=>setStatus(""),6000); return; }
        const ins = await supabase.from("lotes").insert({
          ...payload,
          numero_lote: numNuevo
        }).select().single();
        if(ins.error || !ins.data){ setStatus("no se pudo crear el lote: "+((ins.error&&ins.error.message)||"error desconocido").slice(0,80)); setSavingLote(false); setTimeout(()=>setStatus(""),7000); return; }
        {
          const mvA = await supabase.from("movimientos_lote").insert({
            lote_id: ins.data.id,
            sector_origen_id: null,
            sector_destino_id: editingLote.sector_actual_id,
            cantidad_movida: nuevaCantidad,
          ...
          if(mvA.error) console.error("movimiento de alta:", mvA.error);
        }
      }
  ...
  const delLote = async (id) => {
    ...
    try {
      const dM = await supabase.from("movimientos_lote").delete().eq("lote_id", id);
      if(dM.error){ setStatus("no se pudo borrar el historial del lote: "+dM.error.message.slice(0,70)); setTimeout(()=>setStatus(""),6000); return; }
      const dL = await supabase.from("lotes").delete().eq("id", id);
      if(dL.error){ setStatus("no se pudo borrar el lote: "+dL.error.message.slice(0,70)); setTimeout(()=>setStatus(""),6000); return; }
      setEditingLote(null);
      cargarTodo();
  ...
  const confirmBaja = async () => {
    ...
    try {
      ...
      const uB = await supabase.from("lotes").update({
        cantidad_plantas: restante,
        estado: esBajaTotal ? "descartado" : bajaLote.lote.estado
      }).eq("id", bajaLote.lote.id);
      if(uB.error){ setStatus("error al dar de baja: "+uB.error.message.slice(0,80)); setSavingBaja(false); setTimeout(()=>setStatus(""),6000); return; }

      const mB = await supabase.from("movimientos_lote").insert({
      ...
      if(mB.error) setStatus("baja aplicada, pero no se registró en el historial: "+mB.error.message.slice(0,60));
  ...
  const pasarAMaduracion = async (l) => {
    try {
      const uM = await supabase.from("lotes").update({estado:"maduracion"}).eq("id", l.id);
      if(uM.error){ setStatus("error al pasar a maduración: "+uM.error.message.slice(0,80)); setTimeout(()=>setStatus(""),6000); return; }
      const mM = await supabase.from("movimientos_lote").insert({
        lote_id: l.id,
      ...
      if(mM.error) console.error("movimiento maduración:", mM.error);
      setStatus("guardado"); setTimeout(()=>setStatus(""),1800);
      cargarTodo();
  ...
  const confirmFinalizar = async () => {
    ...
    setSavingFinal(true);
    try {
      const uF = await supabase.from("lotes").update({
        estado: "cosechado",
      ...
      if(uF.error){ setStatus("error al finalizar la cosecha: "+uF.error.message.slice(0,80)); setSavingFinal(false); setTimeout(()=>setStatus(""),6000); return; }
      const mF = await supabase.from("movimientos_lote").insert({
      ...
      if(mF.error) console.error("movimiento de cosecha:", mF.error);
  ...
  const guardarFechaFloracion = async (sub) => {
    ...
    try {
      const uFl = await supabase.from("sectores").update({fecha_inicio_floracion: val || null}).eq("id", sub.id);
      if(uFl.error){ setStatus("no se pudo guardar el 12/12: "+uFl.error.message.slice(0,80)); setSavingFloracion(false); setTimeout(()=>setStatus(""),6000); return; }
  ...
  const confirmarMovimiento = async () => {
    ...
    const subDest = sectores.find(s=>s.id===destinoSub);
    if(carpaCerradaAlIngreso(subDest) && !aceptaCarpaEnFlo){ setStatus(`${subDest.nombre} está en 12/12 — tildá la casilla de confirmación para continuar igual`); setTimeout(()=>setStatus(""),6000); return; }
    // Si el destino está vacío pero arrastra una fecha de 12/12 de un ciclo anterior,
    // se limpia: las plantas que entran arrancan un ciclo nuevo, no heredan el viejo.
    const entroACarpaEnFlo = carpaCerradaAlIngreso(subDest);
    const destVacio = subDest && !lotes.some(l=>l.sector_actual_id===subDest.id && (l.estado==="activo"||l.estado==="maduracion") && (l.cantidad_plantas||0)>0);
    if(subDest && destVacio && subDest.fecha_inicio_floracion){
      const limp = await supabase.from("sectores").update({fecha_inicio_floracion:null}).eq("id", subDest.id);
      if(limp.error) console.error("limpiar 12/12 residual del destino:", limp.error);
    }
    ...
    try {
      ...
      if(esTotal){
        ...
        const mvT = await supabase.from("movimientos_lote").insert({
          ...
          usuario_id: usuario.id,
          tipo: "movimiento",
          motivo: entroACarpaEnFlo ? `Ingreso a ${subDest.nombre} con 12/12 ya iniciado (${subDest.fecha_inicio_floracion}) — confirmado por ${usuario.nombre}` : null
        });
        if(mvT.error) setStatus("lote movido, pero no se registró en el historial: "+mvT.error.message.slice(0,60));
        ...
        const numSplit = await siguienteNumeroLote(movingLote.genetica_id);
        if(numSplit===null){ setStatus("no se pudo asignar número al lote nuevo — probá de nuevo"); setSavingMovida(false); setTimeout(()=>setStatus(""),6000); return; }
        ...
        if(ins.error || !ins.data){
          ...
          const rb = await supabase.from("lotes").update({ cantidad_plantas: movingLote.cantidad_plantas }).eq("id", movingLote.id);
          if(rb.error){ setStatus("GRAVE: falló el destino Y el rollback. Verificá la cantidad del lote origen a mano."); setSavingMovida(false); setTimeout(()=>setStatus(""),15000); return; }
        ...
        }
        {
          ...
          const mvP = await supabase.from("movimientos_lote").insert({
            lote_id: ins.data.id,
            sector_origen_id: movingLote.sector_actual_id,
            sector_destino_id: destinoSub,
            cantidad_movida: cant,
            usuario_id: usuario.id,
            tipo: "movimiento",
            motivo: `Avance parcial de ${cant}/${movingLote.cantidad_plantas} del lote N° ${movingLote.numero_lote} — nace como lote N° ${ins.data.numero_lote}`+(entroACarpaEnFlo?` · Ingreso a ${subDest.nombre} con 12/12 ya iniciado (${subDest.fecha_inicio_floracion}) — confirmado por ${usuario.nombre}`:"")
          });
          if(mvP.error) setStatus("lote movido, pero no se registró en el historial: "+mvP.error.message.slice(0,60));
        }
      }
      ...
      if(esCorteDeFloracion && loteIdQueCorta){
        ...
        if((restantes.data||[]).length===0){
          const cl12 = await supabase.from("sectores").update({fecha_inicio_floracion:null}).eq("id", subOrigen.id);
        if(cl12.error) console.error("limpiar 12/12 de carpa:", cl12.error);
        }
      }
      ...
      if(tareaTrasplanteId){
        ...
        if(tareaOrig){
          ...
          const uTT = await supabase.from("tareas").update(payloadTarea).eq("id", tareaTrasplanteId);
          if(uTT.error) setStatus("trasplante hecho, pero no se pudo actualizar la O.T.: "+uTT.error.message.slice(0,60));
        }
      }
  ...
  const saveSub = async () => {
    ...
    try {
      ...
      const rS = editingSub.id
        ? await supabase.from("sectores").update(payload).eq("id", editingSub.id)
        : await supabase.from("sectores").insert(payload);
      if(rS.error){ setStatus("error al guardar el subsector: "+rS.error.message.slice(0,80)); setSavingSub(false); setTimeout(()=>setStatus(""),6000); return; }
      setEditingSub(null);
      setStatus("guardado"); setTimeout(()=>setStatus(""),1800);
      cargarTodo();
  ...
  const delSub = async (id) => {
    ...
    try { const d=await supabase.from("sectores").delete().eq("id", id); if(d.error){ setStatus("no se pudo borrar el subsector: "+d.error.message.slice(0,70)); setTimeout(()=>setStatus(""),6000); return; } cargarTodo(); }
  ...
  const toggleActivaGenetica = async (g) => {
    try { const d=await supabase.from("geneticas").update({activa: !(g.activa!==false)}).eq("id", g.id); if(d.error){ setStatus("no se pudo cambiar: "+d.error.message.slice(0,70)); setTimeout(()=>setStatus(""),5000); return; } cargarTodo(); }
  ...
  const toggleActivoTipoTarea = async (tt) => {
    try { const d=await supabase.from("tipos_tarea").update({activo: !(tt.activo!==false)}).eq("id", tt.id); if(d.error){ setStatus("no se pudo cambiar: "+d.error.message.slice(0,70)); setTimeout(()=>setStatus(""),5000); return; } cargarTodo(); }
    catch(e) { setStatus("error al actualizar"); setTimeout(()=>setStatus(""),3000); }
  };
  ...
  const delTipoTarea = async (tt) => {
    ...
    try { const d=await supabase.from("tipos_tarea").delete().eq("id", tt.id); if(d.error){ setStatus("no se pudo borrar: "+d.error.message.slice(0,70)); setTimeout(()=>setStatus(""),6000); return; } cargarTodo(); }
  ...
  const updateEstadoTarea = async (t, nuevoEstado) => {
    try {
      ...
      const uT = await supabase.from("tareas").update(payload).eq("id", t.id);
      if(uT.error){ setStatus("error al cambiar el estado de la O.T.: "+uT.error.message.slice(0,80)); setTimeout(()=>setStatus(""),6000); return; }
      cargarTodo();
  ...
  const delTarea = async (t) => {
    ...
    try {
      const dC = await supabase.from("comentarios_tareas").delete().eq("tarea_id", t.id);
      if(dC.error){ setStatus("no se pudieron borrar los comentarios: "+dC.error.message.slice(0,70)); setTimeout(()=>setStatus(""),6000); return; }
      const dT = await supabase.from("tareas").delete().eq("id", t.id);
      if(dT.error){ setStatus("no se pudo borrar la O.T.: "+dT.error.message.slice(0,70)); setTimeout(()=>setStatus(""),6000); return; }
      cargarTodo();
  ...
  const guardarRelevamiento = async () => {
    ...
    try {
      ...
      for(const {sid, linea} of items){
        ...
        if(!loteExistente){
          const numPend = await siguienteNumeroLote(linea.genetica_id);
          if(numPend===null){ setStatus("no se pudo asignar número de lote — probá de nuevo"); setSavingRelevamiento(false); setTimeout(()=>setStatus(""),6000); return; }
  ...
  const aplicarInventario = async (r) => {
    ...
    try {
      ...
      const mvI = await supabase.from("movimientos_lote").insert({
        lote_id: lote.id,
      ...
      if(mvI.error) console.error("movimiento de ajuste de inventario:", mvI.error);
  ...
  const saveUsuario = async () => {
    ...
    try {
      ...
      if(rU.error){ setStatus("error al guardar el usuario: "+rU.error.message.slice(0,80)); setSavingUsuario(false); setTimeout(()=>setStatus(""),6000); return; }
  ...
  const toggleActivoUsuario = async (u) => {
    ...
    try { const d=await supabase.from("usuarios").update({activo: !(u.activo!==false)}).eq("id", u.id); if(d.error){ setStatus("no se pudo cambiar: "+d.error.message.slice(0,70)); setTimeout(()=>setStatus(""),5000); return; } cargarTodo(); }
  ...
  const reactivarODesactivarUsuarioEntidad = async (u) => {
    try { const d=await supabase.from("usuarios").update({activo: !(u.activo!==false)}).eq("id", u.id); if(d.error){ setStatus("no se pudo cambiar: "+d.error.message.slice(0,70)); setTimeout(()=>setStatus(""),5000); return; } cargarPanelEntidad(); }
    catch(e) { setStatus("error al actualizar"); setTimeout(()=>setStatus(""),3000); }
  };
  ...
  const crearMasterGrowerAdmin = async (salaIdExistente) => {
    ...
    try {
      ...
      if(insU.error || !insU.data){ setStatus("no se pudo crear el Master Grower: "+((insU.error&&insU.error.message)||"error desconocido").slice(0,80)); setSavingMGAdmin(false); setTimeout(()=>setStatus(""),7000); return; }
      {
        if(salaIdExistente){
          // Asignar a una sala que estaba sin Master Grower
          const uMGS = await supabase.from("salas").update({master_grower_id: insU.data.id}).eq("id", salaIdExistente);
          if(uMGS.error){ setStatus("el MG se creó pero no se pudo asignar a la sala: "+uMGS.error.message.slice(0,70)); setSavingMGAdmin(false); setTimeout(()=>setStatus(""),8000); return; }
          ...
          if(insS.error || !insS.data){ setStatus("el MG se creó pero falló la sala: "+((insS.error&&insS.error.message)||"error").slice(0,70)); setSavingMGAdmin(false); setTimeout(()=>setStatus(""),8000); return; }
          {
            const uMGU = await supabase.from("usuarios").update({sala_id: insS.data.id}).eq("id", insU.data.id);
            if(uMGU.error) console.error("vincular MG a su sala:", uMGU.error);
            const iSP = await supabase.from("sectores_padre").insert([
              {sala_id: insS.data.id, nombre:"Enraizado", orden:1},
              {sala_id: insS.data.id, nombre:"Vegetativo", orden:2},
              {sala_id: insS.data.id, nombre:"Floración", orden:3},
              {sala_id: insS.data.id, nombre:"Secado", orden:4}
            ]);
            if(iSP.error){ setStatus("la sala se creó pero sin sectores: "+iSP.error.message.slice(0,70)); setSavingMGAdmin(false); setTimeout(()=>setStatus(""),8000); return; }
          }
        }
      }
  ...
  const eliminarEjecutorAdmin = async (u) => {
    ...
    try { const d=await supabase.from("usuarios").update({activo:false}).eq("id", u.id); if(d.error){ setStatus("no se pudo desactivar: "+d.error.message.slice(0,70)); setTimeout(()=>setStatus(""),5000); return; } cargarPanelAdmin(); }
    catch(e) { setStatus("error al desactivar"); setTimeout(()=>setStatus(""),3000); }
  };

  const reactivarUsuarioAdmin = async (u) => {
    try { const d=await supabase.from("usuarios").update({activo:true}).eq("id", u.id); if(d.error){ setStatus("no se pudo activar: "+d.error.message.slice(0,70)); setTimeout(()=>setStatus(""),5000); return; } cargarPanelAdmin(); }
  ...
  const confirmarEliminarSala = async () => {
    ...
    try {
      ...
      // Cascada en orden de dependencia. Cada paso se chequea: si uno falla, se corta
      // y se avisa EN QUÉ paso quedó, para poder retomar a mano sin adivinar.
      // (Nota de deuda técnica: esto debería ser una función SQL transaccional —
      //  hoy, si falla a mitad, no hay rollback automático de los pasos anteriores.)
      const pasos = [
        ["historial de movimientos", loteIds.length ? supabase.from("movimientos_lote").delete().in("lote_id", loteIds) : null],
        ["comentarios de O.T.",      tareaIds.length ? supabase.from("comentarios_tareas").delete().in("tarea_id", tareaIds) : null],
        ["inventarios",              supabase.from("relevamientos").delete().eq("sala_id", salaId)],
        ["órdenes de trabajo",       supabase.from("tareas").delete().eq("sala_id", salaId)],
        ["lotes",                    supabase.from("lotes").delete().eq("sala_id", salaId)],
        ["genéticas",                supabase.from("geneticas").delete().eq("sala_id", salaId)],
        ["tipos de O.T.",            supabase.from("tipos_tarea").delete().eq("sala_id", salaId)],
        ["subsectores",              supabase.from("sectores").delete().eq("sala_id", salaId)],
        ["sectores",                 supabase.from("sectores_padre").delete().eq("sala_id", salaId)],
        ["usuarios",                 supabase.from("usuarios").delete().eq("sala_id", salaId)],
        ["la sala",                  supabase.from("salas").delete().eq("id", salaId)]
      ];
      for(const [nombre, q] of pasos){
        if(!q) continue;
        const r = await q;
        if(r.error){ setStatus(`borrado incompleto — falló en: ${nombre}. ${r.error.message.slice(0,60)}`); setTimeout(()=>setStatus(""),12000); return; }
      }
  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"var(--c074)",minHeight:"100vh",color:"var(--c019)"}}>
      ...
      {movingLote && (()=>{
        ...
        // Las carpas en 12/12 con plantas SIGUEN apareciendo como destino, marcadas con 🔒.
        // Cristian pidió aviso + confirmación, no bloqueo duro: el grower sabe cuándo
        // necesita romper la regla, y un bloqueo absoluto empuja a falsear datos.
        ...
        const subDestElegido = sectores.find(s=>s.id===destinoSub);
        const destinoEnFloracion = carpaCerradaAlIngreso(subDestElegido);
        const g = geneticas.find(x=>x.id===movingLote.genetica_id);
        const cerrar = () => { setMovingLote(null); setDestinoSub(""); setMovingMovida(""); setModoMover(false); setModoRetroceso(false); setTareaTrasplanteId(null); setAceptaCarpaEnFlo(false); };
        return (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:200}}>
            <div style={{background:"var(--c073)",border:"1px solid var(--c011)",borderRadius:"16px",padding:"22px",width:"100%",maxWidth:"380px"}}>
              ...
              {subsDestino.length===0 && (
                <div style={{fontSize:"12px",color:"var(--c022)",background:"var(--c094)",borderRadius:"8px",padding:"10px 12px",marginBottom:"12px"}}>
                  No hay ningún subsector de destino configurado para esta etapa — creá uno en ⚙️ Mi Sala.
                </div>
              )}
              <select value={destinoSub} onChange={e=>{ setDestinoSub(e.target.value); setAceptaCarpaEnFlo(false); }} style={{...inp(),marginBottom:"16px"}}>
                <option value="">Seleccionar...</option>
                {subsDestino.map(s=>{
                  const sp = sectoresPadre.find(x=>x.id===s.sector_padre_id);
                  return <option key={s.id} value={s.id}>{carpaCerradaAlIngreso(s)?"🔒 ":""}{sp?.nombre} › {s.nombre}{carpaCerradaAlIngreso(s)?" (en 12/12)":""}</option>;
                })}
              </select>
              {destinoEnFloracion && (()=>{
                const diasEnFlo = diasEnFloracionDeCarpa(subDestElegido);
                return (
                  <div style={{background:"var(--c094)",border:"1px solid var(--c088)",borderRadius:"10px",padding:"12px 14px",marginBottom:"16px"}}>
                    <div style={{fontSize:"12px",fontWeight:"600",color:"var(--c088)",marginBottom:"6px"}}>⚠ {subDestElegido.nombre} ya está en 12/12</div>
                    <div style={{fontSize:"11px",color:"var(--c022)",lineHeight:"1.55"}}>
                      Esa carpa arrancó floración el {subDestElegido.fecha_inicio_floracion}, hace {diasEnFlo} d. Todas las plantas de una carpa comparten el mismo reloj, así que estas plantas van a figurar con {diasEnFlo} d de floración desde el día uno, y su fecha de corte va a salir desfasada.
                    </div>
                    <label style={{display:"flex",alignItems:"center",gap:"10px",marginTop:"10px",fontSize:"12px",color:"var(--c001)",cursor:"pointer"}}>
                      <input type="checkbox" checked={aceptaCarpaEnFlo} onChange={e=>setAceptaCarpaEnFlo(e.target.checked)} style={{width:"20px",height:"20px",flexShrink:0}}/>
                      Entiendo el desfasaje y quiero continuar
                    </label>
                  </div>
                );
              })()}
