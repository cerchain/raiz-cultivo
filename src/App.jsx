...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      try {
        const r = await supabase.from("usuarios").select("*").eq("email", localStorage.getItem("raiz_email")||"").single();
        if(r.data) setUsuario(r.data);
      } catch(e) {}
      try {
        const saved = localStorage.getItem(STORAGE_KEY);
        if(saved) { const m = migrate(saved); if(m) setDb(m); }
      } catch(e) {}
      setLoaded(true);
    })();
  },[]);
  ...
  const persist = d => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(d)); setStatus("guardado"); setTimeout(()=>setStatus(""),1800); }
  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
      ...
      <div style={{background:"#111e11",borderBottom:"2px solid #2a4a2a",padding:"18px 20px 14px",position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"14px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"6px"}}>
            ...
            {status==="error"    && <div style={{fontSize:"12px",color:"#e74c3c",background:"#2a1010",padding:"5px 14px",borderRadius:"20px",border:"1px solid #6a2020"}}>⚠ error</div>}
      ...
      <div style={{padding:"20px 20px 48px"}}>
            ...
            <div style={card()}>
              ...
              <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
                {[1,2,3,4,5,6,7,8].map(w=>(
                  <button key={w} onClick={()=>!esEjecutor && setWeek(tab,w)} style={{width:"50px",height:"50px",borderRadius:"12px",background:curFW===w?CARPA_COLOR[tab]:(w<curFW?"#1e2e1a":"#141e14"),color:curFW===w?"#fff":(w<curFW?"#4a7a4a":"#5a8a5a"),fontSize:"14px",fontWeight:curFW===w?"700":"400",border:curFW===w?"2px solid #ffb86a":"1px solid #2a4a2a",position:"relative",opacity:esEjecutor?0.7:1,cursor:esEjecutor?"default":"pointer"}}>
                    S{w}
        ...
        {tab==="vege" && (
          <div className="fade">
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"#2ecc71",marginBottom:"18px"}}>🌿 Vegetativo</div>
            <div style={{display:"flex",gap:"8px",marginBottom:"18px"}}>
              {[{k:"v150",l:"150 cc"},{k:"v2l",l:"2 L"},{k:"v5l",l:"5 L"}].map(s=>(
                <button key={s.k} onClick={()=>setVStage(s.k)} style={{padding:"9px 22px",borderRadius:"22px",fontSize:"14px",background:vStage===s.k?"#1e6a2e":"#141e14",color:vStage===s.k?"#dff0cf":"#5a8a5a",border:`1px solid ${vStage===s.k?"#2ecc71":"#2a4a2a"}`,fontWeight:vStage===s.k?"600":"400"}}>{s.l}</button>
              ))}
            </div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"14px"}}>
              <span style={{fontSize:"14px",color:"#5aaa5a"}}>Semana activa</span>
              <span style={{fontSize:"38px",fontWeight:"700",color:"#2ecc71",lineHeight:1}}>S{curVW}</span>
            </div>
            <div style={{display:"flex",gap:"8px",marginBottom:"18px",flexWrap:"wrap"}}>
              {VEGE_NUT[vStage].map(r=>(
                <button key={r.w} onClick={()=>!esEjecutor && setWeek(vStage,r.w)} style={{width:"48px",height:"48px",borderRadius:"12px",background:curVW===r.w?"#1e6a2e":"#141e14",color:curVW===r.w?"#fff":"#4a7a4a",fontSize:"14px",fontWeight:curVW===r.w?"700":"400",border:curVW===r.w?"2px solid #2ecc71":"1px solid #2a4a2a",opacity:esEjecutor?0.7:1,cursor:esEjecutor?"default":"pointer"}}>S{r.w}</button>
              ))}
            </div>
            {(()=>{
              const r=VEGE_NUT[vStage][curVW-1]; if(!r) return null;
              return(
                <>
                  <div style={card()}>
                    <span style={lbl("#4acc8a")}>NUTRICIÓN</span>
                    <div style={{...body,marginBottom:"12px"}}>{r.f}</div>
                    <div style={{display:"flex",gap:"20px"}}>
                      <span style={{fontSize:"14px",color:r.si?"#1abc9c":"#3a5a4a"}}>{r.si?"✓":"–"} Sílicio</span>
                      <span style={{fontSize:"14px",color:r.hu?"#8e44ad":"#3a4a5a"}}>{r.hu?"✓":"–"} Humus líquido</span>
                    </div>
                  </div>
                  {r.n && <div style={{background:"#141e14",border:"1px solid #2a5a2a",borderRadius:"12px",padding:"14px 16px",marginBottom:"16px",fontSize:"14px",color:"#7ec850",lineHeight:"1.7"}}>💡 {r.n}</div>}
                  <NB label={`NOTAS S${curVW} — ${vStage.toUpperCase()}`} value={db[vStage]?.notes?.[curVW]||""} onChange={t=>setNote(vStage,curVW,t)}/>
                </>
              );
            })()}
          </div>
        ...
        {tab==="enraizado" && (
          <div className="fade">
            <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"#27ae60",marginBottom:"18px"}}>✂️ Aeroclonadores 2×38</div>
            <div style={card({borderColor:"#1e4a2e"})}>
              <span style={lbl("#3aaa6a")}>SETUP</span>
              <div style={{...body,color:"#a0c890",lineHeight:"2.1"}}>
                🔧 2 aeroclonadores · 38 posiciones c/u<br/>
                ⏱ Tiempo estimado: 2 a 3 semanas<br/>
                🌡️ Temperatura óptima del agua: 18–22°C<br/>
                ⚗️ pH del agua: 5.8–6.2 + raizal o similar
              </div>
            </div>
            <div style={{display:"flex",gap:"8px",marginBottom:"18px"}}>
              {[1,2,3].map(w=>(
                <button key={w} onClick={()=>!esEjecutor && setWeek("enraizado",w)} style={{width:"60px",height:"56px",borderRadius:"12px",background:curEW===w?"#1a5a2e":"#141e14",color:curEW===w?"#fff":"#4a7a5a",fontSize:"14px",fontWeight:curEW===w?"700":"400",border:curEW===w?"2px solid #27ae60":"1px solid #2a4a2a",opacity:esEjecutor?0.7:1,cursor:esEjecutor?"default":"pointer"}}>S{w}</button>
              ))}
            </div>
            {[
              {w:1,t:"Colocar esquejes. Verificar temperatura del agua (18–22°C). Sin nutrición. Humedad ambiental alta 70–80%."},
              {w:2,t:"Primeras raíces visibles. Revisar turbidez del agua — cambiar si hay algas o mal olor. Las índicas suelen enraizar antes que las sativas."},
              {w:3,t:"Raíces de 2–4 cm → listas para trasplantar a 150cc. Las que no enraizaron: decidir si continuar o descartar."}
            ].filter(r=>r.w===curEW).map(r=>(
              <div key={r.w}>
                <div style={card({borderColor:"#1e4a2e"})}><div style={body}>{r.t}</div></div>
                <NB label={`NOTAS SEMANA ${curEW}`} value={db.enraizado?.notes?.[curEW]||""} onChange={t=>setNote("enraizado",curEW,t)}/>
              </div>
            ))}
          </div>
        )}
        ...
        {tab==="geneticas" && (
          <div className="fade">
            ...
            <div style={{fontSize:"13px",color:"#5a8aaa",marginBottom:"18px"}}>12 ENTRADAS · BATEKÚ #1/#2 · SATELLITE #1/#2 · BANANA BLAZE</div>
            ...
            <div style={card({background:"#121e2a",borderColor:"#1e3a5a"})}>
              <span style={lbl("#4a8aaa")}>USO ACTUAL EN CARPAS</span>
              {["flora1","flora2","flora3"].map(ck=>{
                const total=totalP(ck); const plantas=db[ck]?.plantas||{};
                return(
                  <div key={ck} style={{marginBottom:"14px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:"6px"}}>
                      <span style={{fontSize:"14px",color:CARPA_COLOR[ck],fontWeight:"600"}}>{CARPA_NAME[ck]}</span>
                      <span style={{fontSize:"13px",color:"#7a9a7a"}}>{total} plantas</span>
                    </div>
                    {total===0
                      ? <div style={{fontSize:"12px",color:"#3a5a3a",fontStyle:"italic"}}>Sin registro aún</div>
                      : <div style={{display:"flex",flexWrap:"wrap",gap:"6px"}}>
                          {Object.entries(plantas).filter(([,q])=>q>0).map(([gid,qty])=>{
                            const g=GENETICAS.find(x=>x.id===gid);
                            return g ? <span key={gid} style={{fontSize:"12px",background:`${g.color}22`,border:`1px solid ${g.color}66`,color:g.color,padding:"3px 10px",borderRadius:"14px"}}>{g.nombre} ×{qty}</span> : null;
                          })}
                        </div>
                    }
                  </div>
                ...
                <div key={g.id} style={{background:"#162416",border:`1px solid ${g.color}55`,borderRadius:"14px",padding:"18px",borderLeft:`5px solid ${g.color}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                    ...
                    <div style={{background:"#0e180e",border:`1px solid ${g.color}88`,borderRadius:"10px",padding:"8px 14px",textAlign:"center",minWidth:"64px"}}>
                      <div style={{fontSize:"26px",fontWeight:"700",color:g.color,lineHeight:1}}>{g.semanas}</div>
                      <div style={{fontSize:"11px",color:"#4a6a4a",marginTop:"3px"}}>semanas</div>
                    </div>
                  </div>
                  {g.origen && <div style={{fontSize:"12px",color:"#6a8a6a",marginBottom:"8px"}}>🏷 {g.origen}</div>}
                  <div style={{fontSize:"14px",color:"#90b870",marginBottom:"6px",lineHeight:"1.6"}}>🍋 {g.sabor}</div>
                  <div style={{fontSize:"14px",color:"#c0a860",lineHeight:"1.6"}}>✨ {g.efecto}</div>
                </div>
              ))}
            </div>
          </div>
                  ...
                  <div style={card({borderColor:"#5a4a10",background:"#1a1808"})}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                      ...
                      <div>
                        ...
                        <textarea value={newC.notas} onChange={e=>setNewC({...newC,notas:e.target.value})} placeholder="Observaciones..." style={{...inp(),minHeight:"70px",resize:"vertical",lineHeight:"1.6"}}/>
                      </div>
                      <div style={{display:"flex",gap:"10px"}}>
                        <button onClick={()=>saveCosecha(newC)} style={{flex:1,padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",background:"#3a6a10",color:"#dff0cf",border:"1px solid #5a9a20"}}>✓ Guardar</button>
                  ...
                  <div key={c.id} style={{background:"#141e10",border:`1px solid ${g?.color||"#2a4a2a"}44`,borderRadius:"14px",padding:"16px",borderLeft:`4px solid ${g?.color||"#4a6a4a"}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                      ...
                      <div style={{textAlign:"right"}}>
                        ...
                        {c.diasSecado && <div style={{fontSize:"12px",color:"#7a9a7a"}}>💨 {c.diasSecado} días</div>}
                      </div>
                    </div>
...
function NC({label,value,col}){
  return(
    <div style={{background:"#111e11",border:`1px solid ${col}44`,borderRadius:"12px",padding:"14px 16px"}}>
