...
const loteLabel = (n) => `Lote N° ${String(n).padStart(3,"0")}`;
  ...
  const confirmarMovimiento = async () => {
    ...
    try {
      ...
      if(esTotal){
        ...
        await supabase.from("lotes").update({
          cantidad_plantas: movingLote.cantidad_plantas - cant
        }).eq("id", movingLote.id);
        ...
        if(ins.data){
          await supabase.from("movimientos_lote").insert({
            ...
            usuario_id: usuario.id,
            tipo: "movimiento",
            motivo: `Avance parcial de ${cant}/${movingLote.cantidad_plantas} del lote original`
          });
        }
      }
      setMovingLote(null);
  ...
  return (
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
                          ...
                          return (
                            <div key={l.id} style={{background:"#111e11",borderRadius:"8px",padding:"10px 12px",border:`1px solid ${g?.color||"#2a4a2a"}44`}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px"}}>
                                <span style={{fontSize:"13px",color:g?.color||"#9aaa9a"}}>{g?.nombre||"Sin genética"}{l.numero_lote?` · ${loteLabel(l.numero_lote)}`:""}</span>
                  ...
                  <div>
                    ...
                    <select value={editingTarea.lote_id} onChange={e=>setEditingTarea({...editingTarea,lote_id:e.target.value})} style={inp()}>
                      ...
                      {lotesDelSector.map(l=>{
                        const g = geneticas.find(x=>x.id===l.genetica_id);
                        return <option key={l.id} value={l.id}>{g?.nombre||"Sin genética"}{l.numero_lote?` · ${loteLabel(l.numero_lote)}`:""} ({l.cantidad_plantas})</option>;
                      })}
                    </select>
                  </div>
                )}
      ...
      {(()=>{
        ...
        const grupoRegistros = tabsExtra.filter(t=>["_geneticas","_bajas","_cosechas"].includes(t.key));
