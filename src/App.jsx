...
function RaizApp() {
  ...
  const TareaCard = ({t}) => {
    ...
    const loteTrasplante = esTareaTrasplanteConLote ? lotes.find(x=>x.id===t.lote_id) : null;
    ...
    return (
      <div style={{background:"var(--c076)",border:`1px solid ${TAREA_ESTADO_COLOR[estadoMostrado]}55`,borderRadius:"14px",padding:"16px",borderLeft:`4px solid ${TAREA_PRIORIDAD_COLOR[t.prioridad]||"var(--c057)"}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}>
          <div>
            <div style={{fontSize:"15px",fontWeight:"600",color:"var(--c060)"}}>{t.numero_tarea?`#${t.numero_tarea} · `:""}{t.titulo}</div>
        ...
        {esTareaInventario && docVinculado && (
          <div style={{fontSize:"11px",color:"var(--c095)",marginBottom:"8px"}}>📄 Doc. N° {docVinculado.numero_documento} generado desde esta tarea</div>
        )}
        {esTareaTrasplanteConLote && loteTrasplante && (()=>{
          const gL = geneticas.find(x=>x.id===loteTrasplante.genetica_id);
          return <div style={{fontSize:"11px",color:"var(--c078)",marginBottom:"8px"}}>🪴 {gL?.nombre||"—"}{loteTrasplante.numero_lote?` · Lote N° ${loteTrasplante.numero_lote}`:""} · {t.cantidad_planificada||loteTrasplante.cantidad_plantas} plantas planificadas</div>;
        })()}
        ...
        <div style={{display:"flex",gap:"6px",marginBottom:"10px"}}>
          ...
          {esTareaTrasplanteConLote && loteTrasplante && puedeOperarTarea && t.estado==="en_progreso" && (
            <button onClick={()=>{ setMovingLote(loteTrasplante); setDestinoSub(""); setMovingMovida(String(t.cantidad_planificada||loteTrasplante.cantidad_plantas)); setModoMover(false); setModoRetroceso(false); setTareaTrasplanteId(t.id); }} style={{flex:1,padding:"7px",borderRadius:"7px",fontSize:"11px",background:"var(--c078)",color:"var(--c003)",border:"1px solid var(--c079)"}}>🪴 Trasplantar</button>
          )}
  ...
  const confirmarMovimiento = async () => {
    ...
    try {
      ...
      if(esTotal){
        ...
        const updO = await supabase.from("lotes").update(updOriginal).eq("id", movingLote.id);
      ...
      // Si el trasplante se abrió desde una tarea, y se movió al menos lo planificado
      // (o el lote completo si la tarea no especificaba cantidad), la tarea se completa sola —
      // ya cumplió lo que pedía. Si movió menos, queda en progreso para terminar después.
      if(tareaTrasplanteId){
        const tareaOrig = tareas.find(x=>x.id===tareaTrasplanteId);
        const umbral = tareaOrig?.cantidad_planificada || movingLote.cantidad_plantas;
        if(tareaOrig && cant>=umbral){
          await supabase.from("tareas").update({estado:"completada", fecha_ejecucion_real:hoyStr}).eq("id", tareaTrasplanteId);
        }
      }
  ...
  const saveTipoTarea = async () => {
    ...
    if(yaExiste){ setStatus(`ya existe un tipo de tarea llamado "${editingTipoTarea.nombre.trim()}"`); setTimeout(()=>setStatus(""),4000); return; }
      ...
      if(editingTarea.id) {
        ...
        if(loteIds.length>0){
          unidades = loteIds.map(loteId=>{
            ...
            return { sector_id: lote?.sector_actual_id||null, lote_id: loteId, cantidad_planificada: parseInt(lotesPlan[loteId])||null };
          });
        } else {
          const subIds = (editingTarea.sector_ids && editingTarea.sector_ids.length) ? editingTarea.sector_ids : [editingTarea.sector_id||null];
          unidades = subIds.map(sid=>({ sector_id: sid||null, lote_id: null, cantidad_planificada: null }));
        }
        ...
        for(const u of unidades){
          ...
          filas.push({...base, sector_id: u.sector_id, lote_id: u.lote_id || base.lote_id, cantidad_planificada: u.cantidad_planificada, grupo_id: grupoId, numero_tarea: numRpc.data});
        }
                  ...
                  <div style={{border:"1px solid "+colorDeFase(sp.orden)+"55",borderRadius:"12px",marginBottom:"16px",overflow:"hidden"}}>
                    <div onClick={()=>setTareasSectorAbierto(abierto?"":sp.id)} style={{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",background:colorDeFase(sp.orden)+"18"}}>
                      <span style={{fontSize:"13px",color:colorDeFase(sp.orden),fontWeight:"600"}}>
                        📋 Tareas pendientes · {pendientesSector.length}
                    ...
                    {abierto && (
                      <div style={{padding:"10px",display:"flex",flexDirection:"column",gap:"8px"}}>
                        {pendientesSector.map(t=>{
                          ...
                          const loteTraspSector = esTraspSector ? lotes.find(x=>x.id===t.lote_id) : null;
                          ...
                          return (
                            <div key={t.id} style={{background:"var(--c076)",borderRadius:"9px",padding:"10px 12px",borderLeft:"3px solid "+(atr?"var(--c010)":TAREA_PRIORIDAD_COLOR[t.prioridad]||"var(--c057)")}}>
                              <div style={{display:"flex",justifyContent:"space-between",gap:"8px",alignItems:"flex-start"}}>
                                <span style={{fontSize:"13px",color:"var(--c060)",fontWeight:"600"}}>{tObj?tObj.icono+" ":""}{t.numero_tarea?`#${t.numero_tarea} · `:""}{t.titulo}</span>
                              ...
                              {esTraspSector && loteTraspSector && (()=>{
                                const gL = geneticas.find(x=>x.id===loteTraspSector.genetica_id);
                                return <div style={{fontSize:"10px",color:"var(--c078)",marginTop:"3px"}}>🪴 {gL?.nombre||"—"}{loteTraspSector.numero_lote?` · Lote N° ${loteTraspSector.numero_lote}`:""} · {t.cantidad_planificada||loteTraspSector.cantidad_plantas} plantas</div>;
                              })()}
                              {puedeOperarSector && (t.estado==="pendiente"||t.estado==="en_progreso") && (
                                <div style={{display:"flex",gap:"6px",marginTop:"8px"}}>
                                  ...
                                  {esTraspSector && loteTraspSector && t.estado==="en_progreso" && (
                                    <button onClick={()=>{ setMovingLote(loteTraspSector); setDestinoSub(""); setMovingMovida(String(t.cantidad_planificada||loteTraspSector.cantidad_plantas)); setModoMover(false); setModoRetroceso(false); setTareaTrasplanteId(t.id); }} style={{flex:1,padding:"6px",borderRadius:"6px",fontSize:"11px",background:"var(--c078)",color:"var(--c003)",border:"1px solid var(--c079)"}}>🪴 Trasplantar</button>
                                  )}
          ...
          return (
            <div className="fade">
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"10px",marginBottom:"6px"}}>
                <div>
                  <div style={{fontFamily:"'DM Serif Display',serif",fontSize:"22px",color:"var(--c016)"}}>📋 Tareas</div>
                  <div style={{fontSize:"13px",color:"var(--c033)",marginTop:"2px"}}>{esEjecutor?"TUS TAREAS ASIGNADAS":"TODAS LAS TAREAS"}</div>
                </div>
                {!esEjecutor && (
                  <button onClick={()=>setEditingTarea(emptyTarea())} style={{flexShrink:0,padding:"8px 16px",borderRadius:"20px",fontSize:"12px",fontWeight:"600",background:"var(--c028)",color:"var(--c005)",border:"none"}}>+ Nueva</button>
                )}
              </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
                ...
                {esNueva ? (
                  <div>
                    ...
                    {subIdsElegidos.length>1 && <div style={{fontSize:"11px",color:"var(--c069)",marginTop:"6px"}}>Se van a crear {subIdsElegidos.length} tareas independientes (una por subsector), cada una con su propio estado.</div>}
                  </div>
                ) : (
                  <div>
                ...
                <div>
                  ...
                  {tiposDisponibles.length===0 ? (
                    <div style={{fontSize:"12px",color:"var(--c022)",background:"var(--c094)",borderRadius:"8px",padding:"10px 12px"}}>No hay tipos de tarea para {subIdsElegidos.length?"estos subsectores":"este subsector"} — agregalos en ⚙️ Mi Sala.</div>
                  ) : (
                ...
                {esTrasplanteNueva && (
                  <div>
                    ...
                    {lotesDisponiblesTrasplante.length===0 ? (
                      <div style={{fontSize:"12px",color:"var(--c022)",background:"var(--c094)",borderRadius:"8px",padding:"10px 12px"}}>No hay lotes activos en {subIdsElegidos.length>1?"estos subsectores":"este subsector"} todavía. La tarea queda genérica.</div>
                    ) : (
                      <>
                        ...
                        <div style={{fontSize:"11px",color:"var(--c069)",marginTop:"6px"}}>
                          {Object.keys(lotesPlan).length===0 ? "Sin elegir ninguno: la tarea queda genérica para todo el subsector (cualquier lote se puede trasplantar)." : `Se van a crear ${Object.keys(lotesPlan).length} tarea(s), una por lote elegido.`}
                        </div>
                      </>
                    )}
                  </div>
