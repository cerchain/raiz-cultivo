...
function RaizApp() {
  ...
  const saveUsuario = async () => {
    ...
    try {
      if(editingUsuario.id){
      ...
      } else if(editingUsuario.rol === "master_grower"){
        ...
        if(insU.data){
          const insS = await supabase.from("salas").insert({
            nombre: editingUsuario.salaNombre || `Sala de ${editingUsuario.nombre}`,
    ...
    <div style={{fontFamily:"'IBM Plex Mono',monospace",background:"#0e160e",minHeight:"100vh",color:"#dff0cf"}}>
      ...
      <div style={{background:"#111e11",borderBottom:"2px solid #2a4a2a",padding:"18px 20px 14px",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",marginBottom:"12px"}}>
          {sectoresPadre.map((sp,idx)=>(
            <Fragment key={sp.id}>
              <div onClick={()=>setTab(sp.id)} title={sp.nombre} style={{
                width:tab===sp.id?"13px":"9px", height:tab===sp.id?"13px":"9px",
                borderRadius:"50%", background:colorDeFase(sp.orden),
                boxShadow:tab===sp.id?`0 0 8px ${colorDeFase(sp.orden)}`:"none",
                cursor:"pointer", flexShrink:0, transition:"all .15s"
              }}/>
              {idx<sectoresPadre.length-1 && <div style={{flex:1,height:"2px",minWidth:"10px",background:`linear-gradient(90deg, ${colorDeFase(sp.orden)}, ${colorDeFase(sectoresPadre[idx+1].orden)})`,margin:"0 5px",opacity:0.7}}/>}
            </Fragment>
          ))}
        </div>
        <div style={{display:"flex",gap:"7px",flexWrap:"wrap"}}>
          {sectoresPadre.map(sp=>{
            ...
            return <button key={sp.id} onClick={()=>setTab(sp.id)} style={{padding:"8px 15px",borderRadius:"22px",fontSize:"12px",background:tab===sp.id?c:"#192919",color:tab===sp.id?"#fff":"#7aaa7a",border:`1px solid ${tab===sp.id?c:"#2e4e2e"}`,fontWeight:tab===sp.id?"600":"400"}}>{sp.nombre}</button>;
          })}
          {tabsExtra.map(t=>(
            <button key={t.key} onClick={()=>setTab(t.key)} style={{padding:"8px 15px",borderRadius:"22px",fontSize:"12px",background:tab===t.key?`${t.col}22`:"transparent",color:tab===t.key?t.col:"#5a7a6a",border:`1px solid ${tab===t.key?t.col:"#2a3a32"}`,fontWeight:tab===t.key?"600":"400"}}>{t.label}</button>
          ))}
        </div>
      </div>
            ...
            <div style={card({borderColor:"#6a3a8a",background:"#1a0e22"})}>
              ...
              {editingUsuario ? (
                <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                  ...
                  {!editingUsuario.id && (
                    <div>
                      <div style={{...lbl("#a888c8"),marginBottom:"6px"}}>ROL</div>
                      <select value={editingUsuario.rol} onChange={e=>setEditingUsuario({...editingUsuario,rol:e.target.value})} style={inp()}>
                        <option value="ejecutor">Ejecutor (en tu Sala)</option>
                        <option value="master_grower">Master Grower nuevo (Sala propia)</option>
                      </select>
                    </div>
                  )}
                  {!editingUsuario.id && editingUsuario.rol==="master_grower" && (
                    <div>
                      <div style={{...lbl("#a888c8"),marginBottom:"6px"}}>NOMBRE DE SU SALA</div>
                      <input value={editingUsuario.salaNombre} onChange={e=>setEditingUsuario({...editingUsuario,salaNombre:e.target.value})} placeholder={`Sala de ${editingUsuario.nombre||"..."}`} style={inp()}/>
                      <div style={{fontSize:"11px",color:"#7a5a9a",marginTop:"6px"}}>Se crea con sus 4 sectores (Enraizado/Vegetativo/Floración/Secado) ya armados — no va a verse ni a poder verte a vos ni a tus datos.</div>
                    </div>
                  )}
                  <div style={{display:"flex",gap:"10px"}}>
